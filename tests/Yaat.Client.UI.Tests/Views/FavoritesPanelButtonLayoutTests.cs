...
public class FavoritesPanelButtonLayoutTests
{
    ...
    private static (Window Window, MainViewModel Vm, UniformGrid Grid, int OriginalColumns) ShowGroundPalette(FavoriteCommand[] favorites)
    {
        ...
        ScrollViewer scroll = Assert.IsType<ScrollViewer>((tabs.SelectedItem as TabItem)?.Content);
        UniformGrid grid = Assert.IsType<UniformGrid>(scroll.Content);
    ...
    private static void ClosePalette(Window window, MainViewModel vm, int originalColumns, FavoriteCommand[] favorites)
    {
        ...
        window.Close();
        Dispatcher.UIThread.RunJobs();
        foreach (FavoriteCommand? favorite in favorites.Where(f => !string.IsNullOrEmpty(f.Id)))
        {
    ...
    public void PaletteButtons_FillTheirCells_AndTouchHorizontally()
    {
        ...
        try
        {
            ...
            double cellWidth = grid.Bounds.Width / Columns;
            ...
            Point? firstOrigin = buttons[0].TranslatePoint(new Point(0, 0), grid);
            Point? secondOrigin = buttons[1].TranslatePoint(new Point(0, 0), grid);
            Assert.NotNull(firstOrigin);
            Assert.NotNull(secondOrigin);

            double firstRightEdge = firstOrigin.Value.X + buttons[0].Bounds.Width;
            Assert.True(
    ...
    public void PaletteButtons_OfDifferentHeights_FillTheirCellsVertically()
    {
        ...
        try
        {
            Button shortButton = FindButton(grid, "LayoutShort425");
            Button tallButton = FindButton(grid, "LayoutTall425");
            ...
            int rows = (int)Math.Ceiling(grid.Children.Count / (double)Columns);
            double cellHeight = grid.Bounds.Height / rows;
            ...
            foreach (Button? button in new[] { shortButton, tallButton })
            {
                Assert.True(
