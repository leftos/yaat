...
public class GroundMovementMenuTests
{
    ...
    /// </summary>
    private static void AssertHeaderSequence(ContextMenu menu, params string[] expected)
    {
        Assert.Equal(expected, Headers(menu));
    }
    ...
    [InlineData("Holding After Pushback")]
    [InlineData("Holding In Position")]
    public void GroundMenu_OffersResumeTaxi_WhenHeld(string phase)
    {
        Assert.Contains("Resume taxi", Headers(BuildGroundMenu(phase, held: true)));
    }
    ...
    [AvaloniaFact]
    public void AircraftListMenu_Taxiing_PinsHeaderSequence()
    {
        AssertHeaderSequence(BuildAircraftListMenu("Taxiing", held: false), "Hold position");
    }
