...
// Runs when the test assembly loads, before any YAAT static that reads
// YaatPaths.AppDataRoot. Redirects every path YAAT derives from
// %LOCALAPPDATA%/yaat to a unique temp folder so tests never touch the
// developer's real prefs, logs, or vNAS cache. SpeechSampleStoreTests builds a
// UserPreferences and calls SetSpeechSampleSettings, which persists to disk;
// without this redirect that write lands on the real shared preferences.json
// and races other processes on the fixed .tmp scratch name (IOException in Save).
//
...
{
    private const string RootName = "yaat-client-tests";
