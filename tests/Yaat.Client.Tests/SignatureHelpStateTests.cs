...
{
    private static CommandSignatureSet MakeSet(params CommandSignature[] sigs) => new(sigs);

    private static CommandParameter Lit(string name) => new(name, "literal", false, IsLiteral: true);

    private static CommandSignature MakeSig(string label, CommandParameter[] parameters, string? hint = null) =>
        new(CanonicalCommandType.FlyHeading, label, ["FH"], parameters, hint);
