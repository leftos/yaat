...
public sealed class TrafficAdvisoryMatcherTests
{
    ...
    public void UserScenario_OffByOneMileAndOneBucket_ResolvesIntendedTargetAsExact()
    {
        ...
        AircraftState recipient = Recipient(headingTrue: 150);
        AircraftState intended = TargetAt("N9225L", recipient, clock: 11, distanceNm: 3.0, trackTrue: 180, altitude: 2100);
        AircraftState other = TargetAt("N346G", recipient, clock: 12, distanceNm: 4.0, trackTrue: 180, altitude: 2500);
    ...
    {
        AircraftState recipient = Recipient(headingTrue: 150);
        AircraftState target = TargetAt("N9225L", recipient, clock: 9, distanceNm: 3.0, trackTrue: 180, altitude: 2100);
    ...
    {
        AircraftState recipient = Recipient(headingTrue: 150);
        AircraftState target = TargetAt("N9225L", recipient, clock: 6, distanceNm: 10.0, trackTrue: 0, altitude: 6000);
    ...
    {
        AircraftState recipient = Recipient(headingTrue: 150);
        AircraftState target = TargetAt("N9225L", recipient, clock: 11, distanceNm: 2.0, trackTrue: 180, altitude: 7500);
    ...
    public void ManeuveringRecipient_WidensClockGate()
    {
        ...
        AircraftState stable = Recipient(headingTrue: 150, bankDeg: 0);
        AircraftState banked = Recipient(headingTrue: 150, bankDeg: 13);
        AircraftState targetForStable = TargetAt("N9225L", stable, clock: 8, distanceNm: 3.0, trackTrue: 180, altitude: 2100);
        AircraftState targetForBanked = TargetAt("N9225L", banked, clock: 8, distanceNm: 3.0, trackTrue: 180, altitude: 2100);
    ...
    {
        AircraftState recipient = Recipient(headingTrue: 150);
        AircraftState noseRight = TargetAt("N111", recipient, clock: 1, distanceNm: 2.0, trackTrue: 180, altitude: 2000);
        AircraftState behind = TargetAt("N222", recipient, clock: 6, distanceNm: 2.0, trackTrue: 0, altitude: 2000);
    ...
    {
        RunwayInfo runway = TestRunwayFactory.Make(designator: "28R", heading: 280);
        AircraftState recipient = Recipient(headingTrue: 280);
        AircraftState onBase = PatternCandidate("N111", runway, MakeBasePhase(runway, PatternDirection.Right), distanceNm: 2.0);
        AircraftState onFinal = PatternCandidate("N222", runway, new FinalApproachPhase(), distanceNm: 5.0);
    ...
    public void Pattern_SideMismatch_ReturnsNull()
    {
        RunwayInfo runway = TestRunwayFactory.Make(designator: "28R", heading: 280);
        AircraftState recipient = Recipient(headingTrue: 280);
        AircraftState onLeftBase = PatternCandidate("N333", runway, MakeBasePhase(runway, PatternDirection.Left), distanceNm: 2.0);
    ...
    public void Pattern_DistanceBeyondGate_ReturnsNull()
    {
        RunwayInfo runway = TestRunwayFactory.Make(designator: "28R", heading: 280);
        AircraftState recipient = Recipient(headingTrue: 280);
        AircraftState farBase = PatternCandidate("N444", runway, MakeBasePhase(runway, PatternDirection.Right), distanceNm: 6.0);
    ...
    public void Landmark_NearestWithinRadius_ResolvesAsExact()
    {
        ...
        AircraftState near = AircraftAt("N111", GeoMath.ProjectPoint(landmark, new TrueHeading(0), 0.5));
        AircraftState far = AircraftAt("N222", GeoMath.ProjectPoint(landmark, new TrueHeading(0), 5.0));
    ...
    private static BasePhase MakeBasePhase(RunwayInfo runway, PatternDirection side) =>
        new() { Waypoints = PatternGeometry.Compute(runway, AircraftCategory.Piston, "", 0, side, null, null, null, authoredRunway: null) };
    ...
    private static AircraftState PatternCandidate(string callsign, RunwayInfo runway, Phase phase, double distanceNm)
    {
        ...
        AircraftState ac = AircraftAt(callsign, GeoMath.ProjectPoint(threshold, new TrueHeading(60), distanceNm));
    ...
    private static AircraftState TargetAt(string callsign, AircraftState recipient, int clock, double distanceNm, double trackTrue, double altitude)
    {
        ...
        LatLon position = GeoMath.ProjectPoint(recipient.Position, new TrueHeading(trueBearing), distanceNm);
