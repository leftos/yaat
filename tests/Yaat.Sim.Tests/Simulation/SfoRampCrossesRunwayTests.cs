...
public class SfoRampCrossesRunwayTests(ITestOutputHelper output)
{
    ...
    public void Diagnostic_FindN70234()
    {
        ...
        for (int t = 0; t <= recording.TotalElapsedSeconds; t += 50)
        {
            ...
            {
                output.WriteLine(
                    $"t={t}: N70234 found at ({ac.Latitude:F6}, {ac.Longitude:F6}) "
    ...
    public void TaxiCommand_AcrossRunways_ShouldFail()
    {
        ...
        output.WriteLine(
            $"N70234 at ({aircraft.Latitude:F6}, {aircraft.Longitude:F6}) onGround={aircraft.IsOnGround}");
        ...
        Assert.False(
            result.Success,
            $"TAXI A E 28R HS E should fail (route crosses runways) but succeeded: {result.Message}");
    }
}
