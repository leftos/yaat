...
public class EvaCappDescentBugTests(ITestOutputHelper output)
{
    ...
    /// <summary>Diagnostic: log per-tick state after the first CAPP at t=894.</summary>
    [Fact]
    public void Diagnostic_FirstCapp_LogProfile()
    {
        ...
        output.WriteLine($"--- After 1st CAPP (replay to t={FirstCappElapsedS}) ---");
        LogState(0, aircraft);
    ...
    /// <summary>Diagnostic: log per-tick state after the second CAPP at t=1008.</summary>
    [Fact]
    public void Diagnostic_SecondCapp_LogProfile()
    {
        var recording = LoadRecording();
        var engine = BuildEngine();
        if (recording is null || engine is null)
        ...
        engine.Replay(recording, SecondCappElapsedS);

        var aircraft = engine.FindAircraft(Callsign);
        Assert.NotNull(aircraft);
        output.WriteLine($"--- After 2nd CAPP (replay to t={SecondCappElapsedS}) ---");
        LogState(0, aircraft);
