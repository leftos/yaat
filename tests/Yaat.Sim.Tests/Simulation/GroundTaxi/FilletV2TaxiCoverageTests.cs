...
///
/// Runs the existing OAK + SFO <see cref="TaxiCoverageData"/> smoke pairs — which
...
public class FilletV2TaxiCoverageTests(ITestOutputHelper output)
{
    public static IEnumerable<object[]> Pairs() =>
        TaxiCoverageData.OakSmoke.Concat(TaxiCoverageData.SfoSmoke).Select(p => new object[] { p.PairId, p });
