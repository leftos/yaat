...
public class SnapshotTreeDiffTests
{
    ...
    {
        IReadOnlyList<SnapshotDivergence> divergences = Diff(
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"Scenario":{"Nested":{"A":1}}}""", """{"Scenario":{"Nested":{"A":2}}}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"A":1}""", """{"A":1,"B":"x"}"""));
    ...
    public void PropertyWrittenAsNull_VersusAbsentEntirely_Reports()
    {
        ...
        SnapshotDivergence divergence = Assert.Single(Diff("""{"A":1,"B":null}""", """{"A":1}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"A":null}""", """{"A":5}"""));
    ...
    public void DuplicateCallsignsInOneSnapshot_BothSurface_RatherThanTheLaterReplacingTheEarlier()
    {
        ...
        SnapshotDivergence divergence = Assert.Single(divergences);
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"A":{"X":1}}""", """{"A":5}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(
            Diff("""{"Aircraft":[{"Callsign":"AAL1"},{"Callsign":"SWA2"}]}""", """{"Aircraft":[{"Callsign":"AAL1"}]}""")
        );
    ...
    public void NonAircraftList_StaysIndexKeyed_BecauseItsIndexIsSemantic()
    {
        ...
        IReadOnlyList<SnapshotDivergence> divergences = Diff(
            """{"Phases":[{"Name":"Taxiing"},{"Name":"Landing"}]}""",
            """{"Phases":[{"Name":"Landing"},{"Name":"Taxiing"}]}"""
        );
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"Blocks":[1,2]}""", """{"Blocks":[1]}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"Phases":[{"$type":"Taxiing"}]}""", """{"Phases":[{"$type":"HoldingShort"}]}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"AircraftJson":"not json"}""", """{"AircraftJson":"also not json"}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"TargetNodeId":-101}""", """{"TargetNodeId":42}"""));
    ...
    public void SmallNegativeSentinelOnANodeIdProperty_IsNotNormalized()
    {
        ...
        SnapshotDivergence divergence = Assert.Single(Diff("""{"TargetNodeId":-1}""", """{"TargetNodeId":-105}"""));
    ...
    {
        SnapshotDivergence divergence = Assert.Single(Diff("""{"VerticalSpeed":-101}""", """{"VerticalSpeed":-207}"""));
