...
public class ArcTurnDirectionTests
{
    ...
    public void ExplicitRight_AlwaysTurnsRight(char turn, double start, double end) => Assert.True(GeoMath.ResolveArcTurnRight(turn, start, end));

    [Theory]
    [InlineData('L', 0.0, 90.0)] // minor arc is right, but explicit L is honored
    [InlineData('l', 350.0, 10.0)]
    public void ExplicitLeft_AlwaysTurnsLeft(char turn, double start, double end) => Assert.False(GeoMath.ResolveArcTurnRight(turn, start, end));
    ...
    public void NullTurn_PicksMinorArc_Right(double start, double end) => Assert.True(GeoMath.ResolveArcTurnRight(null, start, end));
    ...
    public void NullTurn_PicksMinorArc_Left(double start, double end) => Assert.False(GeoMath.ResolveArcTurnRight(null, start, end));
}
