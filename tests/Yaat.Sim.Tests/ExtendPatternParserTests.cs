...
public class ExtendPatternParserTests
{
    ...
    {
        ParseResult<ParsedCommand> result = CommandParser.Parse("EXT");
        Assert.True(result.IsSuccess, result.Reason);
        ExtendPatternCommand ext = Assert.IsType<ExtendPatternCommand>(result.Value);
        Assert.Null(ext.Leg);
    }
    ...
    {
        ParseResult<ParsedCommand> result = CommandParser.Parse("EXTEND");
        Assert.True(result.IsSuccess, result.Reason);
        ExtendPatternCommand ext = Assert.IsType<ExtendPatternCommand>(result.Value);
        Assert.Null(ext.Leg);
    }
