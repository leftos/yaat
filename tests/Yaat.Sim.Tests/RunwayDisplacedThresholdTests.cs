...
public class RunwayDisplacedThresholdTests
{
    ...
    {
        AirportGroundLayout layout = GeoJsonParser.Parse("TEST", DisplacedRunwayGeoJson, null);

        GroundRunway rwy = layout.Runways.Single();
    ...
    public void Parse_AbsentOrMalformedThreshold_LeavesDisplacementZero(string thresholdProperty)
    {
        ...
        GroundRunway rwy = GeoJsonParser.Parse("TEST", json, null).Runways.Single();
    ...
    public void LandingThresholdForEnd_ShiftsDownfieldAlongLandingCourse()
    {
        ...
        (double Lat, double Lon) pavement36 = rwy.Coordinates[0];
        (double Lat, double Lon) pavement18 = rwy.Coordinates[^1];

        (LatLon Threshold, double LandingCourseDeg)? landing36 = rwy.LandingThresholdForEnd("36");
        ...
        (LatLon Threshold, double LandingCourseDeg)? landing18 = rwy.LandingThresholdForEnd("18");
    ...
    public void LandingThresholdForEnd_WithoutDisplacement_IsThePavementEnd()
    {
        ...
        AirportGroundLayout layout = GeoJsonParser.Parse("MIA", File.ReadAllText(path), "MIA");

        // KMIA 8L/26R has "threshold": null in the vNAS map.
        GroundRunway rwy = layout.Runways.First(r => r.Name == "8L - 26R");
        (LatLon Threshold, double LandingCourseDeg)? landing = rwy.LandingThresholdForEnd("26R");
    ...
    public void LandingThresholdForEnd_Mia30_AppliesThe957FootDisplacement()
    {
        ...
        GroundRunway rwy = layout.Runways.First(r => r.Name == "12 - 30");
        Assert.Equal(0, rwy.ThresholdDisplacementForEnd("12"));
        Assert.Equal(957, rwy.ThresholdDisplacementForEnd("30"));

        (double Lat, double Lon) pavement30 = rwy.Coordinates[^1];
        (LatLon Threshold, double LandingCourseDeg)? landing30 = rwy.LandingThresholdForEnd("30");
        ...
        (LatLon Threshold, double LandingCourseDeg)? landing12 = rwy.LandingThresholdForEnd("12");
    ...
    {
        GroundRunway rwy = GeoJsonParser.Parse("TEST", DisplacedRunwayGeoJson, null).Runways.Single();
