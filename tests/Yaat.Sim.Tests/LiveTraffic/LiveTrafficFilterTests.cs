...
public class LiveTrafficFilterTests
{
    ...
    {
        Assert.True(LiveTrafficFilter.TryParse(null, out LiveTrafficFilter? fromNull, out _));
        Assert.True(LiveTrafficFilter.TryParse("", out LiveTrafficFilter? fromEmpty, out _));
    ...
    {
        string text = "RULES=VFR;APT=OAK,SFO;MATCH=DEP;NOPLAN=1;CENTER=OAK090010;RADIUS=15";
        Assert.True(LiveTrafficFilter.TryParse(text, out LiveTrafficFilter? filter, out string? error));
        Assert.Null(error);
    ...
    {
        Assert.True(LiveTrafficFilter.TryParse(" rules=ifr ; apt = oak , sfo , OAK ", out LiveTrafficFilter? filter, out _));
    ...
    {
        Assert.False(LiveTrafficFilter.TryParse(text, out _, out string? error));
    ...
    {
        Assert.True(LiveTrafficFilter.TryParse("MATCH=DEP;NOPLAN=1", out LiveTrafficFilter? filter, out _));
    ...
    public void Describe_ReadsAsASentenceFragment()
    {
        ...
        Assert.True(LiveTrafficFilter.TryParse("RULES=VFR;APT=OAK,SFO;NOPLAN=1;CENTER=SUNOL;RADIUS=12.5", out LiveTrafficFilter? filter, out _));
