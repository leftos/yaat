...
public class HoldForReleaseGateTests
{
    ...
    private static CommandResult Dispatch(string command, AircraftState ac)
    {
        ...
        DispatchContext ctx = TestDispatch.Context(new System.Random(0));
    ...
    {
        AircraftState ac = HoldingShortDeparture("N1", held: true);

        CommandResult result = Dispatch("CTO", ac);
    ...
    {
        AircraftState ac = HoldingShortDeparture("N2", held: true);

        CommandResult result = Dispatch("LUAW", ac);
    ...
    {
        AircraftState ac = HoldingShortDeparture("N3", held: false);
