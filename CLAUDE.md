...
- **Branching**: Never commit directly to `master`. Always create a feature branch and work in a **git worktree** — never check out the feature branch in the main repo directory. Use `git worktree add ../yaat-<short-name> feat/<branch>` (or `wt switch <branch>`). This keeps the main repo on `master` and avoids conflicts with other agents or the user.
- **Cross-repo worktrees**: When a feature spans both yaat and yaat-server, create worktrees in both repos (e.g., `../yaat-auto-rwy` and `../yaat-server-auto-rwy`). The server's `Directory.Build.props` resolves `YaatSimProject` via `../yaat/` — from a server worktree this points to the main yaat repo (on master), not your yaat worktree. Override at build/test time: `dotnet build "-p:YaatSimProject=X:/dev/yaat-<worktree>/src/Yaat.Sim/Yaat.Sim.csproj"`. When the server PR depends on yaat changes not yet merged to master, update the server's `extern/yaat` submodule to point to the yaat feature branch commit (see yaat-server CLAUDE.md for details).
