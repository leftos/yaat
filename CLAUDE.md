...
- **Zero warnings**: build with `-p:TreatWarningsAsErrors=true`; run every `dotnet build`/`dotnet test`/`dotnet run` through `tools/gate.sh <log> <command...>`, which keeps the full output in the log, prints only the tail, and exits with the command's own status
...
- Taxi (kts/s, physics is the only ground-speed integrator): accel 1.0 for every category — the breakaway rate, not the sustained idle rate (a light single's breakaway blip gives it *better* ground thrust-to-weight than a loaded transport, so it cannot be slower; re-reviewed 2026-09-15, piston/heli 0.6 → 1.0); brake Jet/TP 5.0, Piston/Heli 2.0. Ground snap window = one sub-tick of change. Tug moves (`PUSH`/`PUSHM`, every category): accel 0.3, brake 1.0 — towbar rates published by `PushbackPhase` as `DesiredAccelRate`/`DesiredDecelRate`; judgement calls under AC 00-65A §11.17, which gives no figure. The tug holds 5 kt (`PushbackSpeed`) through turns, the main gear commanded at 5·cos(steer) (B738 routine radius 3.5 kt, tight 1.9 kt); the 3 kt align creep covers only the last 30 ft of a creep move (`PushbackPhase.AlignCreepFt`).
...
- **No synthetic data in tests**: Use `TestVnasData.EnsureInitialized()` (loads NavData.dat and CIFP via `NavDataPathResolver` / `CifpPathResolver` — one resolve per test process at assembly load in `tests/Yaat.Sim.Tests/ModuleInit.cs`, with bundled `TestData/NavData.dat` and `FAACIFP18.gz` as offline fallbacks). Refresh pins: `python tools/refresh-navdata.py` (NavData); scenarios are downloaded by the yaat-server repo's `uv run python tools/validate-all-scenarios.py`. **NavData and CIFP are hard preconditions: `ModuleInit` throws if it cannot resolve either** — most of the suite asserts against real navdata and procedures, so a run without them is green having proved nothing. NavData resolves offline when a local copy exists (the committed `TestData/NavData.dat` or the user cache) and only downloads when neither is; resolving online would fetch the vNAS config over HTTPS on every test process (~240 ms of TLS handshake), and a test asserts `NavDataPathResolver.ConfigFetchCount` stays zero. CIFP keeps downloads on (it is AIRAC-cycle-specific and already checks its cache before the network); `YAAT_SKIP_CIFP_DOWNLOAD=1` forces it offline. Synthetic stubs hide integration problems. Other missing test data (layouts, recordings, ARTCC configs) still silently skips.
- **Test access**: Making `internal` members `public` for tests is fine. No reflection or `InternalsVisibleTo` hacks.
- **SimLog in tests**: `SimLog` falls back to `NullLoggerFactory` by default — all Yaat.Sim log output is silently swallowed in tests. To see logs, use `SimLogBuilder.CreateForTest(output).EnableCategory("ClassName", LogLevel.Debug).InitializeSimLog()`. Live test output only reaches the console when the test project runs as a process: `tools/gate.sh .tmp/test-output.log dotnet run --project tests/Yaat.Sim.Tests -c Release -- --filter-method "*<TestName>*" --show-live-output on`, then read the log (`dotnet test` reports only summaries and failures).
...
- **Style is a build gate**: `.editorconfig` is the language-conventions skill's canonical C# file plus this repo's ReSharper block; `Directory.Build.props` sets `EnforceCodeStyleInBuild` + `TreatWarningsAsErrors`, so a `warning`-level rule breaks the build: braces, file-scoped namespace matching the folder (IDE0130 skips types with a source-generated partial half — match the folder anyway), `var` only when the right-hand side names the type (`new T()`, a cast) and the explicit type everywhere else, expression bodies for single-line methods, primary constructors, collection expressions. CI additionally fails on any info-level style finding (`dotnet format style yaat.slnx --verify-no-changes --severity info`) and on `dotnet csharpier check .`. Tuple deconstruction (IDE0042) is off: a named tuple local keeps its name.
...
- **Capture all output, print only the tail**: run every `dotnet build`/`dotnet test`/`dotnet run` through `tools/gate.sh .tmp/<name>.log <command...>`. The full output lives in the log; the screen gets the last 20 lines (`GATE_TAIL=<n>` for more), the failure lines on red, and the command's own exit status. For more of the output, read or `rg` the log; never re-run the command. Use a generic name (e.g. `.tmp/build.log`) unless you need to compare multiple runs, then use a unique name. Every printed line is re-read on every later turn of the session, which is why the whole log stays out of context and why the guard denies `| tee`.
- **Build after edits, test after fixes**: Run `tools/gate.sh .tmp/build.log dotnet build -p:TreatWarningsAsErrors=true` after edits and `tools/gate.sh .tmp/test.log timeout 120 dotnet test` after fixes (`timeout 30` when filtering to a few tests). Ensure zero warnings and all tests pass before committing.
...
- **prek's build hook is cross-repo**: `prek.toml`'s `dotnet build` compiles `yaat.slnx`, which names no `yaat-server` project but pulls `Yaat.Server` in through `tools/Yaat.GuideCapture`'s conditional `ProjectReference` to the sibling checkout. A *partial* commit (`git commit -- <subset>`) stashes your other yaat changes, so if yaat-server's on-disk code depends on a just-stashed `Yaat.Sim` change, the hook build fails with `CS0246`/signature errors from `yaat-server`. Before such a partial commit: `git -C "$(git rev-parse --path-format=absolute --git-common-dir)/../../yaat-server" stash push -u` (the sibling of the main checkout, also from a worktree), commit, then `stash pop`. Likewise a dependency pin that's only modified-not-committed gets reverted by the stash during the hook build — commit the pin, don't just leave it in the tree.
- **Roslyn tools stop at the repo boundary**: the `roslyn` MCP server (`claude-roslyn-lsp` plugin) bounds its results and writes to its working directory, so from a yaat session `findReferences`/`resolveSymbol` never list a `yaat-server` file and a count of "N references" is yaat-only. `.claude/yaat-with-server.slnx` (all of `yaat.slnx` plus the sibling's projects; select it with `CLAUDE_ROSLYN_LSP_SOLUTION=.claude/yaat-with-server.slnx` in `.claude/settings.local.json`) makes Roslyn analyse both repos: a `renameSymbol` that reaches the server is then refused whole with "outside the workspace root" instead of applying the yaat half and leaving the server broken. With `yaat.slnx` loaded the same rename applies silently. Either way, before renaming or re-signing a public `Yaat.Sim` member, `rg` its name under `../yaat-server`, make the server edits by hand, and gate with `pwsh tools/test-all.ps1`.
