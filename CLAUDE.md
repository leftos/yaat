...
- **Test timeouts**: Always wrap `dotnet test` in a `timeout` to catch soft hangs from broken graph topology or infinite pathfinder loops. Use **`timeout 30`** for a filtered/targeted run (`--filter ...`) — a targeted YAAT sim run that hasn't finished in 30s is stuck, not slow. Use **`timeout 120`** for a bare full-suite run, which legitimately needs longer. Never run without one.
