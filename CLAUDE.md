...
- **No synthetic navdata in tests**: Do not fabricate approach procedures, transitions, fixes, STARs, or other navdata for tests. Synthetic navdata is confusing — devs cannot verify intended behavior against real charts or aviation standards. Always use the real `NavData.dat` and `FAACIFP18.gz` from `tests/Yaat.Sim.Tests/TestData/`. Even minimal tests can load the real data, resolve a real approach, and assert against real fix names. If the test data files are absent, silently skip (early return).
