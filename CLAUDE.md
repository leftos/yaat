...
**Client→Server (scenario/sim):** `LoadScenario`, `UnloadScenarioAircraft`, `ConfirmUnloadScenario`, `SendCommand`, `SetAutoAcceptDelay`, `SetAutoDeleteMode`, `SendChat`, `LoadWeather(weatherJson) → CommandResultDto`, `ClearWeather()`

**Client→Server (data queries):** `GetAirportGroundLayout(airportId)`, `GetFacilityVideoMaps(artccId, facilityId)`, `GetFacilityVideoMapsForArtcc(artccId)`, `GetServerLogPath`

**Client→Server (assignments):** `AssignAircraft(callsigns, targetConnectionId)`, `UnassignAircraft(callsigns)`, `GetAircraftAssignments`

**Client→Server (room management):** `KickMember(cid)`
...
**Server→Client:** `AircraftUpdated`, `AircraftSpawned`, `AircraftDeleted`, `SimulationStateChanged`, `TerminalBroadcast`, `RoomMemberChanged`, `CrcLobbyChanged`, `CrcRoomMembersChanged`, `WeatherChanged(WeatherChangedDto)`, `PositionDisplayChanged(PositionDisplayConfigDto)`, `AircraftAssignmentsChanged(AircraftAssignmentsDto)`, `KickedFromRoom(string)`

### REST API

**`GET /api/version`** — Returns `{ server, client }` commit hashes. Populated via `YAAT_SERVER_COMMIT`/`YAAT_CLIENT_COMMIT` env vars (set at Docker build time by `deploy-to-droplet.ps1`). Returns `"dev"` when running locally. Used by `start.ps1 -Sync` / `start.sh --sync` to check out the matching client commit.
...
- **TDD for bug fixes**: Use test-driven development for most issue fixes. Write the failing test first (unit or E2E per `docs/e2e-tdd-issue-debugging.md`), confirm it fails, apply the fix, then confirm it passes. This proves the test actually catches the bug.
- **TDD for simulation changes**: Any changes to flight physics, commands, or navdata handling must use TDD. Write tests first to define the expected behavior, then implement. This applies to new commands, command parsing changes, phase transitions, performance constants, navigation logic, and anything in `FlightPhysics`, `CommandDispatcher`, `ApproachCommandHandler`, `ApproachCommandParser`, or similar simulation-critical code.
- **No guessing at root causes**: When debugging, reproduce the problem with a test first. Use real airport layouts and E2E tests (see `docs/e2e-tdd-issue-debugging.md`). Do not speculate about causes — write a failing test that demonstrates the bug, then fix it.
- **No synthetic data in tests**: Always prefer `TestVnasData.EnsureInitialized()` (which loads the real `NavData.dat` and `FAACIFP18.gz` from `tests/Yaat.Sim.Tests/TestData/`) over fabricating inline data. The app doesn't work with incomplete data — synthetic stubs hide real integration problems and make tests unverifiable against real charts or aviation standards. Synthetic data should only be used when absolutely necessary for a specific test scenario that cannot be expressed with real data. If the test data files are absent, silently skip (early return).
...
- **Test access**: Making `internal` members `public` is fine if tests need them. Prefer that over reflection, `InternalsVisibleTo`, or hacky workarounds. This is unreleased software — API surface purity doesn't matter.
- **No optional parameters**: Do not use optional/default parameters (`= null`, `= false`, etc.) unless absolutely necessary. Optional parameters hide missing integration — callers silently get a default when they should be forced to pass the real value. Make parameters required so the compiler enforces wiring at every call site. When you find an existing optional parameter causing a bug, make it required and fix all callers.
- **No repurposing DTO fields**: Never reuse an existing DTO or snapshot field for a different meaning. Add a new field with a clear name. If an old field is truly dead, remove it entirely — don't repurpose it.
...
- **Boolean expressions**: Use parentheses to disambiguate complex boolean expressions — e.g., `(a.IsUnsupported) || (ac.Altitude >= fieldElev + threshold)` not `a.IsUnsupported || ac.Altitude >= fieldElev + threshold`. Makes operator precedence explicit for readers.
- **Line width**: 150 characters, not 80 or 120. CSharpier is configured accordingly.
- **No `-q` flag on dotnet commands**: Never pass `-q` to `dotnet build`, `dotnet test`, or other dotnet commands — it causes spurious errors.
...
- **Pre-commit formatting**: Run `dotnet format style`, `dotnet format analyzers`, then `dotnet csharpier format .` before each commit, followed by a final `dotnet build -p:TreatWarningsAsErrors=true` to verify nothing broke. Do NOT run bare `dotnet format` (its whitespace rules fight with CSharpier). Do NOT pass `-v q`, `--nologo`, or any extra flags to `dotnet format` — the formatters are quiet by default and extra flags cause exceptions.
- **Cross-repo test runner**: Use `pwsh tools/test-all.ps1` to build and test both yaat and yaat-server in one shot. Run this to quickly verify nothing is broken across both repos.
...
- **Cross-repo issue references**: GitHub issues are tracked on the **yaat** repo. Never use `Closes #N` or `Fixes #N` in yaat-server commit messages — that would reference yaat-server's own issue tracker. Instead, use the full URL: `Closes https://github.com/leftos/yaat/issues/N`. Or put the `Closes #N` keyword only in the yaat repo commit.
