...
- [`docs/scenario-validation.md`](docs/scenario-validation.md) — Validating scenario preset commands. Scenarios are downloaded by the yaat-server repo's `tools/validate-all-scenarios.py` (it writes into this repo's `tests/Yaat.Sim.Tests/TestData/Scenarios/`); test via `VnasScenarioParseTests`.
...
- [`docs/command-handlers.md`](docs/command-handlers.md) — **Command handler internals** (companion to command-pipeline.md). Read before editing `CommandDispatcher.cs` or any `*CommandHandler.cs`. Covers the two switch surfaces (`ApplyCommand` vs `TryApplyTowerCommand`) and why verbs sometimes live in both, the handler read/write contract (write `ControlTargets` or install a `PhaseList`; never move the aircraft), the per-domain effect cheat-sheet, dry-run clone safety, and the dimension-aware queue-clearing rule (never call `Queue.Clear()`).
- [`docs/aircraft-data-model.md`](docs/aircraft-data-model.md) — **Aircraft data model** (companion to snapshots-and-replay.md). Read before touching `AircraftState`, `ControlTargets`, any `Aircraft*.cs` satellite, `SimulationWorld`, or before adding a per-aircraft field. Covers the 13 satellites, the mutator map, the three DTO projections (snapshot vs live `AircraftDto` vs CRC) and the "field round-trips in replay but never reaches the client" trap, get-only restore-in-place, and the `SimulationWorld` lock/ownership contract.
- [`docs/training-hub-contract.md`](docs/training-hub-contract.md) — **`/hubs/training` JSON wire contract** (client↔server, the YAAT-native twin of crc-display-state.md). Read before adding a hub method or a DTO field that crosses the wire. Covers the `AircraftStateDto`↔`AircraftDto` name-based pairing, the hub-method/broadcast catalogs, the three source-gen JSON contexts + WASM failure mode, and the canonical add-an-`AircraftUpdated`-field checklist (the delta-fingerprint "appears on join but never updates" trap).
- [`docs/server-rooms-and-hub.md`](docs/server-rooms-and-hub.md) — **Server rooms, tick orchestration & the hub seam** (companion to training-hub-contract.md). Read before touching the hosted tick loop, `RoomEngine`, `TickProcessor`, `AircraftChangeTracker`, or `TrainingRoom*`. Covers the single 1 Hz loop driving every room, the double-cadence gotcha (physics per sim-second vs broadcast per wall-clock tick), the `RoomEngine` routing chain + CRC twins, room isolation, and the per-aircraft delta engine.
- [`docs/logging.md`](docs/logging.md) — **Logging: SimLog & AppLog**. Read before adding a logger to a class or debugging missing log lines. Covers the mandatory per-class pattern, the deferred-resolution + AsyncLocal-vs-static design, the two init paths + sinks, log file locations, the `SimLogBuilder` test-capture recipe, and why `SimLog.Initialize` must never be called from a test.
- [`docs/navigation-database.md`](docs/navigation-database.md) — **Navigation database & route expansion**. Read before touching `NavigationDatabase`, `RouteExpander`, `FrdResolver`, `CustomFixLoader`, or `ApproachGateDatabase`, or fixing a "flight plan turns back through every fix" / "SID won't resolve" / "airport code not recognized" / "custom fix missing" bug. Covers the singleton lifecycle + the xUnit race, FAA↔ICAO normalization (repeated across ~6 lookups), procedure-version resolution, the route-expansion token loop, **the RV-SID emit-all-vs-emit-nothing footgun + caller table**, FRD/custom fixes, and approach-gate distances.
- [`docs/flight-physics.md`](docs/flight-physics.md) — **Flight physics: kinematics, airspeed frames & constants** (the integration-math companion to tick-loop.md, which owns step order). Read before touching `FlightPhysics`, `ControlTargets`, `AircraftPerformance`, `CategoryPerformance`, `WindInterpolator`, or any kinematics field. Covers the `ControlTargets` read/write contract + snap-then-null lifecycle, the IAS/TAS/GS/Mach airspeed-frame model (IAS authoritative, GS derived), per-step integration math, the 7-layer speed cascade, and the full validated per-category constant table (incl. the 4th Helicopter category).
- [`docs/test-harness.md`](docs/test-harness.md) — **Test harness, fixtures & the singleton-race protocol**. Read before writing any Yaat.Sim test or when a test "passes alone but flakes in the suite." Covers `TestVnasData.EnsureInitialized` + the singletons, offline fallbacks + env vars, the two `ModuleInitializer`s (`YAAT_APPDATA_DIR`, `GroundNavigator.ThrowOnOrbit`), the static-singleton-race fix (`EnsureInitialized` in the constructor), the `xunit.runner.json` Content-copy gotcha, `SimLog` capture, the pathfinder oracle/budget infra, and the 30s-timeout discipline.
- [`docs/approach-and-pattern-geometry.md`](docs/approach-and-pattern-geometry.md) — **Airborne approach & pattern geometry** (airborne sibling of landing-and-runway-exit.md). Read before touching `Phases/Approach/` or `Phases/Pattern/`, `PatternGeometry`/`PatternBuilder`, `AirborneFollowHelper`, `HoldingEntryCalculator`, or `ApproachEvaluator`. Covers the cross-track sign convention, the leg state machine (legs complete on along-track/cross-track, **not** waypoint arrival), intercept (the 3 mag-var heading-diff checks), holding entry (110°/250° sectors), procedure turns, glideslope geometry, the visual-follow runaway watchdog, and approach scoring.
- [`docs/conflict-and-visual-detection.md`](docs/conflict-and-visual-detection.md) — **Conflict, alert & visual-acquisition detection**. Read before touching `ConflictAlertDetector`, `GroundConflictDetector`, `AtpaProcessor`, `VisualDetection`/`VisualAcquisition`, or `WakeTurbulenceData`. Four unrelated mechanisms sharing only a threshold-geometry theme; consolidates the tuning-constant tables and where each runs in the tick. Footguns: parallel-treated-as-not-diverging, purely-geometric corridor suppression, `IsExcludedByTcp` always-false, per-detector altitude units.
- [`docs/scenario-loading-and-generation.md`](docs/scenario-loading-and-generation.md) — **Scenario loading & aircraft generation**. Read before touching `ScenarioLoader`, `AircraftInitializer`/`AircraftGenerator`/`SpawnParser`, `GroundSpawnSnap`, or diagnosing a "spawned in the wrong state/route/type/beacon" bug. Covers the two spawn pipelines + four runtime queues, the immediate/delayed/deferred split, the RouteExpander mismatch footgun, airline-fleet→type coupling, presets running through the live dispatcher, and the rewind-reload twin path.
- [`docs/solo-training-evaluation.md`](docs/solo-training-evaluation.md) — **Solo training evaluation & scoring**. Read before touching `SoloTrainingEvaluator`, `AircraftCompletion`, or anything emitting a scoring finding. The rule-to-code map (27 7110.65/AIM refs), the per-tick `Evaluate` / on-demand `BuildReport` entry points + cross-repo wiring, the same-runway/wake separation tracker, the 5 scoring buckets, and the FNV debrief-cache invariant. Footgun: proof-based clearing (findings clear on the controller's command, not on recovered spacing).
- [`docs/track-sharing-and-consolidation.md`](docs/track-sharing-and-consolidation.md) — **STARS/ERAM track sharing, pointouts & consolidation** (the scope-sharing layer above TRACK/DROP ownership). Read before touching `ConsolidationEngine`/`ConsolidationState`, `AircraftStarsState.SharedState`, `AircraftEramState`, the server consolidation handlers, or the `StarsConsolidation` topic. Covers the TCP hierarchy + walk-up/walk-down algorithm + the manual-override second pass, track transfer + handoff redirection, and per-track shared display state.
- [`docs/client-mainviewmodel.md`](docs/client-mainviewmodel.md) — **Client MainViewModel & app orchestration**. Read before touching any `MainViewModel` partial, `MainWindow.axaml.cs`, or wiring a SignalR-driven client feature. Covers the partial-class ownership map, the threading rule (every `ServerConnection` handler must `Dispatcher.UIThread.Post`), the three scenario-activation paths through `ApplyScenarioBootstrap`, the `_isApplyingSessionSettings` echo-suppression guard, tab/pop-out index arithmetic, and the shutdown re-entrancy protocol.
- [`docs/radar-rendering.md`](docs/radar-rendering.md) — **Radar display & rendering (SkiaSharp)**. Read before touching `Views/Radar/` or the shared `Views/Map/` base. Covers the two-thread snapshot split (UI-thread `CreateRenderSnapshot` → render-thread `RenderFromSnapshot`; the no-StyledProperty rule), the viewport projection + Range/Zoom feedback-loop guards, the one-frame render order, the geometry-computed-twice invariant (draw vs hit-test), the pointer priority ladder, and the smart-default context-menu convention.
- [`docs/command-input-ux.md`](docs/command-input-ux.md) — **Command input UX: autocomplete & signature help** (client-side, pre-send). Read before touching `CommandInputController`, `ArgumentSuggester`/`AddCommandSuggester`/`FixSuggester`, `SignatureHelpState`, `CommandInputParseResult`, or `CallsignArgumentResolver`. Covers the parse-once contract, the caret/offset model, the metadata-driven suggesters (most command changes need ZERO suggester code), the ADD/CVA custom paths, FixSuggester's no-global-pickers enforcement, and the send-path callsign rewrite. (`ShownRouteBuilder.cs` lives in `Services/` but is radar-overlay — documented in radar-rendering.md.)
- [`docs/weather-and-wind.md`](docs/weather-and-wind.md) — **Weather parsing, wind interpolation & magnetic declination**. Read before touching `WeatherProfile`/`WeatherTimeline`, `MetarParser`, `WindsAloftParser`, `WindInterpolator`, `MetarInterpolator`, `MagneticDeclination`, or `LiveWeatherService`. Covers the v1/v2 data model, the parsers (DDSS overflow decode), the THREE interpolation axes (altitude / spatial-IDW / time) + the FROM-vs-TOWARD / true-vs-magnetic convention cheat-sheet, and the declination box-cache. The ISA compressible-flow airspeed math is owned by flight-physics.md.
- [`docs/airspace-database.md`](docs/airspace-database.md) — **Airspace (Class B/C) & boundary-crossing detection**. Read before touching `src/Yaat.Sim/Data/Airspace/`, `AirspaceBoundaryHoldPhase`, `PilotProactive.TickAirspaceBoundaryRespect`, or the Class B/C separation-minima path in `SoloTrainingEvaluator`. Covers the containment engine (only Class B/C parsed; D dropped), the 3-tier fixture discovery + Brotli, projected-entry detection, BOTH consumers (VFR boundary hold + the separation-minima path with the synthetic 20 NM Class-C outer-area ring), and the gate-switch-duplicated-in-3-places footgun.
