...
`RTIS` **fails soft**: if the pilot can't acquire the traffic on the first check, the command still succeeds, the pilot reports e.g. *"Negative contact, LEAD, looking"* (or *"clouds between us, looking"* / *"in the turn, looking"* for the cloud-obstructed and bank-occlusion cases), and the pilot keeps re-checking acquisition each tick. When the target becomes acquirable — traffic enters the forward hemisphere, pilot rolls out of the turn, distance closes — the pilot reports *"traffic in sight"* and the look-state clears. A second `RTIS` for a different callsign replaces the prior looking state; target leaving the simulation or an approach-procedure reset silently clear it. Hard failures are still hard: bare `RTIS` with no callsign, or a callsign not on frequency, returns an immediate error. The forced variant `RTISF` sets the flag directly with no acquisition check and no looking state — use it as an instructor override.

`RFIS` retains one-shot behavior: on failure it returns a specific reason (e.g. *"Unable, KOAK behind us"*, *"Unable, KOAK below OVC015"*, *"Unable, KOAK lost visual in the turn"*). `RFISF` is the forced override.
