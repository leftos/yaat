...
| `RNS` / `NS` | Resume normal speed: clears speed/floor/ceiling, preserves SID/STAR via mode. An arrival on a long final that was slowed well below its approach profile speeds back up to it (outside roughly 15 NM; closer in it keeps slowing to its approach speed) |
