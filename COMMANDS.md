...
| `TAXI M4 M1 A A1 RWY 1R` (from a gate) | When the aircraft starts at a gate and the **first** taxiway is a neighbouring ramp lane the ground map does not connect to the gate (SFO B20S → M4, which only joins M1), that lane is omitted, the rest of the route is taxied, and the response warns `unable via M4 — no ramp connection from the gate; taxiing via M3`. Only a lane within ~450 ft of the gate, with no runway between, qualifies (where a tug would push the aircraft) — a taxiway across the field or across a runway is still rejected with `Cannot taxi via …`. Scripted (preset) taxis echo the same warning in the terminal. |
