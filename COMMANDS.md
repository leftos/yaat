...
| `HOLD` / `HP` | Hold position — stop wherever on the ground. While the aircraft is taxiing into position after `LUAW`, it stops where it is instead of continuing onto the centerline; resume the line-up with `LUAW` or `CTO` (not `RES`). Rejected once the takeoff roll has begun — use `CTOC` to cancel the takeoff clearance. A held aircraft can turn around on the spot: re-issuing a `TAXI` in the opposite direction resolves the direct route (a *rolling* aircraft re-cleared against its direction of travel instead plans from its present momentum, which can mean a longer way around — `HOLD` first when you want the about-face). |
