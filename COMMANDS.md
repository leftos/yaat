...
`PUSHM <target> <target> …` chains two or more targets into one tug move (`PUSHM $6A $6B`, `PUSHM @D5 #503 $5A FACE E`): the same `$spot` / `@gate` / `#node` tokens `WARPG` takes, each intermediate spot reached nose-out, an optional final `FACE`/`TAIL` for the last target (not a gate). A single target is plain `PUSH`. Every leg is checked and refused like a `PUSH` (`Unable, leg 2 to spot 6B reaches a runway holding position`), and a leg may end on a movement-area node — a tug move onto a taxiway is real and coordinated — but may not run along one. Each leg is bounded to 2,000 ft from the previous point, a guard against a mis-click on the ground view rather than an aviation rule. A `PUSHM` given to an aircraft already under tow replaces the rest of its move; a first leg that reverses the current motion stops and pauses first. The ground view's **Push route…** draw mode builds the command by clicking the points and previews the planned path — see the User Guide.
