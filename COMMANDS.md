...
**`APT` at the front of a `,` block applies first.** `APT OAK, ELB 28L 4, CLAND` moves the flight plan before the pattern entry resolves its runway, so the entry finds 28L at OAK. The destination change stays applied even if the manoeuvre is then refused, and the response says so (`Destination changed to KOAK; but Runway 35L not found at OAK`). Every other command in a `,` block keeps its place.
...
| Forced tug move (past YAAT's push checks) | `PUSHF $7A`, `PUSHMF $7A $7B` | — | `PUSHF A FACE N`, `PUSHMF @F8 $7A $7B FACE E` |
...
| Hold short | `HS B` | — | `HS C@J` — hold short of C at its crossing with J (target@taxiway; runway targets too: `HS 28R@J`); `HS $17` — hold short of taxi spot 17 |
...
| On the go | `OTG` | — | Condition prefix only — `OTG MLT 28L` fires the command once the aircraft is climbing out after its next touch-and-go, stop-and-go, low approach or go-around; dropped with "unable — landed full stop" if it lands full-stop instead. |
...
| Command | Primary | Aliases | Notes |
|---------|---------|---------|-------|
| Change destination | `APT KSFO` | `DEST` | Changes aircraft destination airport. Accepts FAA (`OAK`) or ICAO (`KOAK`); resolves to canonical ICAO. Rejects unknown airports. A change to a different airport clears the arrival state for the old one — STAR, approach, and any pattern or approach being flown to a runway there (warned as `pattern to RWY 30R cancelled by APT OAK`), so a following `ERD`/`ELB`/`EF` resolves its runway at the new destination. `APT` to the airport the aircraft is already approaching or flying a pattern at only corrects the plan and keeps everything; a departure's climb is never touched. |
| Create IFR flight plan | `FP B738 220 KBOS DCT KJFK` | — | Altitude in hundreds (220 = FL220); `OTP/055` files VFR-on-top at 5,500 ft (an IFR plan with the OTP notation). Type accepts equipment suffix (`B738/L`) — split into AircraftType and FaaEquipmentSuffix. Departure/destination accept FAA (`OAK`) or ICAO (`KOAK`) — resolved to canonical ICAO when recognized; an unrecognized identifier (e.g. a non-US airport like `WSSS`) is kept as typed rather than rejected. Single-token route is treated as destination only (`FP C172 050 MOD`). Create-or-amend: if the aircraft already has a flight plan, the typed fields amend it in place. Auto-assigns a discrete beacon code if none was typed in the command; pilot keeps squawking their previous code until told `SQ`. |
| Create VFR flight plan | `VP C172 5500 KOAK DCT KJFK` | — | Altitude absolute (5500 = 5,500 ft). Type accepts equipment suffix (`C172/G`) — split into AircraftType and FaaEquipmentSuffix. Departure/destination accept FAA (`MOD`) or ICAO (`KMOD`) — resolved to canonical ICAO when recognized; an unrecognized identifier (e.g. a non-US airport like `WSSS`) is kept as typed rather than rejected. Single-token route is treated as destination only (`VP C172 5500 MOD`). Create-or-amend (stars.md Table 23), but only over a VFR plan: if the aircraft already has a VFR flight plan the typed fields amend it in place, while an existing IFR plan (VFR-on-top included) is refused with DUP NEW ID rather than being converted to VFR. Auto-assigns a discrete beacon code; pilot keeps squawking their previous code until told `SQ`. |
| Flight Data (abbreviated FP) | `DA C172 065 4304` | — | CRC F6 key. Optional fields in any order: type (with optional equipment suffix `C172/G`), altitude (hundreds), beacon code (octal — digits 0-7 only), scratchpad (`` `VFF ``), flight rules (`.V` VFR / `.P` VFR-on-top / `.E` IFR). Creates VFR FP by default. Create-only: errors with DUP NEW ID if aircraft already has a flight plan. Auto-assigns a discrete beacon code if none was typed; pilot keeps squawking their previous code (e.g. 1200) until told `SQ`. |
...
| Force altitude | `CMN 240` / `DMN 240` | — | — |
...
| Command | Effect |
|---------|--------|
| `PUSH` | Push **straight back** from parking (a tug at 5 kt) — the nose keeps the stand heading. For a push that ends aligned for the taxi out, use one of the facing forms below (`PUSH Y A1`, `PUSH Y FACE S`, `PUSH $spot`). |
| `PUSH FACE E` / `PUSH >E` | Push back turning the nose onto east: a straight push-off first, then a rolling turn on the type's turn radius (the tight radius when the nose has to come round more than 135°). Every turn happens while moving — a tug never pivots a stopped aircraft. |
...
| `PUSH $7A` | Push onto spot 7A — the tug works the aircraft onto the spot's line from whichever side needs the fewest reversals (a push past the mark then a pull back, or a turn then a pull), always ending with a slow pull forward onto the mark, lined up **nose-out** toward the parent taxiway with the nosewheel on the mark. Off a gate it prefers to push straight back along the lead-in line, pivot onto the spot's lane and pull forward onto the mark. The aircraft is kept clear of the taxiways behind the alley (see below) |
| `PUSH $7A TAIL W` | Push onto spot 7A with tail pointing west (= face east), overriding the default nose-out facing |
| `PUSHM $6A $6B` | A tug move through two or more ramp points in order — `$spot`, `@gate`/`@helipad` or `#node` — planned as one chain of pushes and pulls with a pause at every reversal. The tow passes each point before the last within half a wingspan without stopping there, and the last point is where it stops. Ends as `PUSH` to the last target would (a gate parks, a spot or node holds). An optional trailing `FACE`/`TAIL` sets the final facing, except onto a gate. Issued to an aircraft already under tow it replaces the rest of the move. |
| `PUSHF $7A` / `PUSHMF $7A $7B` | A **forced** `PUSH` / `PUSHM`, for when YAAT refuses or stalls a push it shouldn't. It takes every `PUSH` / `PUSHM` form and reads back the same. It flies the planner's best shape without the taxiway checks, the parked-aircraft checks or the object-free-area clearance, and never stops for parked aircraft. It still yields to moving traffic, and it is still refused onto or across a runway or a runway holding position, past 2,000 ft, or with outlines already overlapping |
...
| `TAXI A F1 B 1L` (SFO) | When the last taxiway carries no holding-position bar for the runway but a short numbered stub off it does (B runs past 01L's south end; the 1L bars are on M1 and A2, within 600 ft), the aircraft taxis the stub and the response notes it — `[via M1 — B reaches 1L through M1]` — with the stub in the route summary (`… B M1, RWY 1L`); the spoken readback stays the clearance as issued. A runway the taxiway does not serve is still refused (`Taxiway B does not reach runway 1R — specify a connecting taxiway.`); an unnamed letter taxiway is never taken as the stub. |
...
| `TAXI C D J HS C@J` | Taxi via C, D, J and hold short of C **where J meets it** — the located hold-short form `target@taxiway`. Disambiguates which crossing binds when a looping route meets the target more than once: bare `HS C` here would bind the first C crossing (right where the aircraft starts), while `C@J` binds the C crossing on J and routes the aircraft down J to it. The location taxiway also steers the route (folded into the path like a taxiway HS target). Works with runway targets too: `HS 28R@J` holds at the 28R holding-position bar on J. |
| `TAXI T421 C Z B M1 1L HS $17` | Taxi via T421, C, Z, B, M1 to runway 1L, holding short of **taxi spot 17** on the way (the `$spot` form inside `HS`). The aircraft stops just short of the spot node — here on Z at the C junction, SFO's west-end coordination point — and continues to 1L on `RES` / `CROSS`. Everything after `HS` is a hold-short target until the next keyword, so a spot or parking *destination* goes before `HS`: `TAXI K $8 HS $8` taxis to spot 8 and holds short of it; `TAXI A B HS 28R @A12` is rejected. |
| `TAXI A $7 @E2` | Taxi via A **through spot 7** to gate E2 — a `$spot` named before an `@gate` (or before another `$spot`) is a via the route must pass through, not a second destination. Only a trailing `$spot` is a destination (`TAXI T5 $5A`). A gate the route cannot reach from the via is refused by name: `Cannot reach parking 'E2' via 7B`; a spot the layout does not have refuses `Unknown spot 7X`. |
| `TAXI S T U @B12 NODEL` | Taxi via S, T, U to parking B12 (exempt from auto-delete) |
| `TAXI B K A T5A @D1` (SFO) | A ramp taxilane named last is driven, never swapped for its parallel sibling: the aircraft taxis down T5A, then cuts across the apron to D1 and rolls in on the stand's heading (see [Taxi to a stand or spot](#taxi-to-a-stand-or-spot) below). The readback is the clearance as issued — `Taxi via B K A T5A @D1`. |
| `TAXI G @SIG1` (OAK, from the 28R bar on G) | The gate's own short lead-in is implied: SIG1 hangs off D, which the clearance does not name, but the aircraft drives 635 ft of D and then only apron, so D is driven silently and the readback stays `Taxi via G @SIG1`. A lead-in longer than 1,000 ft needs its taxiway named. |
| `TAXI D @NEW1` (OAK, from the E exit) | A clearance whose taxiways do not join up is taxied as far as it goes: E does not meet D (C joins them), so the aircraft holds short of C on E and the response notes `[Holding short of C: route to @NEW1 needs C, not in clearance]`. Continue with a `TAXI` that includes C (`TAXI C D @NEW1`); `RES` and `CROSS` are refused there. See [Incomplete clearances](#incomplete-clearances) below. |
| `RWY 28R TAXI F C HS 33` (OAK, from OLD1) | F and C never reach 28R; B does, off the east end of C. The aircraft taxis F and C (holding short of 33 on the way) and holds short of B: `[Holding short of B: route to RWY 28R needs B, not in clearance]`. |
| `TAXI M4 M1 A A1 RWY 1R` (from a gate or mid-lane) | When the **first** taxiway is a parallel ramp taxilane the ground map does not connect to the aircraft's own lane (SFO Terminal 1: M3 / M4 / M5 share one ramp with no painted connectors, and M4 only joins M1), the pilot cuts straight across the apron onto that lane and taxis the clearance as issued — from a gate whose alley opens onto M3, or while already rolling on M3. Only a lane with the same letter prefix (`M3` → `M4`, `M5`) within ~450 ft, with no runway and no other taxiway between, qualifies; a taxiway across the field or across a runway is still rejected with `Cannot taxi via …`. A gate-adjacent lane that is *not* such a sibling lane is omitted with the warning `unable via K — no ramp connection from the gate; taxiing via M3`. |
...
| `TAXI T6A A F CROSS 1L 1R RWY 28L` | Pre-clear **several** crossings inside one taxi clearance. `CROSS` keeps consuming runways until `HS` or `RWY`, so the aircraft taxis the whole route to 28L without stopping at 1L or 1R. Unlike the anchor form above, the destination runway sets the direction and `CROSS` is purely pre-clearance. |
...
| `HS B` | Hold short at the next intersection with taxiway B. Issued to a rolling aircraft, `HS` brakes for the line on the segment it is on; if the aircraft is already inside its braking distance the response is `Unable to hold short of B — stopping` (pilot: "unable to hold short of bravo, stopping") and it stops as short of the intersection as it can. |
| `HS C@J` | Hold short of C **at its crossing with taxiway J** (the located form, `target@taxiway`). Binds the target's crossing on the named location taxiway instead of the first crossing in route order — for routes that meet the target more than once. The target may be a taxiway (`C@J`) or a runway (`28R@J`); the location is always a taxiway. Accepted everywhere a hold-short target is: standalone `HS`, `TAXI … HS …`, `RES … HS …`, `CROSS … HS …`. |
| `HS $17` | Hold short of **taxi spot 17** — the `$spot` sigil names a spot node on the route (spots are the ramp/movement-area boundary points shown on the ground map). The aircraft stops with its nose at the spot mark (like a runway holding position — a spot is a painted point, so there is no crossing pavement to clear); released by `RES`, bare `CROSS`, or `CROSS $17`. A spot has no located (`@`) form. Accepted everywhere a hold-short target is; a route that never reaches the spot warns `HS $17 not applied`. |
...
| `FOLLOWG SWA123` | Follow another aircraft on the ground. Works from parking (the aircraft starts up and falls in behind — no `TAXI` needed first), mid-taxi (drops the current route and chases the leader), or from any holding state (in position, after pushback, after a runway exit, holding short at a taxiway or spot bar). Holding short of a runway it is armed, not refused: the aircraft stays held ("Follow SWA123, hold short of 1R") until `CROSS`, then crosses and follows — `FOLLOWG SWA123; CROSS 1R` works as one transmission, and `CROSS 1L 1R` takes it across both. `RES` does not release an armed hold (issue `CROSS`). At the departure-runway bar before the route is finished it is refused (issue `LUAW` or `CTO`). The leader must exist and be on the ground. Combine with a give-way condition (`BEHIND SWA123 FOLLOWG SWA123`) to wait until the leader passes before pulling out. |
| `GIVEWAY SWA123` | Give way to (yield to) SWA123 on the current taxi route until it passes; when SWA123 is merging onto that route ahead, the aircraft falls in behind it once SWA123 is on the taxiway into the merge and nearer it. As a condition prefix (`GIVEWAY SWA123 TAXI …`) it instead waits for SWA123 before running the command (see [Conditional Blocks](#conditional-blocks)). Can be appended to a taxi clearance: `TAXI A A1 1R GIVEWAY KLM605`. |
...
Every `PUSH` form and `PUSHM` is a **tug move**: a chain of pushes (tail-first) and pulls (nose-first) that the tug steers on the type's turn radius, turning only while rolling — never pivoting a stopped aircraft, never sliding sideways — and reversing only at explicit stop-and-reverse points, where it pauses about 5 s. Off a stand the first move is always a short straight push. The tug runs at 5 kt, through turns as well as straights (the main gear moves slower than the tug through a turn — about 3.5 kt for a 737 on its normal turn radius); the last 30 ft of the creep onto a spot mark is at 3 kt. The move is planned before it starts, and the plan is checked along the path the aircraft will actually fly: a move whose footprint would reach a **runway** or cross a **runway holding position**, or whose fuselage would cross a **taxiway it was not sent to**, is refused naming the leg and the pavement (`Unable, the move to spot 6A would put the aircraft on taxiway A`). The taxiway straight behind the stand is exempt — the push clearance covers pushing onto it and pulling back off. A tug move is also refused when the aircraft already stands with its outline against a parked neighbour's (`Unable, SWA1 is up against SWA2 — their outlines overlap; reposition one of them before towing`).

A bare `PUSH` is a **straight-back push**: the tug reverses the aircraft onto the pavement behind it and the nose keeps the stand heading. That is deliberate — the alignment is part of the instruction, and it is the RPO's to give. A bare `PUSH <taxiway>` pushes straight back onto a taxiway that lies **across** the push, and S-curves onto the centreline of one that runs **alongside** it (the split is 45° between the push line and the taxiway). Use `PUSH <taxiway> <toward-taxiway>` (`PUSH Y A1`: onto Y, lined up along whichever direction of Y leads toward A1), `PUSH <taxiway> FACE <dir>` / `TAIL <dir>`, or `PUSH $spot` (nose-out by default) when the aircraft should end up pointed for its taxi. Pushback orientation accepts the eight compass points: `N`, `NE`, `E`, `SE`, `S`, `SW`, `W`, `NW`. Use `FACE C` (or shorthand `>C`) to specify the nose direction, or `TAIL C` (`<C`) to specify the tail direction. When pushed onto a taxiway, the cardinal acts as a hint — the aircraft lines up along whichever of the taxiway's two directions is closest. For a spot the cardinal is the absolute facing; a gate takes none.

A `PUSH @gate` reaches the stand's line **directly**, pushed or pulled onto it from whichever side needs the fewest reversals (then the shortest path), like the tug working along the ramp lead line — it does not taxi there along the taxiway graph and never routes through a movement-area taxiway to reach the gate. It parks on the stand's own heading; `PUSH @gate FACE …` is refused.

A `PUSH $spot` lines the aircraft up straight on the marking the way a tug positions it onto a stand: it pushes *past* the spot to a staging point behind it, pauses, then creeps **forward** onto the mark so the **nosewheel** sits on it (the fuselage doesn't jut a half-length past the mark toward the adjacent taxiway). Off a gate the preferred shape is **straight, then onto the lane**: the tug pushes straight back along the gate's lead-in line, pivots onto the spot's lane, then pulls forward onto the mark. With no facing given it ends **nose-out**, facing the parent taxiway ready to taxi — add `FACE <dir>` / `TAIL <dir>` to override. Like `@gate` it never routes onto a movement-area taxiway to get there.
...
| `PUSHM $6A $6B` | `Push to spot 6B via spot 6A`; more points are listed in order (`via spot 6A and spot 6`, `via spot 6A, spot 6 and spot 5A`); a final facing appends `, face east`. `/PUSH` and `/PULL` are not read back on a `PUSHM` |
| `PUSH $7A/PULL` | `Pull forward to spot 7A` |
...
**RPO notes on a push readback.** A `PUSH`, `PUSHF`, a mid-push `FACE`/`TAIL` amendment, a `PUSHM` or a `PUSHMF` can carry notes in parentheses at the end of its readback. They appear on the RPO terminal only; the pilot never says them.

| Note | Meaning |
|------|---------|
| `(<part> will foul taxiway <X>, coordinate with ground)` | No path to the spot, gate or node keeps the aircraft outside X's object-free area; the push flies the one that reaches in least, and `<part>` (nose, tail, left wing, right wing) is what reaches in deepest. |
| `(forced: tow enters taxiway <X>, coordinate with ground)` / `(forced: tow runs past taxiway <X>, …)` | A `PUSHF` onto pavement a plain push refuses. |
| `(forced: <part> fouls taxiway <X>, coordinate with ground)` | A `PUSHF` whose path reaches into X's object-free area. |
| `(forced: passes <n> ft from <callsign>)` / `(forced: overlaps <callsign>)` | A `PUSHF` closer to a parked aircraft than a plain push allows; the figure is between modelled outlines, not skin to skin. |
| `(forced: nose swings <n>°, lane needs <m>°)` | A `PUSHF` onto a spot's lane swinging wider than a plain push allows. |
| `(forced: will not stop for parked aircraft)` | A `PUSHF` with parked aircraft nearby: the conflict detector won't stop it for them. |
| `(taxiway <X> is <n> ft away; long push)` | `PUSH <X> …` tows more than 500 ft across the ramp before the aircraft is lined up on X — accepted, but check the taxiway name. |
| `(<facing> is <n> ft away; facing only)` | The facing taxiway's junction with X is more than 1,500 ft from where the push ends, so it only chose which way the nose points along X; the push ends at the exit. |

A spot is not a stand, so a completed `PUSH $spot` leaves the aircraft **holding after pushback** rather than at parking: the Aircraft List reads "holding position", the aircraft no longer names the gate it was pushed off, and a further `PUSH` or `PUSHM` is accepted from there without a `TAXI` in between. `PUSH @<gate>` parks it, as a push onto a stand should.

`PUSHM <target> <target> …` chains two or more targets into one tug move (`PUSHM $6A $6B`, `PUSHM @D5 #503 $5A FACE E`): the same `$spot` / `@gate` / `#node` tokens `WARPG` takes, and an optional final `FACE`/`TAIL` for the last target (not a gate). A single target is plain `PUSH`. Every point before the last is a waypoint for the tug. The aircraft passes within half its wingspan of it and keeps moving, so `PUSHM $7A $7B` from SFO F8 flies the same tow as `PUSH $7B`, which already passes 7A. The tow reverses at a waypoint only when the geometry needs it, and it stops only at the last point. Every point and the tow through them are checked and refused like a `PUSH` (`Unable, the move to spot 6B reaches a runway holding position`). Only the last point takes a facing (`Unable, the marked point is passed through; only the last point takes a facing`). A point may lie on a movement-area node — a tug move onto a taxiway is real and coordinated — but the tow may not run along one. Each point is bounded to 2,000 ft from the previous one, a guard against a mis-click on the ground view rather than an aviation rule. A `PUSHM` given to an aircraft already under tow replaces the rest of its move; a first leg that reverses the current motion stops and pauses first. The ground view's **Push route…** draw mode builds the command by clicking the points and previews the planned path — see the User Guide.

**Forcing a push.** `PUSHF` and `PUSHMF` are for the RPO whose aircraft YAAT won't push. When a plain `PUSH` or `PUSHM` is refused for a reason forcing gets past, the refusal ends with the command to use (`Unable, the move to spot 7A would swing into UAL12. To force it: PUSHF $7A`). A forced tow skips the taxiway checks (entering or running past a taxiway it wasn't sent to), the parked-aircraft check, the object-free-area clearance and the swing limit, and the ground conflict detector does not stop it for parked aircraft for the whole tow. Among the shapes it may fly, it prefers the one with the most room from parked aircraft. When no shape template fits, it tows onto the spot's line from a point behind it. It still yields to taxiing traffic, and it keeps the runway and runway-holding-position refusals (a push carries no crossing clearance), the 2,000 ft guard and the overlap refusal. The pilot's readback is the same as for `PUSH` / `PUSHM`; the RPO terminal adds a `(forced: …)` note for each check the tow overrode.

While the stand push-off of a `PUSH` is still running — the short straight push before any turn — a **heading-only** `PUSH FACE C` / `PUSH TAIL C` / `PUSH >C` / `PUSH <C` amends the facing: the same push is re-planned on the new facing from the stand it started at, and the push-off carries on without a restart. Once a turn has begun the amendment is rejected with `Unable, pushback turn in progress`; a push to a gate is never amended (`Unable, a pushback to a stand keeps the stand's heading`); a `PUSHM` carries no amendment. Any other `PUSH` during a tow is rejected (`only face/tail amendment accepted during pushback`) — redirect the tug with `PUSHM`, or `TAXI` out of it.
...
`HS` can be issued to a taxiing aircraft to add a hold-short point at the first upcoming intersection with the given taxiway or runway along the remaining route, or at the named taxi spot (`HS $17`) when the route passes through it.

Any hold-short target — in standalone `HS`, `TAXI … HS …`, `RES … HS …`, or `CROSS … HS …` — may be **located** with `@`: `C@J` means "hold short of C at its crossing with J" (mirroring the `28R@E` intersection-departure notation: the thing before the `@`, the location after). Use it when the route meets the target more than once and the bare form would bind the wrong crossing — e.g. after `TAXI C D J`, a bare `HS C` binds the first C crossing at the start of the route, while `HS C@J` binds the far one where J re-crosses C and taxis the aircraft down J to it. The location must be a taxiway; the target may be a taxiway or a runway (`28R@J` picks the 28R holding-position bar on J). Inside a `TAXI`, the location taxiway also steers the route the same way a taxiway HS target does. When a bare taxiway target that is also a cleared path element binds at the very start of the route (the aircraft holds immediately and never taxis), the TAXI echo warns and points at the `@` form.

A hold-short target may also be a **taxi spot**: `HS $17` (the same `$` sigil as a `$spot` taxi destination). Inside an `HS` clause every following token is a target until the next keyword (`RWY`, `CROSS`, `NODEL`), so `$17` there is a spot hold-short and `@A12` is rejected — put a spot or parking *destination* before `HS` (`TAXI K $8 HS $8`). Spots have no located form.

A `TAXI $spot` arrives **along the lane the spot sits on** and stops facing along it (nose at the mark), never across it — even when that means the long way round (SFO `TAXI $5A` from gate D2 taxis out to Alpha and back up T5A instead of cutting across the five alley). An aircraft stopped across its lane juts into the neighbouring lead-in lane and blocks it.

#### Taxi to a stand or spot

A ramp taxilane the clearance names is driven, even when a parallel lane of the same ramp would reach the stand too (SFO `TAXI B K A T5A @D1` drives T5A, not T5B). From the named lane the aircraft leaves the painted line and crosses the apron to the stand: first to a point one fuselage length out on the stand's centreline, then in on the stand's heading, so it parks lined up with the stand. The crossing is at most about 450 ft in all, never crosses a runway, and stays on that ramp's own pavement; when no point on the named lane qualifies, the aircraft crosses onto the stand's own lane and follows its lead-in instead (SFO `TAXI B4 T8 @F1`: F1 is 521 ft from T8, so the aircraft drives T8, then crosses onto T9). A route that reaches a stand only the long way round is shortened the same way, rolling in on the stand heading. A spot is always entered along its own lane (above).

On the way from the last cleared taxiway to a gate or spot the route uses only the cleared taxiways, ramp taxilanes and the apron — never a movement-area taxiway the clearance did not name. The one exception is the destination's own **lead-in**: when a gate or spot hangs off an uncleared taxiway, the route drives only apron after it, and the run on it is no more than 1,000 ft, that taxiway is implied and driven silently (OAK `TAXI G @SIG1` drives 635 ft of D). The 1,000 ft limit is a leniency of the simulator; no FAA document gives a figure.

The TAXI response reads back the **clearance as issued**, and so does the pilot. Ramp taxilanes the driven route adds are not named (`TAXI B4 T8 @F1` reads back `Taxi via B B4 T8 @F1`, although the aircraft also drives T9), and neither is the numbered runway-entry connector a runway clearance implies (OAK `TAXI U W RWY 30` ends on W1, and neither the echo nor the pilot says W1). The taxiway the aircraft is on is named when the route drives along it. Any other movement-area taxiway the route drives that the clearance did not name is reported: `[taxiing via T — not in the route issued]`.

#### Incomplete clearances

A `TAXI` whose taxiways do not reach its destination is not refused when one more taxiway would complete it. The aircraft taxis the route as issued as far as it goes and holds short of the missing taxiway; the response says what the route needs: `[Holding short of B: route to RWY 28R needs B, not in clearance]` (the destination reads as written — `RWY 28R`, `@OLD1`, `$9` — or, with no destination, as the clearance's last taxiway). This happens at either end of the route:

- **At the end** — the named taxiways stop short of the gate, spot or runway (OAK `RWY 28R TAXI F C HS 33` from OLD1 holds short of B; `TAXI C @OLD1` from C holds short of F, whose run to OLD1 is too long for an implied lead-in). For a runway the missing taxiway chosen is the one whose entry is nearest the threshold.
- **At the start** — the taxiway the aircraft is on does not reach the first cleared one (N9225L `TAXI D @NEW1` from the E exit: C joins E to D). The aircraft holds short of the missing link on the taxiway it occupies.

A runway-entry connector that is not a numbered variant of the last cleared taxiway is never implied, so it also ends in this hold. A hold that would fall on runway pavement moves back to the runway's near-side holding position, and the aircraft holds short of the runway there. When no hold can be placed the `TAXI` is refused, naming the missing taxiway, with the resolver's own reason alongside when it has one: `Unable, route to G from E needs C, not in clearance`.

At the hold the pilot reports `holding short of C at E, request further taxi`. `RES` and `CROSS` are refused there (`holding short of C where the route issued ends — issue a TAXI that includes C`): the route goes no further, so the way on is a new `TAXI` that includes the missing taxiway (`TAXI C D @NEW1`). An `HS C` there keeps the aircraft where it is.
...
`CROSS` inside a `TAXI` accepts more than one runway — it keeps consuming runway tokens until `HS` or `RWY` ends the list. `TAXI T6A A F CROSS 1L 1R RWY 28L` therefore taxis the whole route to 28L without stopping at either crossing. This is how a scenario preset clears crossings the file expects every aircraft to make — for instance a parallel pair that is closed for the current configuration — instead of leaving each departure holding short for a manual `CROSS`. Crossings cleared this way are owned by the clearance, not by the AutoCrossRunway session toggle, so turning that toggle off later does not revoke them.
...
| `CTOC` | Cancel takeoff clearance (7110.65 3-9-11). While the aircraft is lining up onto the runway it stops immediately where it is (does not continue onto the centerline); a fresh `CTO` resumes the line-up and departs. If already lined up and waiting, it holds. Mid-roll the pilot aborts below V1 (≈ Vr − 5 kts, indicated): the aircraft keeps accelerating for a ~2 s reaction, brakes at the max-effort rejected-takeoff rate on the centerline, reports "stopped on the runway, standing by", and holds in position awaiting an exit/taxi instruction or a fresh clearance (`TAXI`, or `LUAW`/`CTO` again). At/past V1 the pilot answers "unable, past V1" and continues — unless another aircraft blocks the runway ahead beyond what the departure can safely overfly, in which case the pilot aborts anyway. Helicopters have no V1: mid-liftoff CTOC sets back down (below ~50 ft) or stops in a hover, awaiting instructions. |
...
| `CLAND 28R` | Cleared to land on a named runway. For an aircraft that is **following** traffic but has no runway of its own (`RTIS`/`FOLLOW` issued, not yet sequenced onto final), the clearance is **armed** and applied automatically when the follower joins the traffic's runway final — it lands behind the traffic without a second `CLAND`. A bare `CLAND` while following inherits the lead's runway. If the named runway differs from the runway the follower actually joins, the clearance is not applied and the follower awaits an explicit `CLAND` on the actual runway. (Not for an enroute aircraft with no approach, no follow and no queued pattern entry — assign a pattern entry like `EF 28R` first.) |
| `CLAND` behind a queued pattern entry | The clearance is **pre-issued** when the pattern entry that would build the approach is still sitting in the command queue — `DCT VPCOL; ERD 28R` and then, while the aircraft is still en route to VPCOL, a separate `CLAND`. It becomes the standing clearance the moment the entry reaches its fix and builds the circuit, so the aircraft lands full-stop without a second `CLAND`. Applies to `TG` / `SG` / `LA` / `COPT` the same way, each building its own terminal (touch-and-go, stop-and-go, low approach, option). A bare verb adopts the runway the queued entry names; a runway that **contradicts** it is rejected outright ("… is queued to enter the pattern for runway 28R — re-issue the entry for runway 30, or CLC first"), and if the entry is later re-issued for a different runway the pre-issued clearance is voided with a warning. A clearance names a runway (7110.65 §3-10-5.a), so if neither the clearance nor the queued entry names one the clearance is refused. `CLC` cancels it. The Info column shows `CLAND 28R armed` (or `TG`/`SG`/`LA`/`COPT`) while it is pending. Works in one transmission too: `DCT VPCOL; ERD 28R; CLAND`. |
...
| `LAHSO 33` | Cleared to land, hold short of runway 33 (LAHSO). Includes landing clearance. The aircraft exits at a taxiway before the hold-short point when it can make one; otherwise it brakes to stop with its nose short of the point and waits for a taxi/cross command. A go-around, `TG`/`SG`/`LA`/`COPT` or `CANCLAND` afterwards cancels the hold-short. |
| `CLC` / `CTLC` | Cancel landing clearance |
| `GA` | Go around (VFR: re-enter the traffic pattern; instrument: fly published missed approach; otherwise: runway heading, 2,000 AGL). Accepted in the air and during the landing rollout, but **refused once the aircraft has decelerated below the rejected-landing speed** (jets 60 kt, turboprops 50, pistons 40; helicopters have no floor) — *"aircraft is below the go-around speed gate after touchdown; GA is no longer available"*. Below that speed it can no longer get airborne, so it continues the rollout to its runway exit instead. |
...
**Occupied-runway advisories (7110.65 3-9-4).** At airports without a full safety logic system, clearing an aircraft for a full-stop, touch-and-go, stop-and-go, low approach, or option while another aircraft is **holding in position or taxiing to line up** on the same runway draws an amber RPO warning naming the traffic — and the reverse: authorizing `LUAW` while an aircraft already holds one of those clearances (`CLAND`/`COPT`/`TG`/`SG`/`LA`/`LAHSO`/`CLANDF`, active or pre-issued) for that runway warns too. The clearance itself always goes through — this is a mentor reminder, not enforcement. A departure that has **started its takeoff roll** no longer triggers it, and `CTO` with an arrival cleared to land never does (ordinary anticipated separation). The advisory is suppressed at airports whose vNAS ARTCC config carries an ASDE-X safety-logic runway configuration (e.g. SFO) — there CRC's ASDE-X Safety Logic models the incursion itself. A second `LUAW` on a runway that already has an aircraft holding in position (either end) warns everywhere: 7110.65 3-9-4.h forbids simultaneous line-up-and-wait on the same runway between sunrise and sunset unless the local assist/local monitor position is staffed. `CTO` issued while another aircraft is **holding in position on that runway without its own takeoff clearance** also warns everywhere (7110.65 3-9-6 same-runway separation): the cleared aircraft additionally will not taxi through the traffic — it stops behind the occupant and continues once the runway is clear.

**Rolling clearance while still taxiing.** `CTO` / `LUAW` issued while the aircraft is still taxiing to its destination runway — including while it is crossing another runway on the way there — is stored and applied when the taxi route reaches that runway: the destination hold-short (and any remaining crossings of the same runway) is pre-cleared, and with a `CTO` in hand the aircraft rolls straight through the line-up without stopping (Super/Heavy still make a standing-start takeoff per 7110.65 §3-9-5.3).

#### CTO Departure Modifiers

All CTO modifiers accept an optional altitude suffix using the same format as CM/DM (see [Altitude Arguments](#altitude-arguments)) — except that on the closed-traffic modifiers `MLT`/`MRT` the first token competes with the runway slot: a 1–2 digit token there is a runway, so an altitude given without a runway needs three or more digits (`CTO MLT 015`); after a runway the shorthand works as usual (`CTO MLT 28R 15`). A bare number (1-360) without a modifier prefix is interpreted as a heading: `CTO 270` = fly heading 270, `CTO 270 050` = fly heading 270, climb to 5,000 ft.
...
| `MRT [rwy] [alt]` / `MLT [rwy] [alt]` | Closed traffic with optional altitude override (e.g., `CTO MLT 28R 15` = left traffic 28R at 1,500 ft; `CTO MLT 33` = left traffic runway 33; `CTO MLT 015` = current runway at 1,500 ft). In the first slot a 1–2 digit token (with an optional L/C/R) is a runway, so a pattern altitude given alone needs three or more digits (`MLT 015` = 1,500 ft, `MLT 15` = runway 15); after a runway the altitude slot is unambiguous and the usual shorthand works (`MLT 15 15` = runway 15 at 1,500 ft). A runway the airport does not have is rejected. | VFR only |
...
**Pattern direction on the ground** — `MRT`/`MLT` issued to a departure still on the ground (taxiing, holding short, crossing a runway, or lined up) only records the pattern side — the persistent direction later pattern operations use (closed traffic, go-around pattern re-entry). It does not build a circuit and never splices airborne pattern legs into a ground aircraft; to depart into closed traffic, clear it with `CTO MLT` / `CTO MRT`.
...
A pattern entry that is still **queued** behind another instruction (`DCT VPCOL; ERD 28R`) also accepts a clearance ahead of time: `CLAND` / `TG` / `SG` / `LA` / `COPT` issued in that window is pre-issued against the queued entry and becomes the circuit's standing clearance the moment the entry builds it. See the `CLAND` rows in the Tower Commands table.

| Command | Effect |
|---------|--------|
| `ELD` / `ERD` | Enter left/right downwind. An aircraft still outside the pattern flies a 45°/extended-downwind entry to the midfield abeam point; one already alongside the downwind (at or past the entry point, laterally near the track) joins from present position instead of backtracking to an entry point |
...
| `FOLLOW UAL123` | Follow traffic (VFR): sequence behind the lead — joins the lead's runway pattern on the follower's own side, or falls in trail on its final |
...
- **Without a distance** — the aircraft turns perpendicular to the extended centerline from its current position (matching 7110.65 §3-1-9 *"TURN BASE LEG NOW"*). The base-leg length equals the aircraft's current cross-track; the final turn fires at the aircraft's projection onto centerline. Rejected with `"Unable, too close for base"` if the along-track from threshold is below the category floor (jets/turboprops 2.0 nm, pistons 1.0 nm, helicopters 0.5 nm) or the aircraft is past the threshold. Altitude is budgeted the way the base leg is flown: the aircraft's own base speed (a standing `SPD` assignment wins), descending over the base leg to the 3° glideslope altitude at the base-to-final rollout, at the category's maximum pattern descent rate (1500 fpm light aircraft / 2000 fpm jets, capped by a 7.5° / 6.5° path angle at the planned speed — slowing the aircraft down does not buy descent room). Up to 1.5× that rate the entry is still accepted but raises a controller-facing warning (`"… high for base to 28R — may need S-turns or a go-around"`) — the pilot may go around at the missed approach point. Beyond that it is rejected with `"Unable, too high for base"` — issue `DM` (descend maintain) first.
...
**Runway changes void the landing clearance.** A landing clearance names a runway (7110.65 §3-10-5), so any pattern entry (`EF` / `ERB` / `ELB` / `ELD` / `ERD` / `ELC` / `ERC`) whose runway argument names a *different* runway than the standing clearance cancels it — reissue `CLAND`/`TG`/`SG`/`LA`/`COPT` for the new runway. Re-entering the pattern for the same runway keeps the clearance. A **pre-issued** clearance (one waiting on a queued pattern entry) names a runway the same way: if the entry that finally builds is for a different runway, it is voided with an RPO warning instead of applying. The instrument sidestep (`EF` to a closely-spaced parallel while established on `FinalApproach`) is exempt: there the approach clearance itself authorizes the landing on the parallel (7110.65 §4-8-7, AIM §5-4-19). The **low-approach runway change** (`CLAND <divergingRunway>` during an `LA`) is the one case where `CLAND` *itself* performs the change-to-runway: it reassigns the runway, voids the low-approach clearance, and re-issues the landing clearance for the new runway in one step (7110.65 §3-10-5.a.3).
...
- **Same-runway pattern member** (follower is already entering or flying the pattern for the lead's runway — including mid-`ERD`/`ELD`, and even during a wrong-side midfield crossing): the follow is recorded in place; the entry or leg continues unchanged and the pattern spacing below takes over. While the lead is ahead of the follower in the pattern (on base or final to the same runway), the follower holds its base turn — extending the downwind past its normal base-turn point — until turning base would roll it out at least the category spacing behind the lead, so it sequences in trail instead of cutting inside and overtaking it. The same in-trail leg-hold applies on the **upwind and crosswind** legs: rather than turning ahead of the traffic it is following, the follower extends its current leg to stay behind it. A follower **never turns on its own to break the hold** — past a 4 nm extension it reports "extending … unable to turn" and keeps flying the leg until you turn it or re-sequence it.
- **Pattern join** (follower is *not* in the lead's pattern — hand-flown on headings/direct, on some other phase, or flying a pattern to a different runway — and the lead is established toward a runway: entering or flying its pattern, on final, or landing): the follower joins the **lead's runway pattern from where it is**, on the runway's **established circuit side** (the lead's own traffic direction, or the runway's natural side — 28R with 28L present flies right traffic). An aircraft already alongside that downwind joins it directly rather than backtracking to an entry point; one on the wrong side crosses midfield at pattern altitude first (the standard AIM 4-3-3 wrong-side entry). The pattern-tight spacing (1.0/1.5/2.0 nm by category) and leg-holds above then sequence it behind the lead. This is what makes "follow the traffic on final" work from a downwind: the follower continues the downwind, extends as needed, and turns base behind the traffic instead of turning toward it.
- **Straight-in sequencing** (lead is on final/landing and the follower is already positioned out on the approach corridor behind it — near the extended centerline on the arrival side, with no parallel runway's final in between): the follower falls in trail directly instead of flying a pattern. Once it is at least the in-trail minimum behind the lead and within a sane intercept of the extended centerline (≤ 1 nm cross-track, ≤ 30° intercept — a join that would cut across a closely-spaced parallel's final approach course is refused), it is sequenced onto that runway's final via `PatternEntryPhase → FinalApproachPhase → LandingPhase`. The follower descends on the glideslope behind the traffic, but **no landing clearance is set** — it holds for a separate `CLAND` (FAA 7110.65 §3-10-6) and goes around at minimums if never cleared. The clearance can also be issued **up front** while the follower is still being sequenced: `CLAND 28R` (or a bare `CLAND` to inherit the lead's runway) arms it, and it is applied automatically once the follower joins the runway final, so the follower lands behind the traffic without a second clearance. The in-trail minimum keeps it genuinely behind the traffic (AIM §4-3-4.4 — never cut in front) at the 1.5 nm same-runway floor for a light single behind same/lighter traffic, raised to the CWT wake minimum (7110.65 Table 5-5-2) when the lead is heavier. If the lead lands before the follower has rolled onto final, the follower is still sequenced onto that runway's final rather than levelling off over the field. `FOLLOW` is rejected outright when the lead is a **super** (visual separation not authorized behind a super, 7110.65 §7-2-1).
- **Free pursuit** (lead is genuinely free-flying — not established toward any runway): the follower flies a trail behind the lead rather than aiming its nose at the lead's instantaneous position (AIM §5-5-12 — the pilot maneuvers as necessary to keep in-trail separation). Steering is relative to the lead's ground track: far behind, it lag-pursues a point at the desired distance behind the lead and curves into trail; once established, it flies parallel to the lead's track with only a gentle cross-track correction; when too close, it slows first and — if it's already at approach speed and slowing alone can't open the gap — makes a shallow widen (a few degrees off the lead's track, AIM §4-3-5) to bleed distance. Speed still tracks the lead with distance-based correction (±20 kts, wider free-flight spacing of 1.5/2.0/3.5 nm by category). Altitude is left at whatever the controller last assigned — real pilots do not dive/climb onto the lead; they maintain visual separation from their current level. (When the lead is nearly stopped, its ground track is unreliable, so the follower simply points at it. If the lead later enters a pattern, the follower auto-joins it once within 3 nm of the lead's downwind abeam point, within 5 nm of the lead, and on the correct side of the runway.)

- **Cross-runway re-sequence** (follower is flying a pattern to one runway, the lead is landing another — e.g. a 28L pattern told to follow traffic landing 28R): in-trail sequencing only means something on a shared runway, so `FOLLOW` **moves the follower onto the lead's runway** rather than leaving it on a pattern where nothing sequences — the pattern join above, built on the follower's current side of the lead's runway. A runway-specific landing clearance is **not** carried across (7110.65 §3-10-5.c — a runway change needs a new clearance), so re-issue `CLAND` for the new runway. This is refused once the follower is already on **base or final** — *"Unable, established for runway 28L — vector or go around"* — because swinging a low, close-in aircraft onto a closely-spaced parallel would cross the original runway's final approach course (AIM §4-3-3 FIG 4-3-3 note 7; §4-3-5). Re-sequence it yourself with `ELB`/`ERB`, vector it, or send it around.
...
Approach clearances use FAA [CIFP](#glossary) procedure data. Approach IDs can be full CIFP identifiers (e.g., `I28R`) or common shorthand (e.g., `ILS28R`, `RNAV17L`, `LOC30`, `VOR27`, `NDB16`). The runway number may be typed FAA-style without the leading zero — `I8R` resolves the same as `I08R`, and `ILS8R` the same as `ILS08R`.
...
**In-trail speed restated with the clearance:** An approach clearance cancels previously assigned speeds (7110.65 §5-7-1.d). When the simulated approach controller is holding the arrival to an in-trail speed reduction (the *Auto arrival spacing behind traffic on the runway* setting), that reduction stays in force, so `CAPP` and `PTAC` (and their forced variants) restate it in the terminal log as §5-7-1.c requires: `NCT → UAL123: maintain 180 knots (in-trail spacing, restated with the approach clearance)`. Nothing is restated when no reduction is in force, or when the standing instruction is the simulated tower's "reduce to final approach speed".
...
**Visual approach:** `CVA 28R` clears the aircraft for a visual approach to runway 28R. No CIFP procedure is required — the aircraft navigates visually. **Requires an IFR flight plan**; VFR pattern entry uses `ELD`/`ERD`/`SI`. Per 7110.65 §7-4-3 the pilot must have reported **either** the field in sight (`RFIS`/`RFISF`) when number-one, **or** the preceding traffic in sight (`RTIS`/`RTISF`) when given a `FOLLOW` clause — a following aircraft need not report the field (§7-4-3.c.2 NOTE). Without the required report `CVA` is rejected (*"Field not in sight — issue RFIS first"* / *"Traffic not in sight — issue RTIS first"*). Visual separation is not authorized behind a **super**, so `CVA … FOLLOW` a super is refused. **Weather minimums are enforced** (7110.65 §7-4-3.b / AIM §5-5-11.b.1): `CVA` is rejected below a 1000 ft ceiling or 3 SM visibility (*"Unable, … weather below visual approach minimums"*). The check uses the weather YAAT resolves for the destination — a field with no station of its own is judged on area weather interpolated from nearby stations (the §7-4-2 "reasonable assurance" proxy); only a scenario with no weather at all skips the gate entirely. Note the practical envelope at exactly 1000/3: the pilot can hold the field in sight out to roughly 5 nm at pattern altitudes (farther from above the haze layer), so a 3 SM visual is a close-in conversion, not a 10-mile sequence. A wide-angle join (>90° off the final course) additionally requires a reported ceiling of at least 2500 ft AGL — a **sim margin**, stricter than the regulation (a visual only requires "clear of clouds"), chosen because the fixed 2000 ft AGL IFR downwind would otherwise be built inside or scraping the deck and the pilot would immediately break off with nothing in sight (*"Unable, ceiling … too low for a visual pattern entry — vector for a closer join"*). The one-shot **`CVAF`** (`VISUALF`, RPO-only, rejected in solo training) folds the required `RFISF`/`RTISF` into the clearance so no separate report is needed, and bypasses the weather gates. Pattern-entry geometry is sized for an IFR aircraft being vectored from cruise: downwind altitude is held at 2000 ft AGL above the field (clear of standard 1000 ft VFR pattern traffic), and parallel-runway pattern deconfliction does not apply. Options:
...
| `CVAF 28R` | Forced visual approach — bypasses the RFIS/RTIS prerequisites and the weather gates (RPO-only) |
...
| `RNS` / `NS` | Resume normal speed: clears speed/floor/ceiling, preserves SID/STAR via mode. An arrival on a long final that was slowed well below its approach profile speeds back up to it (outside roughly 15 NM; closer in it keeps slowing to its approach speed). When the arrival is already on its final approach speed profile — closer in than that, or within 10 knots of the profile speed — nothing changes, and the command answers `Resume normal speed — already on its final approach speed profile, no change` instead of `Resume normal speed` |
| `EXP` | Expedite the climb/descent already in progress. Climbs gain ~15% (thrust-limited) up to a per-category cap; descents roughly double (drag-limited) within a per-category band — e.g. a jet descent tops out at 4,000 fpm, a piston at 1,500. Requires an active climb or descent — rejected when the aircraft is level at its assigned altitude. On the ground with an assigned taxi route, raises the taxi speed cap by ~30% (jet 30→39 kts; mutually exclusive with a numeric `SPD` taxi speed); cleared by next HOLD/RES/HS. On a just-landed aircraft (rolling out or exiting), expedites the runway exit instead — earliest reachable exit + max-effort braking (see the exit commands above) |
...
**Traffic-pattern speeds** — each leg of the pattern has its own baseline speed (upwind/crosswind/downwind, then base, then final), and an aircraft joining a pattern targets the speed of the leg it actually joins: an `ELB`/`ERB` or `EF` entry slows toward base or final speed rather than accelerating to downwind speed first. A speed **you** assign outranks those baselines and holds for the rest of the circuit — per 7110.65 §5-7-4 only the controller terminates a speed adjustment, so turning onto the next leg does not revert it. `RNS` hands the leg baselines back. The 5nm gate below applies only to an aircraft actually **on final**, so traffic on upwind, crosswind, downwind, or base accepts normal speed instructions even when it happens to be within 5nm of the threshold.

**Auto-cancel at 5nm final** — Per 7110.65 §5-7-1.b.4, an ATC speed assignment is released — and a new speed instruction (`SPD`, `RFAS`, `RNS`, `DSR`, `MACH`) declined with a pilot **"unable"** — when an aircraft is **inbound to land** within 5nm of the runway's landing threshold (the controller can no longer adjust the speed; the pilot owns the approach speed). On a runway with a displaced threshold the 5 miles are measured from the displaced (landing) threshold, not from the start of the pavement. The instruction is refused, not obeyed; the aircraft stays on its stabilized approach (it is never knocked off the final). Inside the gate the aircraft never speeds back up: an arrival flying an approach or pattern slows to its own approach speed, and a hand-vectored arrival holds its last-assigned speed (or eases further down) but is never re-accelerated toward the descent default. (`RNS` issued on a long final, well outside the gate, does return a slowed arrival to its approach profile speed.) This applies only on final — departures, go-arounds, and missed approaches climbing within 5nm of the field are **not** affected and accept normal speed assignments. To deliberately assign a speed to an arrival inside the 5nm gate (e.g. military or compression scenarios), use `SPEEDF`, which overrides the restriction and persists past the gate.
...
- **Lined up**: `{runway}` is the runway designator (e.g., `28R`). Aircraft spawns on the runway threshold, ready for takeoff clearance. An IFR lined-up spawn files the primary airport as its departure, so it gets a departure strip and, where the facility has TDLS, a queued PDC like a scenario departure; the airborne, fix and parking forms file no departure airport.
...
- **At parking/helipad**: `@{spot}` is a parking or helipad name (e.g., `@H1`, `@B12`). Aircraft spawns at ground level with no departure or destination filed (so no departure strip or PDC is printed for it — amend the flight plan first if you want one). Useful for helicopters and ground operations — use the `H` engine token (`ADD VFR S H @H1`) to spawn a helicopter.
...
| `CMN 50` / `DMN 50` | Force altitude: immediately set altitude to 5,000 ft |
