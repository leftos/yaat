...
| **Approach** | `ApproachCommandHandler` | CAPP/JAPP/JFAC/PTAC/CVA. Deferred clearance → `aircraft.Approach.PendingClearance` (`:112`); immediate → `aircraft.Phases = new PhaseList { AssignedRunway, ActiveApproach }` (`:130`, `:303`, `:391`, `:481`). Procedure-turn engagement (see [phases.md](phases.md)). `ClearArrivalProcedureState` (`:1757`) tears down STAR/pending/expected/route on airport change. |
