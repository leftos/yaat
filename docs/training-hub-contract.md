...
| `RecordingLoaded` | `RewindResultDto` — the full load result, not just a manifest | `RecordingLoaded` — the third world-replacing reload, and the widest: a recording swaps the scenario *and* enters playback, so scenario identity and tape bounds are needed alongside the aircraft. Clients feed it to the same `ApplyRecordingResult` the loader uses, then fetch `GetTerminalLog` for the recording's tape. `OthersInGroup` — the loader already applied this DTO as its RPC result. Bookmarks arrive separately via `BookmarksChanged`, which `RoomEngine.LoadRecording*Async` fires after seeding them from the archive |
...
- **Beacon code is two fields, not one.** `AircraftTransponder` has `AssignedCode` (what ATC filed/recycled) and `Code`
  ...
  flash after a `SQVFR`/`SQV` or `SQ 1200` (released when a new code is assigned, on `SQ` any other code, or on a bare `SQ`); it is in `TrainingDtoFingerprint` so the flash turns
  off / back on live.
