...
- `AsdexState.Reset()` / `SaidState.Reset()` clear the active alerts (ASDE-X only), the per-tick termination drain and the coasting surface tracks — but **not** temp data or presets. Those are facility furniture, not scenario state; see "Surface temp data" below. The ASDE-X safety-logic configuration is scenario state (`SimScenarioState.AsdexSafetyLogicConfig`), so it goes with the engine the unload drops.
