...
GroundConflictDetector.cs      # Static pairwise ground proximity → SpeedLimit overrides. Runway priority via RunwayOccupancy.ClassifyByPhase;
                               # tug move vs parked/held neighbour: the outline sweep (TugMoveFoulsParkedAt, floor anchored to the move's start) and
                               # the outline stop (TugMoveLimit, margin from PushbackSpeed); an active PushbackPhase is never Stationary.
