...
Services/
  ...
  UserPreferences.cs            # JSON to %LOCALAPPDATA%/yaat/preferences.json (incl. macros, favorite command scope/panel-grid/spacer metadata, named favorite-command sets + loaded-set list (FavoriteCommandSet, NormalizeFavoriteSets invariant, FavoriteSetsChanged event), favorite video maps per ARTCC/airport/scenario, favorite METAR stations per scenario, and the macOS renderer-backend override read by Program.BuildAvaloniaApp)
  FavoriteSetSerialization.cs   # Parses/serializes .yaat-favorites.json export files, auto-detecting flat favorites list (array root) vs all-sets bundle (object root, FavoriteSetsExportFile)
...
ViewModels/
  ...
  MainViewModel.Favorites.cs    # Partial: favorite commands (quick-access bar/panel, global/scenario/airport scope, ground overrides, blank spacers) + named favorite sets (ComposeDisplayFavorites overlays loaded sets after the scope-filtered base pool; container-aware mutators route edits to the owning set; bundle import merge)
  FavoriteSetEditorModel.cs     # Pure list surgery behind the Favorites Editor: multi-select MoveUp/MoveDown (contiguous-block aware), CopyTo (clones — no cross-container reference bleed), MoveTo
...
Views/
  ...
  FavoritesBarView.axaml.cs     # Favorite command buttons bar and tabbed panel content (click/ctrl+click/right-click); Sets load/unload flyout (bar + palette header); palette-header Import/Export (.yaat-favorites.json, flat + all-sets bundle)
  FavoritesPanelWindow.axaml.cs # Pop-out favorite commands panel with saved geometry
  FavoriteImportWindow.axaml.cs # Favorites import prompt: Append vs Replace all (returns FavoriteImportMode?); CreateForBundle variant relabels to Merge / Replace All Sets for all-sets files
  FavoritesEditorWindow.axaml.cs # Favorites Editor (Sets → Manage sets…): container pane (base pool + named sets with Loaded checkboxes; set create/rename/delete) + multi-select favorites pane (move up/down, copy/move across containers, delete)
...
FinalApproachPhase.cs          # Glideslope; no-clearance warning/go-around at DA/MDA when published, otherwise 200ft AGL; illegal intercept check (§5-9-1); two-stage decel to Vref at a per-aircraft distance (FinalApproachSpeedVariety)
...
AirborneFollowHelper.cs        # Shared spacing math. GetAdjustedSpeed for pattern phases (ctx-based, slow-only: capped at leg baseline) + AdjustedFreeFlightSpeed for VfrFollowPhase (wider margins) + ComputeFreePursuitHeading (free-pursuit lateral trail-keeping law: parallel/lag-pursuit/widen regimes, FollowWidenState hysteresis). Auto-cancels with warning if follower can't maintain separation at min speed. ShouldBreakOffFollowForSpacing: a base/final follower in a structural overtake (own Vref > lead gs + 10 kt) closing inside 0.8 nm breaks off + goes around rather than overfly/cut in front
