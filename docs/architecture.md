...
SimulationEngine.Generators.cs # Arrival/VFR/overflight generators: spawning, spacing, weight and engine selection; stand down in every replay/playback; AutoTrackGeneratedSpawn (a spawn whose generator carries an autotrack configuration is owned, then recorded, then its [AutoTrack] lines emitted); TickPrePhysicsResult is one SpawnedAircraft list
