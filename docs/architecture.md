...
ActionRouter.cs                # SimulationEngine.Actions. Issue(ActionInput, host) for a fresh command, Apply(RecordedCommand, host) for a recorded one,
                               ...
                               # with the identity code resolved, a CRR group via the host slot; a refusal is a replay-fidelity warning), IssueDerived(record, host)
...
IActionHost.cs                 # The action-path view of a host, part of ISimulationHost: slots for the bodies the server still owns (ApplyStrip, ApplyTdls,
                               ...
                               # ApplyTransport, ApplyRecordedAsdexMutation / ApplyRecordedSaidMutation / ApplyRecordedEramCrrGroup for the recorded CRC display
                               # mutations and CRR groups), one query (IsPositionAttended — CRC attendance, the C1 input a full CON+ and the handoff redirect read; false on
