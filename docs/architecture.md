...
Phases/RunwayInfo.cs           # Runway geometry. Both ends' thresholds/headings/elevations; Designator picks the active one. PavementLengthFt is DERIVED from the two ends' coordinates and is the only length — the nav data's landing_distance_available is deliberately not stored (declared per end, differs between them, and no caller wants it; an arrival's usable distance comes from LandingThreshold).
