...
Commands/TrackEngine.cs             # Pure domain logic for STARS track ops: Track, Drop, Handoff, Accept, Cancel, PointOut, Acknowledge,
                                    ...
                                    # leaves) until tick step 3d-5 — keep both in sync.
...
ActionArms.cs                  # The Sim bodies: Aviation (ParseCompound → ReactionDelayPolicy → defer or DispatchCompound → ApplyPostDispatch), FlightPlan
                               ...
                               # (TrackEngine.Dispatch), SquawkAll, HFR/HFROFF/REL (baked jitter else ReleaseJitterRng), Cfr (baked clock else now),
                               # Timer, TaxiAll, AddAircraft (SimulationEngine.AddAircraft; bakes the spawned aircraft onto a fresh record); NotAvailable
                               # for the rows whose body has not crossed yet (ACCEPTALL/HOALL, GHOST, RPOS, CONS/DECON)
