...
FinalApproachPhase.cs          # Glideslope; no-clearance warning/go-around at DA/MDA when published, otherwise 200ft AGL; illegal intercept check (§5-9-1); uncontrolled decel stages clean → approach-flap (FinalApproachSpeedSchedule, ~9nm) → 1.3·Vref (5nm) → Vref at a per-aircraft distance (FinalApproachSpeedVariety)
