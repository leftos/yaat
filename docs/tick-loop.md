...
The action path has the same shape: `IActionHost` (`Simulation/Actions/`) is the host's view for a controller *action* — slots for the bodies the server still owns (strips, TDLS, coordination, ASDE-X alert inhibits, bookmarks, the room clock, the flight-plan verbs' normalization) and consumers a Sim arm notifies (a spawned or deleted aircraft, a selected position, changed timers or held departures, an applied amendment). `BareHost` refuses every slot and discards every consumer; `ReplayHost` delegates to it. The server hosts implement it when reconstruction moves onto the router (tick step 3d-4), at which point it folds into `ISimulationHost`. See [command-pipeline.md](command-pipeline.md) and [snapshots-and-replay.md](snapshots-and-replay.md) for the router itself.
