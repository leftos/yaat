...
The action path has the same shape, and `ISimulationHost` includes it: `IActionHost` (`Simulation/Actions/`) is the host's view for a controller *action* — slots for the bodies the server still owns (strips, TDLS, coordination, ASDE-X alert inhibits and the recorded ASDE-X / SAID mutations, bookmarks, the room clock, the flight-plan verbs' normalization), one query (CRC attendance) and consumers a Sim arm or applier notifies (a spawned or deleted aircraft, a selected position, changed timers, held departures or consolidation, an applied amendment, a weather change). `BareHost` refuses every slot and discards every consumer; `ReplayHost` delegates to it; yaat-server's `RoomHost` answers with the room's bodies, gating the room-side tails on whether the room is replaying (a rewind must not reprint strips or re-broadcast what the clients already saw). See [command-pipeline.md](command-pipeline.md) and [snapshots-and-replay.md](snapshots-and-replay.md) for the router itself.
