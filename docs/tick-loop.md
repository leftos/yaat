...
PostPhysics    SpineOrder.PostPhysics — the live server's 32-step order
               ...
               ├─ host AutoArrivalStrips, AutoApproachDepartureStrips
               ...
               ├─ sim drain TDLS changes → host                       what this second's TDLS steps touched, before AutoDelete so the item's aircraft still resolves
...
The action path has the same shape, and `ISimulationHost` includes it: `IActionHost` (`Simulation/Actions/`) is the host's view for a controller *action* — slots for the bodies the server still owns (strips, coordination, ASDE-X alert inhibits, the recorded ASDE-X / SAID mutations, CRR groups, strip requests and the safety-logic configuration, bookmarks, the room clock) and consumers a Sim arm or applier notifies (a spawned or deleted aircraft, a hidden live-traffic shadow, a selected position, an acquired track, a lifted ghost overlay, a terminated surface track, changed timers, held departures or consolidation, an applied amendment, a weather change, a queued-commands listing, and the TDLS change set — `OnTdlsChanged`, declared on `ITdlsChangeConsumer` so the router's action view and the spine's consumer view share one implementation). TDLS is no slot at all since 2026-09-07: its bodies are the engine's (`SimulationEngine.Tdls.cs`, `TdlsCommandHandler`), the engine's spawn hook (`AfterAircraftSpawned`) runs before the host's spawn consumer, and the host only broadcasts what the change tracker drained. `BareHost` refuses every slot and discards every consumer; `ReplayHost` delegates to it; yaat-server's `RoomHost` answers with the room's bodies, and its consumers carry the room's tails for a fresh action (the spawn hooks and broadcasts, display config on a position change, the CRC and hub broadcasts) while a replaying room keeps its change tracker honest and nothing more.
