...
The action path has the same shape, and `ISimulationHost` includes it: `IActionHost` (`Simulation/Actions/`) is the host's view for a controller *action* — slots for the bodies the server still owns (coordination, ASDE-X alert inhibits, the recorded ASDE-X / SAID mutations, CRR groups and the safety-logic configuration, bookmarks, the room clock) and consumers a Sim arm or applier notifies (a spawned or deleted aircraft, a hidden live-traffic shadow, a selected position, an acquired track, a lifted ghost overlay, a terminated surface track, changed timers, held departures or consolidation, a weather change, a queued-commands listing, and the strip and TDLS change sets — `OnStripsChanged` / `OnTdlsChanged`, declared on `IStateChangeConsumer` so the router's action view and the spine's consumer view share one implementation). Strips and TDLS are no slots at all since 2026-09-07: their bodies are the engine's (`SimulationEngine.Strips.cs` / `.Tdls.cs`, `StripCommandHandler`, `StripRequests`, `TdlsCommandHandler`), the engine's spawn hook (`AfterAircraftSpawned`: the PDC auto-queue, then the strip auto-print) runs before the host's spawn consumer, the amendment reprint is the engine's, and the host only broadcasts what the change trackers drained. `BareHost` refuses every slot and discards every consumer; `ReplayHost` delegates to it; yaat-server's `RoomHost` answers with the room's bodies, and its consumers carry the room's tails for a fresh action (the spawn hooks and broadcasts, display config on a position change, the CRC and hub broadcasts) while a replaying room keeps its change tracker honest and nothing more.
