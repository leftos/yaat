...
- **Findings so far.** *Physics was identical on all four run kinds from the first sweep* — zero position/altitude/speed/heading divergence over 600 s without weather; with weather it diverged on every leg (the three-semantics cascade ADR 0001 predicts) until one advance semantic landed (step 5, 2026-09-04). The remaining gap was ATC/track state the host owned: the auto-accept, delayed-handoff and autotrack passes, position-history sampling, auto-delete. Each retired as its body crossed into the engine (steps 5 and 4), and **since step 4 sub-commit B (2026-09-06) all twelve baselines are empty** — every leg reproduces live over what the fixtures reach. *What they do not reach* (a green sweep is not full coverage): the deferred / FP-creator autotrack passes and a generator spawn carrying an autotrack configuration (pinned by `AutoTrackStepTests` in Yaat.Sim.Tests instead), coordination timers, solo-training evaluation, the auto-delete mode gates, and everything outside `CaptureSnapshot` (strips, TDLS, ASDE-X, tower lists). Attendance is reached: the `actions` fixture signs a CRC client on to 4U from second 37 to 91 (`ScriptedAttendance`), so a leg that failed to apply the recorded attendance would auto-accept `HO 4U` at 43 and show on `Track.Owner`.
