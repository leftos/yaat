...
- **The legs.** `live` is `RoomEngine.AdvanceLiveSecond()`, the production per-second body. `test` is `SimulationEngine.TickOneSecond()`. `replay` is `SimulationEngine.ReplayOneSecond()` — which runs the *same* `TickPrePhysics → TickPhysics×4 → TickPostPhysics` sequence as `test` and differs from it only in the replay guards and in injecting recorded actions. That is precisely why it earns its own leg: the run-profile work replaces those guards, and a regression there can surface on no other pair. `reconstruct` is `RecordingManager.ReconstructViaServerTick()` — the only leg that is a genuinely different code path rather than a differently-guarded one, since it drives `BeginSecond` + `RunSecondPhysics` and therefore runs the live `TickProcessor` post-physics list. Every room is loaded through the server's own `LoadScenarioSeededAsync`, so the scenario load is not a variable and only the per-second stepping differs; the replay room is armed afterwards with `SimulationEngine.ArmReplay(actions)`, using an action log a first live pass collected (it must be complete before arming — `Arm` latches `ReplayHasRecordedAircraftSpawns` from the list it is handed).
- **Findings so far.** *Physics is identical on all four* — zero position/altitude/speed/heading divergence over 600 s. The entire live-vs-test and live-vs-replay gap (14 paths each, the same 14) is ATC/track state: the live-only auto-accept, autotrack and delayed-handoff passes, position-history sampling, and one `InternalAirports` argument asymmetry. *Reconstruction reproduces live exactly except position history* — one path, because it runs the server's own post-physics list; everything the other two legs disagree about, it gets right. Its set is near-disjoint from theirs, which is the shape ADR 0003 predicts.
...
- **Accepted divergences**: one baseline file per compared pair under `yaat-server/docs/tick-oracle/`, named for the pair (`live-vs-test.baseline.json`), one entry per normalized path with the step it is attributed to. The pair itself is an `OraclePair` in the driver — key, the two side labels, and the long comparison string — so adding a leg is that record plus the loop driving its right-hand side; the csproj globs `*.baseline.json`, so adding one does not mean editing it. The path set is asserted exactly, so both a new divergence and a silently-fixed one fail. Regenerate deliberately with `YAAT_ORACLE_REBASELINE=1` and review the diff before committing.
...
**Three things it cannot see, all worth knowing before trusting a green run.** It sees exactly what `CaptureSnapshot` covers, so strips, TDLS, ASDE-X alerts, tower lists and the approach evaluator — room-held, no snapshot DTO — are invisible until that state moves into `Yaat.Sim`. It sees only what its scenario exercises: the current fixture defines no weather and parks nothing within the window, so the weather-advance cascade and the missing `TickAutoDelete` do not appear in any baseline even though both are real. And **the replay leg is weaker than it looks on this fixture** — the action log carries no `RecordedAircraftSpawn` (asserted, not assumed), so the generator stand-down guard is never reached, while recording suppression is invisible in principle because the action log is not part of `ScenarioSnapshotDto`. It is not simulation state. The test's own doc comment carries the current blind-spot list.
