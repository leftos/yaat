...
| File | Owns |
|---|---|
| `SimulationEngine.cs` | Engine state (`World`, `Scenario`, the evaluators and pools), the lifecycle events (`TickCompleted`, `WarningEmitted`, `PilotSpeechEmitted`, `TerminalEntryEmitted`) and their `Fire*` helpers, the constructor, `FindAircraft`, `DeleteAircraft`, and the terminal-entry sink every other partial writes to. |
...
PrePhysics     SpineOrder.PrePhysics
               ...
               └─ sim TickLiveTrafficSync(host.LiveTrafficFeed)      last, so a sample placed at this second records at this second; the host's feed port answers, the engine spawns/refreshes/removes
...
|---|---|---|---|
| `BareHost` | `Yaat.Sim` — `TickOneSecond`, the `RunKind.Test` run | all empty (delayed handoffs, auto-accept and the point-out timeout are sim steps, so a bare run performs them) | the engine's events (`WarningEmitted`, `TerminalEntryEmitted`, `PilotSpeechEmitted`); the strip and TDLS change sets, the ASDE-X alert diff, auto-deleted aircraft and solo findings are discarded (the removal, the standing alert set and the evaluator's record already happened in the sim) |
...
The action path has the same shape, and `ISimulationHost` includes it: `IActionHost` (`Simulation/Actions/`) is the host's view for a controller *action* — no `Apply*` slot is left (every recorded state change has an engine body; the last, the ASDE-X safety-logic configuration, is `SimulationEngine.ApplyRecordedAsdexSafetyLogic` writing `SimScenarioState.AsdexSafetyLogicConfig`), only the consumers a Sim arm or applier notifies (a spawned or deleted aircraft, a hidden live-traffic shadow, a selected position, a lifted ghost overlay, a terminated surface track, changed timers, held departures or consolidation, a weather change, a queued-commands listing, and the strip and TDLS change sets plus the coordination, bookmark, session-clock and CRR-group dirty flags and the ASDE-X alert diff — `OnStripsChanged` / `OnTdlsChanged` / `OnCoordinationChanged` / `OnBookmarksChanged` / `OnSimStateChanged` / `OnEramCrrGroupsChanged` / `OnAsdexAlertsChanged`, declared on `IStateChangeConsumer` so the router's action view and the spine's consumer view share one implementation). Bookmarks and the session clock crossed 2026-09-08 (`BookmarkCommandHandler` / `SimulationEngine.Bookmarks.cs`, `TransportCommandHandler` / `SimulationEngine.Transport.cs`; `RecordingPolicy.Never` on both verbs is unchanged — a recorded `BM` or `PAUSE` stays inert, the body's home has nothing to do with whether it is replayed — and the server's bookmark RPC wrappers call the engine bodies then drain, so a bookmark added on a paused room still broadcasts). Strips, TDLS and coordination are no slots at all since 2026-09-07: their bodies are the engine's (`SimulationEngine.Strips.cs` / `.Tdls.cs`, `StripCommandHandler`, `StripRequests`, `TdlsCommandHandler`), the engine's spawn hook (`AfterAircraftSpawned`: the PDC auto-queue, then the strip auto-print) runs before the host's spawn consumer, the amendment reprint is the engine's, the coordination verbs (`CoordinationCommandHandler` in `Simulation/Coordination/`), the coordination timers step and the removal of an aircraft's items on `TRACK` are the engine's (`SimulationEngine.Coordination.cs`; an item's id is `{ListId}-{SequenceNumber}`, so every run kind mints the same one), and the host only broadcasts what the change trackers drained — for coordination the whole `StarsCoordination` topic, since that is what the wire carries. `BareHost` discards every consumer; `ReplayHost` delegates to it; yaat-server's `RoomHost` owns no action body, and its consumers carry the room's tails for a fresh action (the spawn hooks and broadcasts, display config on a position change, the CRC and hub broadcasts) while a replaying room keeps its change tracker honest and nothing more.

**`IHostSteps` has no simulation-affecting member left** (ADR 0001's residue is gone as of tick-path 04e): the last one, `LiveTrafficSync`, became `SimulationEngine.TickLiveTrafficSync` over a feed port the host supplies (`IHostConsumers.LiveTrafficFeed`, `Yaat.Sim.LiveTraffic.ILiveTrafficFeedPort`). The port answers "what does the feed say this second" (`BeginSecond` → `LiveTrafficFeedSecond`: sync or inert, a clear-all reason, the scoped tracks already on sim time; `ShadowStatus(callsign)` per standing shadow; `EndSecond`); the engine decides spawn vs refresh, the collision and filter gates and the removal tiers, and reports each outcome through `IHostConsumers.OnLiveTraffic*` consumers. `BareHost` and `ReplayHost` answer with `EmptyLiveTrafficFeedPort`, which is always inert. The wall clock never enters the Sim: the port converts observation times to sim seconds.
...
- **ASDE-X alert processing is a sim step; the host only broadcasts the diff.** `SimulationEngine.TickAsdexAlerts` (`SimulationEngine.Asdex.cs`) reads the scenario's safety-logic configuration (`SimScenarioState.AsdexSafetyLogicConfig`), builds the runway surfaces and taxiway segments, calls `AsdexSafetyLogicDetector.Detect` once and folds the findings into the standing set, `SimScenarioState.ActiveAsdexAlerts` (snapshotted as `ScenarioSnapshotDto.ActiveAsdexAlerts`); with no configuration standing it clears the set instead. What leaves the engine is `IStateChangeConsumer.OnAsdexAlertsChanged(newAlerts, clearedAlertIds)` — a diff, because CRC's alert topic is additive with an explicit delete, never called with both sides empty. `RoomHost` sends it as `ReceiveAsdexAlerts` / `DeleteAsdexAlerts` unless `Room.IsBroadcastSuppressed`; `BareHost` discards it and `ReplayHost` delegates to `BareHost`. The standing set is an ordinal `ImmutableSortedDictionary` swapped by reference: it is written only on the tick thread, and the CRC initial-data build reads it from a connection's thread, off the tick gate — take the reference once per body.
...
**What it still cannot see, worth knowing before trusting a green run.** It sees exactly what `CaptureSnapshot` covers, so the approach evaluator — room-held, no snapshot DTO — is invisible until that state moves into `Yaat.Sim` (the ASDE-X safety-logic configuration and the standing alert set are covered — `ScenarioSnapshotDto.AsdexSafetyLogicConfig` / `.ActiveAsdexAlerts` — but no fixture pushes a configuration, so both are null on every leg and the alert step is pinned by `AsdexAlertStepTests` instead); tower lists are covered since 2026-09-07 (`ServerSnapshotDto.TowerLists`, the dwell entries, schema 23), and the step runs on every leg. Strips and TDLS *are* covered since 2026-09-07 (`ServerSnapshotDto.Strips` / `.Tdls`), and since their bodies crossed the same day the spawn hook (N342T's delayed spawn at t=271), the flight-plan-amendment reprint (the scripted `FP` at t=50), the TDLS arm and the `ADD` spawn hook all run on every leg — the `actions` fixture scripts `TDLSQ`/`TDLSS`/`TDLSW` on N172SP and an IFR `ADD` on 28R, so they are reached; a verb that mints a Guid id (`SEP`, `HSC`, `SCAN`) cannot be scripted in it — the live leg is a second run compared against the first run's log, and two runs never draw the same Guid, so those stay pinned by `StripIdBakingTests`. The fixture's `ADD V S P @NEW1` is a parking spawn, and a parking spawn files no departure airport (`AircraftGenerator.GenerateAtParking` builds an empty plan; only the IFR runway form files one), so `IsDepartureAircraft` is false and the spawn hook is not reached through the `ADD` arm on any leg, and no fixture departure acquires a track owner inside its window, so the `TdlsTrackRemoval` step never fires (pinned by `TdlsStepTests` instead). Coordination is reached since 2026-09-07: the `actions` fixture scripts `RD` / `RDH` / `RDACK` / `RDR` on NCT's `POAK` list (the student OAK_TWR is its only sender, 4U a receiver), the acknowledgement at t=54 crosses the expiration warning at t=114 and the recall at t=64 its removal at t=74, and the deterministic item id (`POAK-1`) is what lets two live passes agree. Solo-training evaluation is unreached by every fixture. And **the replay leg is weaker than it looks**: no fixture declares a runtime generator, so no action log carries a `RecordedAircraftSpawn` (asserted in the sweep, not assumed) and the generator stand-down in replay never has anything to stand down (it is pinned by `ReplayGeneratorStandDownTests` in Yaat.Sim.Tests instead), while recording suppression is invisible in principle because the action log is not part of `ScenarioSnapshotDto` — it is not simulation state. The test's own doc comment carries the current blind-spot list.
