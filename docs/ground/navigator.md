...
**Current-arc flat cap (`_currentSegmentArcMaxKts`, issue #236).** The braking curve treats a corner arc's `MaxSafeSpeedKts` only as a *future* approach limit (a constraint at the arc's entry node), so once the aircraft is *on* a long arc — entered slow, next corner far ahead — the curve permits it to accelerate mid-arc toward taxi max and brake hard on the far side (observed ~19–30 kt through a 72 ft-radius fillet). `ComputeTargetSpeed` therefore also applies a **flat cap** to the *current* segment's own arc speed when it is a `GroundArc`: a corner arc is never flown faster than its safe cornering speed anywhere along it. Recomputed each `BuildSpeedConstraints`, so it round-trips through a snapshot for free; a no-op on straights and on arcs already entered at their safe speed. This is a sim-wide invariant (all corner arcs), not scoped to lane changes.
