...
- Per-segment geometry, compiled into a `PathPrimitive` by `PathPrimitiveBuilder.FromSegmentV2` (`PathPrimitiveBuilder.cs:63`): `PathPrimitiveStraight` for straight edges, `PathPrimitiveBezier` for `GroundArc` fillets (played as their true cubic Bézier).
...
1. Compile the current segment to a primitive (`PathPrimitiveBuilder.FromSegmentV2` — returns `PathPrimitiveStraight` or `PathPrimitiveBezier`).
