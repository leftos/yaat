...
| `FindRunwayRoute` | `(AirportGroundLayout, GroundNode startNode, string runwayId, AircraftCategory)` | Empty-path runway destination route (`TAXIAUTO 28L`). Tries full-length hold-short candidates in threshold order and returns the first route that reaches a real destination runway hold-short without traversing that runway surface. |
...
| Membership arc continuation | 0.5 nm | taxiway-junction arc ("X - Y") used as continuation vs turn; exempt when it is the intended turn from the previous cleared taxiway (see Caveats, issue #236) |
| Runway centerline | ×10 distance | makes on-runway transit ~10× worse |

**Uncleared along-runway travel is a hard constraint, not just a cost.** Beyond the ×10 multiplier, `SearchContext` compiles `AllowedCenterlineNames` — runways named as path waypoints plus any runway whose centerline passes through the start node (post-landing / lined-up starts) — and every search caps consecutive centerline pavement on any *other* runway at `MaxUnclearedCenterlineRunNm` (0.15 nm — enough to CROSS a runway whose crossing is stitched through centerline nodes, e.g. MIA taxiway S over 12/30, never enough to back-taxi one). `AutoRouter.Run` additionally validates the finished path against `HasSameSideCenterlineRun` — a run that enters and exits on the *same side* of the runway is an along-travel shortcut between two exits, not a crossing (OAK: onto 10L at G, off at the adjacent E exit) — and re-runs with that run's centerline moves banned rather than judging it per-expansion (which would poison the `(node, bearing-bucket)` closed set with path-dependent dead ends). There is no soft fallback: a destination unreachable without taxiing along an uncleared runway fails. On a parking/spot destination, `GroundCommandHandler.TryTaxi` then retries with each single named taxiway dropped and keeps the shortest resolution, warning e.g. `[D dropped — no route via D reaches @GA1]` — the S1-OAK-7 `TAXI C D @GA1` case, where D leads away from GA1 and the old resolver satisfied the contradiction by back-taxiing the full length of 10L.
...
`RouteMaterialiser.AnnotateHoldShorts` tags every `RunwayHoldShort` node with `RunwayCrossing` (or `ExplicitHoldShort` when the runway is in the command's `HS` list). Multi-runway bars (`28L/28R`) add one point with the full string. **A runway the controller cleared the aircraft to taxi *along*** (named as a path waypoint, collected into `clearedRunways` from `ctx.WaypointSequence`) is exempt — no hold-short is placed at its boundary, so the aircraft taxis straight onto and along it — **unless the same runway is also in the command's `HS` list**: the explicit hold-short overrides the straight-on entry and arms an `ExplicitHoldShort` bar at every entry-side boundary of that runway on the route (S1-OAK-7 `TAXI F 33 D C B RWY 28R HS 33` silently dropped the HS and rolled onto 15/33 unimpeded before this); a *different* runway the route merely crosses still pairs and annotates as before, and the destination runway (handled first) keeps its terminus stop. The materialiser also annotates `HS` targets that name a **taxiway** (not a runway): it scans the resolved segments and adds one `ExplicitHoldShort` at the first node adjacent to that taxiway. A **located** target (`HoldShortTarget.OnTaxiway`, the `C@J` / `28R@J` command form from issue #358) additionally requires the binding node to sit on the location taxiway — for both the taxiway scan and the runway-bar match (`MatchesExplicitHoldShort` takes the node) — so only the crossing the controller named binds when a looping route meets the target more than once. A **spot** target (`HoldShortTarget.IsSpot`, the `$17` command form from issue #394 — vNAS scenario presets like `TAXI T421 C Z B M1 1L HS $17`) binds the `GroundNodeType.Spot` node with that name when the route passes through it (`IsSpotNode`), never a taxiway edge; its `HoldShortPoint.TargetName` is the canonical `$17` (`HoldShortTarget.MatchKey`) so it can't be mistaken for runway or taxiway `17` anywhere the name is displayed or matched. The route is then **truncated** by `FindTruncationIndex`, discarding trailing pavement. `IsCleared`/`ClearedByAutoCross` start false (downstream owns them).
...
A taxiway `HS` target can be more than a post-hoc annotation — it can also **steer** the route, but only as a fallback. `GroundCommandHandler.HoldShortTaxiwaysToFold` collects the taxiway-named `HS` targets the path doesn't already name; with a **destination** (runway / parking / spot), `TryTaxi`'s `ResolveRoute` first resolves the clearance **as cleared** and keeps that route when it honors every such target (`AsClearedRejectionReason` returns null): the destination is reached (a runway destination has its `DestinationRunway` stop; a parking/spot route is only non-null when reached), the target is bound (`RouteMaterialiser.FindBoundHoldShort` — the same predicate behind the "HS … not applied" warning), **and the route continues on a cleared taxiway past the bound bar** (`ContinuesOnClearedTaxiwayPast`, exact segment names — a `"C - E"` junction arc is the turn *off* the cleared taxiway, not travel along it). Only when the as-cleared route fails one of those does `AugmentPathWithHoldShortTaxiways` fold the targets' taxiways into the waypoint sequence and re-resolve. So SFO `TAXI T7A A A1 1R HS H` (issue #395) stays `T7A A A1` and holds short of H where A crosses it — appending H after A1 sent the route back through A2/M1/A to the 1R bar on H, across 01L/19R twice — while OAK `TAXI D C HS E RWY 28R` still routes `D → C → E`: as cleared, C reaches E but the route ends short of 28R (the runway-destination fallback A* is confined to the cleared taxiways), so the fold applies and E is threaded exactly as if it were named. The continuation test is what separates the two — merely *touching* the target's taxiway at a junction and then reaching the runway over free numbered pavement is not the cleared route. Without a destination the `HS` taxiway is the only direction cue, so it is always folded (`TAXI A HS H` ends at the A/H junction). A **located** target folds its `OnTaxiway` instead of the target (`HS C@J` folds `J` — the aircraft travels J and holds short *of* C, so C must not join the cleared path). Runway `HS` targets are never folded (they have no taxiway nodes); they stay in `ExplicitHoldShorts` and match `RunwayHoldShort` nodes only. Spot targets (`HS $17`) are never folded either — `HoldShortTaxiwaysToFold` skips them: a spot is a point the cleared route already passes through, not a taxiway to steer onto; a route that misses it gets the `HS $17 not applied` warning.

A located target also gives the final-taxiway walk its direction. `SegmentExpander.ResolveTerminusBias` already aimed the walk at a runway `HS` bar on the final taxiway (`TAXI B K HS 10R`); a located target whose `OnTaxiway` **is** the final taxiway extends that: a taxiway target biases toward `FindIntersectionNode(final, target)` (the C∩J crossing), a runway target toward its bar on that taxiway. Without the bias, `TAXI C D J HS C@J` never commits down J at all — the no-destination/no-bias gate stops the route at the D/J transition junction, which is exactly the issue #358 symptom ("taxis C D, holds short J"). The bias bypasses that gate, the walk runs J, and truncation then ends the route one segment past the bound bar.

#### Hold-shorts issued *after* the route is resolved

`HS <target>` and `RES HS <target>` land on an already-resolved, possibly already-cleared route, so they take a different path: `HoldShortAnnotator.PlanExplicitHoldShort` (classify, pure) + `ApplyExplicitHoldShort` (commit). It is the **only** post-hoc writer — `GroundCommandHandler.TryHoldShort` and `TryAddExplicitHoldShorts` both funnel through it, and planning every target before committing any keeps a multi-target `RES HS 28R 09L` atomic.

Because `AnnotateHoldShorts` keeps exactly one `HoldShortPoint` per crossed runway, on the **entry side**, an existing point *is* the authoritative near-side bar — never re-derive it by walking segments for a `RunwayHoldShort` node, which finds the far-side bar once the aircraft is across. Rules:

- An existing point for the target is **re-armed**: `ExplicitHoldShort`, `IsCleared = false`, `ClearedByAutoCross = false`. The clearance is revoked whatever set it (AutoCross via `TaxiRouteAutoCross.Apply`, an earlier `CROSS`, the implicit first-crossing clearance) — the controller's hold-short is the most recent instruction. Re-arming only the `Reason` and leaving `IsCleared` set is what let an aircraft taxi straight across a runway it had just been told to hold short of.
- A bar is **behind** the aircraft only if `IsCleared && segmentIndex < CurrentSegmentIndex`. Testing `IsCleared` first is load-bearing: `TaxiingPhase.BuildResumePhases` bumps `CurrentSegmentIndex` past the bar the aircraft is *stopped at*, so the index alone would read a legitimate hold as "already passed". If every candidate bar is behind, the outcome is `AlreadyEntered` and the command is rejected.
- A `DestinationRunway` point is a **no-op** — re-arming it as a plain `ExplicitHoldShort` would strip the LUAW/CTO departure gate.
- Only when no point exists yet does it walk the remaining segments to add one (runway bar first, then a taxiway edge match; a spot target matches only the named `Spot` node).
- A **located** target (`HS C@J`) filters both steps by node incidence with the location taxiway — existing candidate points off `J` are ignored (so the located command adds/re-arms only the crossing it names), and the segment walk skips nodes not on `J`. With no layout to check incidence against, a located target fails closed (`NotOnRoute`) rather than binding an unverifiable crossing.
...
- **A single through-taxiway that both crosses AND continues past a cleared (non-destination) runway** (`TAXI G HS 28R` where G runs beyond the crossing, no different taxiway named beyond it) hits `FindPairedFarSideRunwayHoldShortIndex` instead: it extends the route through the crossing to the runway's **far-side (exit) hold-short** (matching the same combined `28R/10L` id), mirroring `TaxiingPhase.FindRunwayCrossingExitNode` (see [navigator.md](navigator.md)). Truncating one segment past the near bar instead would strand the aircraft mid-runway between the two hold-short bars once a later `CROSS` clears it.
...
**Final transition into a parking/spot/helipad destination is reach-probed.** The geometric anchor for a parking destination (`ComputeLookaheadPenalty` against the parking node's position) is a coarse binary test — it clears any junction that has *some* toTaxiway edge pointing toward the parking, ignoring whether the aircraft can *admissibly* turn onto that edge from its arrival bearing. At an airport where the last taxiway meets the previous one at more than one junction and the parking hangs off a RAMP spur (SFO `TAXI B K A @F10`, issue #235), both junctions pass the coarse test, the tie breaks on distance-along-the-previous-taxiway, and the cheaper-to-reach junction can leave the aircraft facing *away* from the gate — after which the destination-aware terminus search can't reach it without a U-turn and the append-only extension loops the long way around (`D B K A Q1 B D A RAMP @F10`). So — exactly as the runway case treats the runway hold-short as the de-facto next waypoint — the final transition into a parking/spot/helipad destination replaces the coarse penalty with a **destination-reach probe** (`ProbeParkingReachCost`): each junction candidate is scored by the realized cost of resolving the last taxiway to the destination from it (`LocalSearchToJunction` direct-to-dest, else `SelectBestParkingStop` walk+extend, capped at `ProbeStopCandidateCap = 4` stops), returning `TailUnresolvablePenaltyNm` when the destination is not reachable staying on the taxiway from that junction. The junction from which the gate is admissibly reachable wins (SFO node 46: probe cost ~0.04 nm vs the wrong junction's ~1.4 nm). Bounded to the top level (`enableLookahead`) so it never fires inside a `ProbeTailCost` recursion, and only for the final transition (no meaningful tail) — runway, node, and end-of-taxiway destinations keep the runway anchor / geometric heuristic.

**Intermediate transitions are destination-reach-scored.** The tail probe scores a junction by the cost of *resolving the remaining tail tokens* from it — but "resolved the tail tokens" is not "reached the destination". At a collapsed multi-way interchange a junction can land the aircraft on the next taxiway facing the wrong way, where the onward turn toward the destination is an inadmissible U-turn: the tail's destination-aware terminus walk then dead-ends *short* of the destination and the probe returns a deceptively cheap cost, so that junction out-scores the one from which the destination is genuinely reachable. `ProbeTailCost` therefore adds the true remaining reach cost — a bounded `AutoRouter` from the tail terminus to the destination node (capped at `MaxDetourExpansions`, inheriting the tail's arrival bearing) — whenever the tail terminates short of a concrete-node destination (`Parking`/`Spot`/`Helipad`/`Node`). It only *adds* the real remaining distance, so it can only re-rank a junction that pretends to reach the destination below one that actually does — it never distorts an already-correct pick. (e.g. OAK `TAXI C D @NEW1`: naming C makes G→C an intermediate transition, unlike the destination-anchored final C→D of `TAXI G D @NEW1`; without the reach term the wrong-way-onto-C junction — whose tail dead-ends where turning onto D is a ~185° U-turn — out-scored the correct junction, producing a 91-segment loop across runway 28R before reaching NEW1.)

**Single named taxiway to a ramp-hung gate joins the implied ramp connector.** The reach-probe above only fires at a *junction transition* (≥2 named taxiways). A single-named-taxiway clearance to a parking/spot/helipad that hangs off a numbered ramp connector (SFO `TAXI B @F1`: F1 is on a RAMP reached from B via connector T9) has no transition, so `ExpandLastWaypoint` relies on `SelectBestParkingStop`. Two failures compounded there: (1) its stop-node candidates were the taxiway nodes *nearest the gate by straight line*, but a ramp connector can loop away from the gate before curving back — so the B/T9 junction was ~11th-nearest and crowded out by ten dead-end B nodes, leaving no reachable stop → the greedy terminus walk went the wrong way and the append-only extension looped cross-field (`B A L C Z B1 Q A T9 RAMP`, 232 segments). (2) even once the connector junction is a candidate, the unrestricted extension takes the geometrically-shorter path that threads *uncleared* letter taxiways (`B Q A T9 RAMP`). Fixed on both fronts: `SelectBestParkingStop` seeds its candidate pool with **one junction per distinct numbered-connector / RAMP name** off the cleared taxiway (`RampConnectorNames`, guaranteed in regardless of the straight-line cap), and `ExtendToDestination` **confines the extension to the cleared taxiways + numbered connectors + RAMP** (hard-excluding uncleared letter taxiways, unconstrained fallback when that finds no route — mirroring the runway-destination last-resort A*). The join stays on the cleared taxiway until a numbered ramp connector branches off (`B T9 RAMP`, ~1.1 nm) instead of threading the movement area.
...
**Curated connectors are threaded as named tokens and preferred over a blind detour.** The blind bridge above minimizes cost with no notion of *the* real connector, so it can pick a numbered ramp lane (which escapes the letter penalty) over the physically-correct letter connector. For pairs where a specific taxiway is the real-world bridge, curate it per-airport in the ARTCC sidecar as an `ImplicitConnectorEntry { connector, between }` (`src/Yaat.Sim/Data/ARTCCs/<id>/Airports/<icao>.json`) — SFO `L`/`F` bridge via `LF`, and `A`/`B1` (B1 is a stub off `Q`) bridge via `Q`. It is authorized only when the controller clears the two `between` taxiways adjacent. `SegmentExpander.Run` resolves the clearance both as-cleared and with each applicable connector threaded in (`BuildConnectorVariants`), then keeps the route that honors the clearance with the **fewest mandatory blind-detour insertions** (`IsBetterRoute`; `TaxiRoute.MandatoryConnectorCount`, distance breaks ties). So `TAXI A B1` on a map where A/B1 don't meet routes `A Q B1` (0 insertions) instead of a blind bypass that silently drops A and B1 — the bug where UAL194 at spot T9 got `T9 B K A F` because the blind detour preferred the numbered `T9` over the letter `Q`. The curated connector is inert when the pair connects directly (the as-cleared route then wins on distance).
...
**5. Final named token — junction stop or terminus walk.** A **bare final taxiway** — no destination of any kind, no `>`/`<` turn hint on it, no hold-short bias, reached by transitioning from a preceding *taxiway* (not a runway token) — makes the route **hold at the transition junction** (`TerminateAtTransitionJunction`): the clearance gave no onward direction, so the aircraft stops where it meets the final taxiway (OAK `TAXI C D` holds at C/D, whether D extends one way or both ways from it) and a `holding at the C/D intersection — route ends at D, no destination given` advisory is copied onto the route's warnings for the TAXI echo. A preceding **runway** token is exempt — stopping at the runway/taxiway "junction" would end the route on the runway surface with no crossing bar (MIA `TAXI M1 RWY08R/26L L1`), so the final taxiway is walked clear of the runway as before. Everything else falls to `WalkToNaturalTerminus` (`:1351`): walks forward along the taxiway one admissible edge at a time until none remain. **Requirement ①:** an exact **single-name** edge ranks *strictly above* a membership-taxiway junction arc (`IsMembershipTaxiwayJunctionArc: true` — multi-name and not a runway-crossing arc). A multi-name junction arc is a turn *off* the taxiway, not a continuation; single-name wins regardless of cost, cost only breaks ties within a tier. Runway-crossing arcs (`IsRunwayJunction`, e.g. `H - RWY01L/19R`) *continue* the taxiway across a runway and are not excluded. (This greedy one-step walk is a known gap vs. multi-candidate discipline — see Caveats.)
...
- `startOverride` lets callers (detour, extension) seed the search with a prior `PartialRoute` so admissibility fires on the first expanded edge against the inherited heading — without it the first edge could U-turn. For a **cold start** (no `startOverride`), `GeometricAdmissibility.IsAdmissible` still admits the first edge in *any* direction (its `LastEdge is null` bypass) and the turn-budget term is skipped, so `RouteCostFunction.IncrementalCost` applies a soft **first-hop heading bias** (`FirstHopHeadingBiasNmPerDeg`, turn-budget scale) toward `SearchContext.StartHeadingTrue` when the heading is known and no `>`/`<` turn hint governs the first taxiway — so a taxi doesn't begin with an unmotivated turn away from where the aircraft is facing. It is finite, g-score-only (never in the heuristic), and applied equally to every first-edge candidate, so a genuinely-required reversal (the only route goes backward) still wins.
...
- **Look-ahead is bounded to one level.** Probes run with `enableLookahead: false`, which both bounds recursion and suppresses the detour inside probes. A pathological sequence needing two levels of look-ahead to disambiguate will not get it. This is intentional (recursion bound) but is a real limit. (The specific failure where a one-level probe's tail dead-ends *short of a concrete-node destination* is caught separately by `ProbeTailCost`'s destination-reach term — see "Intermediate transitions are destination-reach-scored" above.)
