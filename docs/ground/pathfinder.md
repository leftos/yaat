...
**5. `WalkToNaturalTerminus`** (`:882`, the final named token). Walks forward along the taxiway one admissible edge at a time until none remain. This is where **requirement ①** lives: an exact **single-name** edge ranks *strictly above* a junction arc that matches the taxiway only by membership (`edge is GroundArc { TaxiwayNames.Length: >= 2 }`). A multi-name junction arc is a turn *off* the taxiway, not a continuation; single-name wins regardless of cost, cost only breaks ties within a tier. (This greedy one-step walk is a known V2 gap vs. its own multi-candidate discipline — see Caveats.)

**6. Variant resolution** (`TryVariantExtension`, `:1144`). When the destination is a runway and the walked route hasn't already reached a hold-short for it (`RouteReachesRunwayHoldShort`), the expander looks for **numbered variants** of the last named taxiway whose hold-short serves the runway (`B` → `B1`). Exactly one → auto-extend onto it (`ExtendToVariant`, via `LocalSearchToJunction` then `AutoRouter` fallback). More than one → `TransitionAmbiguous` failure naming the candidates. Zero variants → try same-name hold-shorts, else stop at natural terminus.
...
- A* with a `PriorityQueue<PartialRoute, double>`, a per-node `bestGScore` map for duplicate pruning, and the `GeometricAdmissibility` hard gate on every edge. `IncrementalCost` (the same cost function) prices each edge. Heuristic = straight-line nm. Cap: **`MaxExpansions = 200_000`** → `SearchExhausted` (raised from the design's 50k after SFO cross-field routes legitimately explored 100k+).
...
- **Membership junction arcs ("X - Y").** `GroundArc.MatchesTaxiway` matches if the queried name is *any* of the arc's `TaxiwayNames` (`AirportGroundLayout.cs:312`). Fillet V2 retains junction arcs named e.g. `C1 - B`, `A - RAMP`, `A - Q1`. A bare-taxiway walk continuing `B` therefore *sees* a `C1 - B` arc as a valid `B` step. **Requirement ① (single-name beats membership-only arc) is implemented in `WalkToNaturalTerminus` but NOT yet in `LocalSearchToJunction`** (the multi-segment path) — `LocalSearchToJunction` still admits purely on `MatchesTaxiway` membership + g-score (`SegmentExpander.cs:724`). Repro of the gap: `IssueFllDal880TaxiBacktrackBTests`, `Issue166CrossShortcutsGrassTests` (both pass on Legacy fillets, fail on V2). This is the active frontier — see `docs/plans/ground-graph-v2.md:121`.
