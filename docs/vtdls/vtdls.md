...
![A vTDLS display](img/tdls.png)

*A vTDLS display*

> ℹ️ vTDLS is accessed at <https://tdls.virtualnas.net/>.

> ℹ️ This guide is intended for controllers. If you are a Facility Engineer seeking TDLS configuration documentation, please see the [TDLS configuration section](../vnas-data-admin/facilities.md#tdls-configuration) of the vNAS Data Admin website documentation.
...
![A TDLS window](img/tdls-empty.png)

*A TDLS window*

### Header

The TDLS header contains system information, most of which is not simulated in vTDLS. The active facility indicator on the top left displays the current TDLS facility ID (**bosa** in [Figure 6](#fig-tdls-window)). Clicking this indicator opens the [Facility Menu](#switching-facilities).
...
> ℹ️ If a pilot pre-files prior to connecting to the network, their flight plan is displayed in the DCL list, even if the pilot is not yet connected.
...
The TDLS footer displays system status information. Although most information is not simulated in vTDLS, there is a system clock on the right that displays the current Zulu time. When enabled, the footer also contains the [Ops Config](#operational-configurations) menu.
...
Operational Configurations (Ops Configs) are used by some facilities to associate various SID and transition defaults with different configurations. For example, a facility may choose to have a West Configuration that applies different defaults to a SID than would be applied in an East Configuration (i.e. different departure frequencies and/or different maintain altitudes).

### Selecting an Operational Configuration

![Operational Configuration Menu](img/opsconfig.png)

*Operational Configuration Menu*

To change the active Operational Configuration, click the **Ops Config** menu in the Footer, select the desired configuration, and click **Save**.
...
![A TDLS flight plan](img/flight-plan.png)

*A TDLS flight plan*

[Figure 8](#fig-flight-plan) labels the flight plan fields as follows:
...
> ℹ️ Only values predefined by the Facility Engineer may be selected.
...
> ⚠️ Flight plans cannot be amended through TDLS, even if the selected SID differs from the flight plan. Amendments must be done through a radar client.
...
> ℹ️ Facility Engineers are able to specify a variable in contact information values that is automatically substituted by your active frequency. If the Facility Engineer utilizes this feature, your active frequency is automatically listed in the contact information values.
...
![A PDC received in a pilot's client](img/pilot-pdc.png)
...
> ℹ️ PDCs can be sent before the aircraft connects to the network. In this situation, the pilot receives the PDC as soon as they connect.

> ℹ️ If a pilot disconnects and then reconnects before departure, a new copy of their PDC is automatically sent to them.

> ⚠️ In order to send PDCs, your controlling session must be activated on your primary controlling client.

> ⚠️ A PDC cannot be amended once it has been sent. Any amendments to the clearance must be done over voice (or text).
...
![Switching facilities](img/facility-menu.png)
