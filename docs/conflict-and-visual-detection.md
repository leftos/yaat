...
| `Crossing` | Paths cross. `ComputeClosingLimit` clears a mover laterally in two ways: the room along its current direction, and — against a **parked/held** obstacle only — the room along its remaining taxi route (`RouteLateralClearanceFt`: min point-to-segment distance over the next `distFt` + obstacle length of route, arcs read as chords). The route test exists because a mover braked mid-turn points between two alley lanes; without it an aircraft stopped inside a parked neighbour's stop ring (SFO alley lanes ~140 ft apart, B738 beside E75L needs ~131 ft) could never leave. Against a **moving** obstacle the current-direction lateral test runs only when the two are on near-parallel tracks with room to pass — `HasParallelTrackLateralRoom`: tracks within `ParallelTrackToleranceDeg` (20°) of parallel or anti-parallel and each aircraft's offset from the other's track above `RequiredLateralClearanceFt`; the same helper makes `ResolveHeadOn` (the 300 ft, ≥150° head-on hold, which has no lateral test of its own) stand down. Without it SFO's parallel taxiways A and B (~160 ft apart, passes bottoming out at 238 ft inside the ~255 ft trail ring and the 300 ft head-on ring) crawled and stopped opposite-direction traffic (GC 28/01 bundle, 2026-09-22). A crossing more than 20° off parallel keeps the straight-line stop/trail rule |
