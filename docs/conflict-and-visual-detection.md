...
| `Stationary` | Both parked or held — no resolution runs (a genuinely stopped pair needs none). "Stationary" requires being actually at rest: a stationary-*named* phase that is rolling (a `LineUpPhase` aircraft driving onto the runway, a held aircraft still decelerating) classifies as a mover instead — the phase-name check is gated on `IsAtRest`: `GroundSpeed < HeldStationarySpeedKts` **and** not commanding forward speed (`TargetSpeed > 0` makes it a mover however slowly it rolls — the 2 kt lineup speed sits below the held threshold, so the speed gate alone let a lining-up aircraft creep through the LUAW occupant ahead one sub-tick in four) (#407, #409) |
| `Pushback` | A pushing aircraft is involved. A pusher hard-stops for a moving/taxiing/pushing aircraft in its rear arc (within `PushbackBufferFt`, less than 90° off the push direction) **unless it has wingspan lateral clearance along the push direction** — `HasWingspanLateralClearance(pusher, pushDirection, other)`: half-spans plus `WingtipBufferFt` measured perpendicular to the tail-first travel, so two tugs pushing side by side into the outer lanes of an SFO alley (5A/5B, 6A/6B — the alleys are three lanes ~70 ft apart, and only the outer pair at ~140 ft fits two B738-class aircraft; SOP 3-5.c.i uses that pair simultaneously below B757) pass wingtip to wingtip while a pusher aimed at the other's fuselage still stops. A pinned pusher carries `AutoYieldTarget` so the datablock shows who it waits for. A **genuinely parked/held** neighbor (`IsParkedOrHeld`) is judged by the **outline rule** (`TugMoveLimit` / `TugMoveFoulsParkedAt`, for any `PushbackPhase` mover, push or pull): the mover's `GroundOutline` (fuselage with a 30 ft tug lead on a pull, wing, tailplane) is swept along `PushbackPhase.RemainingPath` — the rest of the move plus the queued moves up to the first reversal (`TugRunContinuation`) — against the neighbour's; it passes while the clearance never drops below `min(WingtipBufferFt, clearance at the move's start) − 0.5 ft` (never below the 0.5 ft slack; the floor is anchored to the move's start so it cannot ratchet the mover into contact), and otherwise, once the interpolated floor crossing lies within the stopping margin (the braking distance from `PushbackSpeed` at `CategoryPerformance.TugDecelRate` plus one detector interval, 23 ft for a jet), takes a braking limit (`outline stop`, `TugMoveBrakingLimitKts`: `√(2·a·d)` down to zero inside the last interval, yield target shown from the first foot of braking, i.e. any limit under the speed the move is commanding — `PushbackPhase.CommandedSpeedKts`, the creep speed on a creep move's last stretch) that the physics clamp follows down, so a gate pushback clears an aircraft parked at the adjacent gate and a tow brakes at the towbar rate to a stop short of where the two outlines would meet. An aircraft under an active `PushbackPhase` is never `Stationary`, so a stopped pull stays in resolution (physics nulls `TargetSpeed` at a zero limit). A tow whose outline already touches a parked neighbour's at its start is refused by the command handler instead (see [ground/pushback.md](ground/pushback.md)). The two sides are evaluated independently (`ResolvePushbackSide` for (a,b) and (b,a)) — except when they would stop each other: a mover ahead of the push with no lateral room, which the pusher would yield to, that also has the pusher within 90° of its own nose, which it would trail. `TryResolveGiveWayToPushback` breaks that deadlock in favour of the push (the taxiing aircraft holds with reason `give way to pushback` and `AutoYieldTarget` = the pusher; the pusher continues when its remaining push leg — from its position to the leg's end, `PushbackPhase.TryGetPushLegEnd` — clears the holder by half-spans plus the wingtip buffer (the tail is leaving the holder's lane), and otherwise runs the graduated closing limit against it as a stationary obstacle; a push aimed at the holder inside collision distance is a genuine wedge left to the controller). Priority is granted only to leg 1 of a push off a stand (`PushbackPhase.HasRampPriority`: the push-off once its tail has left the stand, plus the moves flown through from it before the first reversal): a push just cleared while traffic rolls by holds instead, and a tow that has reversed at least once — the second leg of a `PUSHM`, the push half of a three-point turn — takes the ordinary pusher hard stop and shows the taxiing aircraft as its own yield target. It is never granted against an aircraft on or crossing a runway (AIM 4-3-21.b: the exiting aircraft clears even into a ramp area) or against a live-traffic shadow (which cannot be held — the pusher keeps its hard stop). a pushback in progress has ramp priority — the tug crew cannot see behind the tail and a push cannot pause mid-alley without blocking both lanes — and 7110.65 §3-7-2 NOTE 2 leaves nonmovement-area separation to the pilots and ramp, so this is the ramp convention, not an ATC instruction |
...
| `Crossing` | Paths cross. `ComputeClosingLimit` clears a mover laterally in two ways: the room along its current direction, and — against a **parked/held** obstacle only — the room along its remaining taxi route (`RouteLateralClearanceFt`: min point-to-segment distance over the next `distFt` + obstacle length of route, arcs read as chords). The route test exists because a mover braked mid-turn points between two alley lanes; without it an aircraft stopped inside a parked neighbour's stop ring (SFO alley lanes ~140 ft apart, B738 beside E75L needs ~131 ft) could never leave. Against a **moving** obstacle the current-direction lateral test runs only when the two are on near-parallel tracks with room to pass — `HasParallelTrackLateralRoom`: tracks within `ParallelTrackToleranceDeg` (20°) of parallel or anti-parallel and each aircraft's offset from the other's track above `RequiredLateralClearanceFt`; the same helper makes `ResolveHeadOn` (the 300 ft, ≥150° head-on hold, which has no lateral test of its own) stand down. 7110.65 has no taxiway-separation paragraph (its §3-1-1 NOTE, AIM 4-3-18.b and AIM 2-3-4.b.1 put wingtip avoidance on the pilot), so `WingtipBufferFt` and the 20° tolerance are detector-frame judgement calls; the helper also looks ahead: each aircraft is projected along the bearing of its current taxi-route segment (its travel direction when it has no route, or when the segment runs more than the tolerance off it — a tow, a finished route), so nose wander inside a lane does not spend the margin (a lane that curves within the horizon is still read as its current segment's chord, and just past a node that chord can be a short fillet piece up to the tolerance off the lane — both err towards losing the bypass, never granting it wrongly), for the longer of the pair's stopping times (ground speed ÷ `CategoryPerformance.TaxiDecelRate`, capped at `ParallelTrackLookAheadCapSeconds` = 12 s), and the bypass holds only while both signed offsets clear the requirement at both ends and neither changes sign — so two tracks converging inside the tolerance, or crossing each other's line within the horizon, lose it while there is still room to brake (the old instantaneous test let two pistons on twin lanes close through a 61 ft requirement with ~95 ft needed to stop). Without it SFO's parallel taxiways A and B (~160 ft apart, passes bottoming out at 238 ft inside the ~255 ft trail ring and the 300 ft head-on ring) crawled and stopped opposite-direction traffic (GC 28/01 bundle, 2026-09-22). A crossing more than 20° off parallel keeps the straight-line stop/trail rule |
