...
```
            ...
            └──────────────────────────┬────────────────────────────────┘
                                       ...
                                       ▼
   ┌─────────────────────┐   ┌─────────────────────────────┐   ┌─────────────────────────┐
   │ HeadlessRoom    │   │ TickProcessor.ProcessPost-  │   │ Recording (v4 archive)  │
...
| [07-soak-runner.md](07-soak-runner.md) | HeadlessRoom, CLI, episode model, artifacts, throughput plan |
...
| [ ] | CA2 | Tower brain v1 + coordination + precedence | **v1 slice phases 4–5** ([12](12-milestone-v1-scope.md)); waits on the tick-path programme (`MAIN.md` Current focus, steer 2026-09-02) and then the per-frequency radio model ([11](11-radio-model.md)). [05](05-tower-brain.md), coordination bus, auto-accept/pointout skips, auto-CTO suppression, `IAiStaffing`. First full gate-to-gate loop |
| [ ] | H1 | Smallest useful soak | **v1 slice phases 6–7** ([12](12-milestone-v1-scope.md)); after CA2. Detector framework (TDD) + Tier A + stuck-aircraft + ai-rejection, findings.jsonl + summary, per-finding snapshots + bookmarks. Target: GC+LC, one scenario, one seed |
...
- **`RoomEngineTestHarness`** — the wiring blueprint for `HeadlessRoom` (stays test-only).
...
- The `FrequencyState` partition per answering position (an AI Ground's answer clearing the gate held for a pilot waiting on tower) is RADIO-02 in [11-radio-model.md](./11-radio-model.md) — not tracked here
...
- `HasLeftStudentFrequency` (`ContactCommandHandler.Route`) being one bool for a multi-position world is RADIO-07/08 in [11-radio-model.md](./11-radio-model.md) — not tracked here
...
- AI `CT` readback landing on the RPO lane in instructor rooms is RADIO-07 in [11-radio-model.md](./11-radio-model.md) — not tracked here
...
- [ ] Generic runway usability by pavement length vs aircraft category belongs in `RunwayUsabilityGate` too — filter runway ends by pavement length against the aircraft category before ranking by wind (§3-5-1.b.1; aviation review 2026-09-01; today the gate is tailwind-only)
