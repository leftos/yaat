...
- [ ] **Retire the post-hoc node-merge (cross-junction):** `FilletGraphNormalizer.MergeCoincidentNodesDefensive` still merges cross-junction coincident tangent cuts; move those into the plan (global cut-coalesce) and delete the post-hoc merge. `V2_NoCoincidentIntersectionNodes` guard already exists
