...
- [ ] **Deferred to follow-up session**: CUDA runtime download. LLamaSharp.Backend.Cuda12.Windows's native DLLs have runtime dependencies on CUDA toolkit runtime libraries (`cudart64_12.dll` / `cublas64_12.dll` / `cublasLt64_12.dll`) that live in the CUDA Toolkit's `bin` directory — not in the NuGet package. End users typically have the NVIDIA display driver (providing `nvcuda.dll`) but NOT the toolkit. A follow-up can either auto-detect a matching CUDA install on the user's system or offer instructions for a lightweight toolkit install. Vulkan-on-NVIDIA already gives ~70–80% of native CUDA speed so most users won't need it.
- [ ] **Deferred to follow-up session**: Whisper.net GPU runtime download. Whisper.net's native loading convention differs (`build/win-x64/` instead of `runtimes/win-x64/native/`) and their managed loader does not expose a `WithSearchDirectory` equivalent. Needs investigation into whether we can use a different Whisper.net loading hook or a file-copy strategy. Whisper on CPU is near real-time for `base.en` so the GPU story is lower priority than LLM GPU.
