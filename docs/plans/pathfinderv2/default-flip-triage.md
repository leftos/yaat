...
- **Cluster C** — hold-short annotation — **2/3 done**: ✅ entry/exit pairing in `RouteMaterialiser.AnnotateHoldShorts` (`4d70500b`, ported from `HoldShortAnnotator` + start-node pre-seed) fixes `OakCross28R.RerouteFrom28R` (exit node 186 no longer a crossing) and `OakExplicitHsAutoCross.ExplicitHs28R` (one entry-side ExplicitHoldShort). ⏳ **`OakCrossThenHold.AfterRes`** deferred → merged into the **#350-terminus + parking-direction** effort (below): the cross-through route turns onto C via the directional corner arc (commits one way, reports `CurrentTaxiway="C - G"`) and holds 64 ft from the sharp #350; a follow-up `TAXI C @JSX1` (parking, east) then fails because the final-taxiway walk has no parking-direction bias. Needs the route to terminate at the neutral #350 AND parking/spot direction routing — overlaps Cluster B remnants.
...
- **Cluster B remnants** — **mostly done** via best-parking-stop (`c6eea556`, V2 port of v1 `SelectBestStopNode`: evaluate the taxiway nodes nearest the parking as stop points, route to each + extend, pick lowest walk+extension): ✅ `SKW3078` (`TAXI E A @B10` via A1), ✅ `IssueFllDal880` (U-turn), ✅ `OakPostLandingReversals.N436MS` (`TAXI C @JSX1`). ⏳ `OakPostLandingReversals.N9225L` (`TAXI D @NEW1`) still holds short of 28R/10L — North-Field parking is across the runway, a distinct auto-cross-on-parking-route issue, not best-stop. `BundleReplay` (same SKW3078 route) likely cleared — reverify.
