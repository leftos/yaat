...
#### Test coverage (Cursor + Codex coverage gaps) — DONE (Phase 5)

- [x] **Strengthened `Issue165_V2_SkwRoute_ResolvesWithoutFailure`** — now asserts V2 U-turn count ≤ V1 for the same SKW3404 waypoint sequence (`A E B B3 A B1 Z S`) over the same graph, via `PathfinderComparison.CompareExplicit`.
- [x] **Explicit-path coverage in `PathfinderComparison`** — new `CompareExplicit` (the explicit-path analogue of `Compare`, same V2-U-turns-≤-V1 metric) + `ExplicitPathComparisonTests` over `TaxiRouteCatalog`-style named instructions (OAK `W`→30, `K W`→28L; SFO `B M1`→1L).
- [x] **V1-static call sites labelled.** `TaxiPathfinderTests` carries a class-level V1-only-regression-pin label (drives the V1 static API directly, asserts V1 route shapes, deleted with V1 at the flip); `Skw3404_Seg12` already routes via `TaxiPathfinderRouter.Current`. The broader sweep of the remaining `TaxiPathfinder.<static>` references across grid/issue/helper files is intrinsically the **Phase-7 V1-deletion** job (deleting V1 forces each to migrate or be deleted).
- [x] **Behaviour tests for the Codex HIGH findings.** New: `DestinationRunway` reason + `RWY <id>` summary (`RouteMaterialiserTests.RunwayDestination_ToSummary_IncludesRwyDesignator`). Pre-existing: reciprocal HS matching (`ExplicitHoldShort_ConfiguredInContext_TaggedCorrectly`), full-length lineup requested end (`FindFullLengthLineupHoldShort_MultipleHoldShorts_PicksBarNearestRequestedThreshold`), detour authorized-taxiway policy (`SegmentExpanderTests` detour tests + `JunctionContinuationTests`), state-aware pruning (`StateAwarePruningTests` + necessity sweep), non-jet preview category (`GroundViewModel.CategoryFor` threaded through all preview sites).
...
- [x] **Latency budget guard (Phase 5).** `PathfinderLatencyBudgetTests` times V1 vs V2 `FindRoute` over OAK parking→runway pairs in microseconds, reports the median ratio, warns above the 2× design target, fails above a 5× ceiling. `PathfinderGrid`-gated. Local median ≈ 1.3× V1.
- [x] **`MaxDetourExpansions` is enforced** — `RunBoundedDetour` passes it as `AutoRouter.Run`'s `maxExpansions` (`SegmentExpander.cs:2018`). The Codex LOW finding was stale.
- [x] **Priority tie-breaker (Phase 5)** — the code is correct (it prefers DEEPER routes on equal f-scores, the standard A\* drive-to-goal tie-break, fewer expansions); the *comment* was wrong and is now fixed (`AutoRouter.cs`).
- [x] **Client preview category mismatch (Phase 5)** — all `GroundViewModel` preview paths now thread the real aircraft category via `GroundViewModel.CategoryFor` (the two node-id helpers take a category; the two direct router calls and the context-menu route options derive it from the routed aircraft; the route-drawing preview uses the drawn aircraft).
- [ ] **Split `SegmentExpander.cs`** (1095 lines as of `46e13d5a`, now larger). Self-contained sub-pipelines to extract: variant resolution (`TryVariantExtension` / `ExtendToVariant`), detour fallback (`TryDetour` / `RunBoundedDetour`), node-ref routing (`RouteToNodeRef` / `RouteFromNodeRefToTaxiway`). *(Pure refactor — Phase 7.)*
- [ ] **Reconcile `DirectionReversalCostNm` asymmetry.** Currently applied in `SegmentExpander.LocalSearchToJunction` only, omitted from `AutoRouter.IncrementalCost` to preserve A\* heuristic admissibility (design decision §7). Cursor flagged that auto-routes will be more zig-zaggy than explicit ones. Either accept the asymmetry and pin it with a test, or move the reversal cost into the heuristic itself so admissibility still holds. *(Phase 7 — no failing repro; the auto-route U-turn guards in the stress grids currently hold.)*

### Probably-defunct after the fillet rewrite — re-evaluate before doing

- [x] **`IsNoOpEdge` guard in V2 — kept (Phase 5).** Still load-bearing while the Legacy fillet generator is the runtime default (it emits the zero-distance pairs, and the V2 pathfinder router can run on either fillet graph). Fillet V2 removes them at the source (guarded by `V2_EdgeSplit_NoZeroDistanceEdges`), so once the joint flip makes fillet V2 the only graph this guard becomes pure defence-in-depth — re-evaluate keeping vs removing it **at the flip** (documented in the `NoOpEdgeThresholdNm` summary).
- [ ] **B1's tangent-on-anchor merge pass in `FilletArcGenerator`** (commit `fd7e35fe`, `BuildMergeMap` second pass). Moot — it lives in the Legacy `FilletArcGenerator`, which is **deleted with V1 at the flip** (Workstream 1); no separate action needed.
