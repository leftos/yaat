...
| A8-1 | Callsign pronunciation | `IcaoToSpoken` never appends **"heavy"/"super"** — affects *every* heavy/super transmission | "American Twenty Two" → "American Twenty Two heavy" | 4-2-4.a.5 | HIGH | TODO |
| A1-2 | Initial contact | `"with information Alpha"` ATIS letter is **hardcoded in ~6 sites** | drive from the live scenario ATIS letter; suppress when no ATIS exists (keep the colloquial "with information" wording) | 4-2-3.a.3, 4-1-13 | HIGH | TODO |
| A5-1 | Traffic / visual | `BuildLostSightOfField` / `BuildLostSightOfTraffic` say **"negative contact"** (a radio-contact term, not loss of visual) | "lost sight of the field/traffic" / "no joy" | 5-5-8/10/11 | HIGH | TODO |
