...
- [x] Display-field audit vs vatsim-server-rs / swim-runner-rs (2026-08-31): ranked candidates recorded in yaat-server `08-remaining-work.md` §6 — top picks: TAIS `lld` → `GlobalLeaderDirection`, TAIS `assignedBeaconCode` → FP beacon, cps coarse-vs-specific precedence fix, SFDPS `pointout` element; none wired yet (user to pick)
