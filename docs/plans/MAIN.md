...
- [ ] A Sim-side replay (client playback, `Yaat.Sim.Tests`) never fills `SimScenarioState.AtcPositions` — only the server's `InitializeTrackPositions` does — so `TickDeferredAutoTrack` and a spawn-time `autoTrackConditions` have no roster to claim with there and the departure replays unowned until the next snapshot restore (finding 2026-09-06, step-4 C aviation review; the server's rewind reloads through the full pipeline and is unaffected). Persist the resolved roster in the recording beside `StudentPositionState`, or resolve `AtcEntries` against the bundled ARTCC config in `ReplayDriver`
- [ ] `SessionPersistenceService.RestoreRoomFromArchiveAsync` mutates a room already registered in `TrainingRoomManager` without the room's tick gate (finding 2026-09-06, step-4 A review): the tick loop can call `AdvanceLiveSecond` on it mid-restore, racing `RestoreFromSnapshot`, `PositionSelections.Restore`, `Attendance.Replace` and the restore's own `SyncAttendance` — wrap the body in `room.GuardAsync(...)`
