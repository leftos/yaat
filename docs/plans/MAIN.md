...
- [ ] **Reloading a scenario or leaving a room does not reset vStrips state** (reported 2026-09-07; #424): after a reload the strip bays should return to the scenario's starting strips, keeping **only** the separators the user placed; today the previous session's strip state persists, and after Leave Room the popped-out Strips view still prints and moves the old room's strips until the next join. Check `ScenarioLifecycleService` unload/reload vs `FlightStripState` and the Strips client's handling of the reload broadcast, and `MainViewModel.Rooms.LeaveRoomAsync`, which never clears `VStripsViewModel`; related to the step-4 item "FlightStripState / TdlsState into ScenarioSnapshotDto"
