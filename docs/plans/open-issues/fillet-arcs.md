...
### Coincident node merge pass with bezier adjustment (implemented)
Complex intersections (5+ edges) and adjacent filleted intersections produce tangent-point nodes at the same position (within 5ft). Global post-fillet pass `MergeCoincidentNodes` in `FilletArcGenerator.Apply` iteratively merges them (up to 5 passes for transitive chains), adjusting bezier control points by translating them with the victim→survivor displacement to preserve tangent handle vectors. After all merge passes, `RecomputeDistances` recomputes cached `DistanceNm` for all edges and `MinRadiusOfCurvatureFt`/`DistanceNm` for all arcs. Also removes self-loop edges/arcs, duplicate edges, and redundant arcs. Per-intersection merge in `FilletNode` between Phase B and Phase C handles within-intersection coincident tangent points.
