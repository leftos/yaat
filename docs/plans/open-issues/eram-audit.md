...
- [x] Target `SymbolType` engine (root cause C): ident, primary(standby), VFR(1200), MCI/uncorrelated-beacon, reduced-sep from existing transponder data. *(#250)*
- [x] ERAM track `Status` (root cause D): coast-on-target-loss (CST), and a frozen field for `QH`. *(#251)*
- [x] Publish `EramTargetHistories` (root cause F) from `AircraftState.PositionHistory`. *(#252)*
...
- [x] `QR` → controller-entered reported altitude (currently mis-wired to route display). *(#254)*
...
- [x] `QS` HSF fourth-line fields (heading/speed/free-text) instead of STARS Scratchpad1. *(#257: `DispatchQs` rewritten to the Table-5 forms — `QS <heading>` (magnetic 001–360, zero-padded), `QS /<speed>` (knots or Mach `/M82`), `` QS `<text> `` (free text), `QS */`/`/*`/`*` delete heading/speed/all, bare `QS <FLID>` = client-side display toggle (accepted no-op). New `AircraftEramState.AssignedHeading`/`AssignedSpeed`/`FreeText` (manual annotation, snapshotted); `ToEramTrack` emits them into `EramTrackDto` Keys 19/20/18 (free text on the Scratchpad slot), decoupled from STARS Scratchpad1. Re-send via a new `AircraftChangeTracker.UpdateEramHsfState` flip fed into `eramTrackChanged` (mirrors the conflict/accepted-indicator triggers — so a QS edit reaches CRC even on a paused/stationary track, not just via the per-tick EramTarget flip). Aviation- + code-reviewed (fingerprint gap, empty-backtick→FORMAT, speed bounds all fixed).)*
- [x] `QP` point-out: fix inverted `ClearEramPointout` caller-auth; typed `QP A <sector> <FLID>`; `QP <FLID>` minimize. *(#258)*
- [x] **C1: FLID resolution by CID / beacon code** in `FindAircraft`. *(#259)*
- [x] EramTargetDto.GroundSpeed population; leader-direction numeric parse. *(#260)*

**Tier 3 — niche / low incidence:**
- [x] **#261** niche display grab-bag. Done: DRI halos `QP J`/`QP T` (new `AircraftEramState.DriHaloType`, per-track manual annotation → `HaloType` wire field, change-tracker re-send); `LA`/`LB`/`LC` readouts (distance + magnetic/true bearing + flying time; `LC` speed-to-cross-fix uses wall-clock at issuance, no persistent sim clock); Non-RVSM box from FAA suffix (7110.65 TBL 2-3-10: `/L`/`/Z`/`/W`/`/H`/`/O`); ground-target B757 heavy exception; autotrack FAA/ICAO equivalence (`NavigationDatabase.AirportIdsMatchResolved`, non-CONUS via the FAA/ICAO index). Already covered elsewhere: `QH` freeze (#251), block/above altitudes + ParsedAltitude (#263 PlannedAltitude). SatComm stays false (no FAA-suffix representation). **Split out:** CRR groups + RDB → **#268**; CDB (uncorrelated Mode-C intruder) → **#269**; `RF` FP transfer → **#270**.
- [x] **#262** MCA parsing nuances. Done: **C3** multi-FLID batch (`QQ 110 A/B/C`), **C4** `QQ` `/TT`/`///` overrides. **C2** re-verified as a no-op for ERAM — the ERAM Field-E SPC (ADIZ 1276 / AFIO 7777 / …) is derived client-side by CRC from the beacon code (`EramSpecialPurposeCode.ToSpc`), which YAAT already sends, and `BlinkSpc` already covers 1276/7777 (regression-guarded). **Split out:** **C5** external-ARTCC edit lock (needs an edit-ownership-gating system YAAT lacks) → **#270**.
