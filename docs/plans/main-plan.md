...
2. **Approach intercept validation**: When joining an approach course (`CAPP`/`JAPP`/`PTAC`), the aircraft's heading must achieve a ≤30° intercept angle with the final approach course. If the angle exceeds 30°, the command warns RPOs via terminal broadcast and the aircraft does not turn onto the approach. `CAPPF`/`JAPPF` (force variants) override the check and join regardless of intercept angle.
   - **Illegal intercept distance warning** (implemented pre-M5): `FinalApproachPhase` checks whether the aircraft was turned on final closer than the minimum intercept distance (7110.65 §5-9-1 approach gate). CIFP data pipeline (`CifpDataService` → `CifpParser` → `ApproachGateDatabase`) computes per-runway minimums from FAA FAF positions. Warning fires via `PendingWarnings`; pattern traffic is exempt. This feature becomes fully useful once CAPP/PTAC commands are implemented to vector aircraft onto approaches.
