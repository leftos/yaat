...
| A5 | Arrival waits on the other lane, turns in when the departure pulls to the spot (28:10) | D15 `PUSH $6B`; arrival `TAXI T A T6A @D16` | have | same class `.ArrivalWaitsOnOtherLane`: the arrival stops short of the pusher on 6A, resumes when the pusher rolls to 6B, parks at D16; no head-on deadlock |
...
| C3 | Crossing at H: one runway per instruction (35:36; SOP 3-3.b.iii) | `TAXI B H CROSS 1L HS 1R RWY 28R` | have | pin: 1L pre-cleared, 1R held; FK: `CROSS 1L 1R` on H warns |
...
| # | Technique | YAAT form | Status | Test to add |
|---|---|---|---|---|
| H1 | A388 out of the A ramp: A or B to H, cross at H, 28R by default (23:41) | `TAXI A H 28R HS 1R` (A388) | have | pin route; FK: no super on A north of H |
...
| H3 | Super arrival to the A ramp: Q B, hold short F1, then B H A (25:56) | `TAXI Q B HS F1`, `RES`, `TAXI B H A $1` | have | pin the two-step |
...
- [ ] Route pins per configuration: B1, B3, B5, B8 (28/1); D1, D2, D5, D7 (19/10); E1–E3 (19/19); F1 (10/10); H1–H3 (supers) — one `[Theory]` class per configuration over `sfo.geojson`, asserting the resolved hold-short list and zero reversals
