...
|---|---|---|---|---|
| A1 | Yankee-area gates (B23…C8) push onto taxilane Y instead of A when smaller than a B757 (05:11; SOP 3-5.c) | `PUSH Y A1` from B12 (E75L), then `TAXI Y A A1 1R` | probe (p13 for the taxi) | shipped 2026-09-15: `SfoYankeePushTests` pins the push (plain `PUSH Y` keeps the stand heading; `PUSH Y A1` ends on Y); the taxi-out shipped 2026-09-16: `SfoYankeeTaxiOutPinTests` and `Pathfinding/SfoYankeeConnectorChoiceTests` pin the bridge leaving Y through AY3 ahead of the nose (the detour ranks connectors by pavement cost plus a reversal charge against the aircraft's pose, `docs/ground/pathfinder.md`). A plain `PUSH Y` is a straight-back push that leaves the nose on the stand heading by design (user decision 2026-09-15) — the RPO names the facing (`PUSH Y A1`, `PUSH Y FACE S`) |
...
- [ ] A2 in `SfoYankeePushTests` (A1 shipped 2026-09-15; its taxi-out shipped 2026-09-16 with the connector-detour ranking)
- [ ] F-6 give-way to a pushback engaged too late (found by A4, 2026-09-15; **not reproduced after the tug-motion rework, 2026-09-16**): the old symptom was `WouldDeadlock` holding an arrival rolling down T6A only ~143 ft from an E-pier push, after which `PushLegClearanceFt` refused the push's remaining leg and both wedged (E6 125 ft, E8 66, E10U 23, E12 13). With tug moves the E6 push is still on its first leg when the arrival is through the alley (arrival parked at E9 in 75 s, push done at 194 s, minimum separation 314 ft), so `SfoSixAlleyArrivalAheadOfPushTests` now pins that clean pass and guards against a re-introduced hold. The realistic behaviour for a push that *does* reach the mover's lane first — mover holds at the alley entrance, push completes, mover continues — is still unpinned: it needs a new scenario where the push is already crossing (e.g. issue the arrival's taxi later, or push from a gate nearer the alley mouth) before anything here is treated as fixed
