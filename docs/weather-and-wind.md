...
- **Replay / scenario engine** (`SimulationEngine.cs`): the timeline is collapsed at restore/replay sites (`World.Weather = timeline.GetWeatherAt(t)`), and `ApplyWeatherJson` (`SimulationEngine.cs:2562`) routes a `LoadWeather` JSON through `WeatherTimelineParser` — a timeline is stored on `Scenario.WeatherTimeline` and immediately collapsed; a v1 profile clears the timeline and sets `World.Weather` directly. Snapshots serialize `World.Weather` as JSON (`SimulationEngine.cs:135`, `:148`).
- **Live server tick** (`SimulationHostedService.cs:152-162`): each second, if a timeline is active, `GetWeatherAt(elapsed)` is recomputed and only broadcast (`BroadcastWeatherChanged`) when `WeatherTimeline.HasMeaningfulChange(old, new)` is true.
- **`HasMeaningfulChange(a, b)`** (`WeatherTimeline.cs:94`) rate-limits broadcasts: true if precipitation changed, METAR list changed, layer count changed, or **any layer's direction differs by > 1° or speed by > 0.5 kt**.
