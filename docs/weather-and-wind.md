...
- Enabled per load via the `reconstructMetars` flag on `LoadWeather` — **true** for file/API weather, **false** for live-fetched weather (left untouched), replay, session restore, and report generation (the last three also keep replay deterministic, since the issuer anchors to `DateTime.UtcNow`).
