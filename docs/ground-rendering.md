...
6. **Aircraft symbols** then **datablocks** (always drawn, independent of the layout toggle). A fixed-wing aircraft on the ground whose `AircraftModel.TowbarHeading` is set (the wire's `TowbarTrueHeadingDeg`, present only under a `PushbackPhase`) also gets its **tug** (`DrawTug`): a towbar line from the nose gear (`0.12 × length` aft of the nose tip, a judgement call) `TowbarLengthFt` = 18 ft along the towbar heading, then a 15 × 8 ft tractor body with its cab at the hitch end, filled with the aircraft paint and rotated to the towbar heading. The heading comes from the sim's bicycle model (`PushbackPhase.SteerTowbar`, [ground/pushback.md](ground/pushback.md)), so the tug swings to the aircraft's left when a push yaws the nose right. Skipped below `MinTugPx` (4 px of tug length) so a zoomed-out view is not speckled; not hit-testable
...
| Push-route preview | `PushRoutePreview` (`TugPlan?`) + `DrawWaypoints` | interactive "Push route…" mode: `GroundViewModel.RefreshPushRoutePreview` re-plans through `TugMovePlanner.Plan` on every added or undone point, against the same parked neighbours the server uses (`TugParkedNeighbours.Build` over the client's aircraft), resolving tokens with the sim's own `GroundCommandHandler.ResolveTugGoal` (a lone spot or stand target with `ResolveStandOrSpotGoal`, the `PUSH $spot` / `@stand` body, facing and refusals included) and marked points with `ResolveMarkedPointGoal` (declination at the pushed aircraft, as the sim), then applying each target's forced kind as `ResolveMultiGoals` does; per-target forced kinds and marked points are keyed by waypoint index and published as `PushWaypointMarks` | `DrawPushLegs`: each move's simulated samples as a polyline in push/pull colour, an arrowhead at the move's end, a dot at every reversal. Not a `TaxiRoute` — the plan carries its own path. `DrawDrawnRoute` draws markers at marked points too, a `PUSH`/`PULL` label beside a forced target's marker and a facing tick on a faced marked point |
...
Whenever a route must be drawn, `GroundViewModel.ResolveRemainingRoute(ac)` **reconstructs the geometry locally**: it parses the taxiway-name string, finds the aircraft's nearest ground node, trims the sequence to start at `CurrentTaxiway`, and re-runs `TaxiPathfinder.ResolveExplicitPath` against the client's cached `AirportGroundLayout` (`_domainLayout`). It passes `AssignedRunway` (when set) as `ExplicitPathOptions.DestinationRunway` so the reconstruction **truncates at the runway hold-short** — the same hint the server used to build the route. Without it the resolver has no runway terminus and walks the last taxiway to its full physical extent, drawing past the hold-short bar (the `TAXI D C B 28R` "highlights all of B past 28R" bug). When that resolution fails, it hands the structured failure to `RampLaneReposition.TryPlan` with the aircraft's live position, heading and `CurrentTaxiway` — while the pilot is cutting across a ramp onto a parallel lane the map does not connect (SFO M3 → M4, issue #396) the nearest graph node is still on the old lane, and this reconstructs the same free-space `VirtualNode` leg the server planned so the overlay follows the crossing. The destination-end twin (OAK `TAXI V T TE @22`, issue #400) needs `TaxiDestination`: with the stand as `DestinationHintNode` the rebuilt route either fails or — from the aircraft's own end of the lane — "reaches" the stand only by doubling back down the lane, so a route that starts more than 135° off the nose is replaced by `RampLaneReposition.TryPlanDestinationCut`'s free-space leg when one plans. A route rebuilt from the aircraft's nearest graph node also gets the server's **approach leg** re-applied (`TaxiApproachLeg.Prepend` with the live position — the drive from where the aircraft stands onto that first node, see [ground/pathfinder.md](ground/pathfinder.md#approach-leg--a-free-space-drive-from-where-the-aircraft-stands-to-the-routes-first-node-taxiapproachleg)), so during the ~15 s after a plain `PUSH` the overlay starts at the aircraft, not 100 ft away at the ramp node. `DrawRoute` resolves each segment's endpoints through `GroundRenderer.TryResolveRouteSegmentEndpoints`, which falls back to the segment's own node positions for ids the layout does not hold — free-space legs (the approach leg, the #396/#400 cuts) draw as straights between their endpoints. Until 2026-09-10 it skipped any segment with an unknown id, so none of those legs were drawn.
...
**Ctrl node label.** The Ground View copies the Radar View's Ctrl+hover MVA tooltip (`docs/minimum-vectoring-altitude.md`). `OnPointerMoved` samples Ctrl from `e.KeyModifiers` into `_ctrlHeldAtPointer`. `OnKeyDown`/`OnKeyUp` set the flag too, so pressing or releasing Ctrl takes effect without moving the pointer. The Radar View handles its MVA tooltip the same way. `CreateRenderSnapshot` checks that flag, `IsPointerOver` and `ShowYaatLayout` (no marker when the layout is hidden), then resolves `FindCtrlHoverNode(_pointerPos)`: the nearest node of any type within `CtrlHoverNodeRadiusPx` (40). The label comes from `FormatCtrlNodeLabel`, as `#<id> · <Type>` plus the name when there is one. `#<id>` is the `NodeRefToken` form `PUSH`/`PUSHM` take. The label and the node's screen point go into the snapshot's last field. `GroundRenderer.DrawCtrlNodeHover` draws it last in the frame, on top of everything, outside the label-deconfliction pass. The label moves to the node's left side when it would run past the canvas's right edge.
...
- **Never `Dispose()` an `SKImage` a `RenderSnapshot` may still hold — drop the reference instead.** `CreateRenderSnapshot` (UI thread) captures
  `BackgroundImage` (the `TowerCabImage`'s `SKImage`) into the immutable snapshot that `RenderFromSnapshot` reads later on the compositor thread.
  The snapshot keeps the managed wrapper alive, but an explicit `Dispose()` frees the native handle regardless, so disposing on the UI thread while
  a frame is mid-render reads freed memory in `DrawBackgroundImage` → `sk_image_get_width()` and kills the process with an uncatchable
  `ExecutionEngineException`. `GroundViewModel.ClearLayout()` therefore sets `BackgroundImage = null` and lets finalization reclaim the image once
  no in-flight snapshot references it — the same swap `LoadTowerCabLayersAsync` does on a scenario switch. `GroundViewModelClearLayoutTests`
  pins it by asserting the image's `Handle` is still non-zero after `ClearLayout()`.
