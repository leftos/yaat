...
the Sim's phase gate: `STRIP`/`SCAN`/`HSM` presets used to be refused while parked
(`WAIT 30 STRIP Local` at a gate, issue #396), taxiing, or holding short because only
`Annotate` and the half-strip create/amend/delete verbs were listed.)
