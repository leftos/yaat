...
- **Snapshot generation needs the room's resolved positions.** Server-side snapshot generation (`RoomEngine.CreateTempReplayEngine`) builds the temp replay room with the bare Sim-side `LoadScenario`, which does **not** populate `StudentPosition` / `StudentTcp` / `AtcPositions` / `ArtccConfig` (those come from `ScenarioLifecycleService.InitializeTrackPositions`). They are copied from the live scenario right after load — without that copy, AS-prefixed track-ownership commands resolve to a null identity during the generating replay and every snapshot captures `Track.Owner = null`. The live rewind path is unaffected because it reloads via `ReloadForRewindAsync`, which resolves positions.
