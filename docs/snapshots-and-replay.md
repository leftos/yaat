...
- **The live host and the reconstruction host differ only in tape handling and METARs.** Both run the same spine and the same `TickProcessor` bodies; `LiveRoomHost` plays a tape back when the room is in playback and issues METARs, while `ReconstructionHost` re-applies a recorded log and never issues METARs. Weather advances in the sim on both.
