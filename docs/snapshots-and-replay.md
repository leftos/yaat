...
- **The live host and the reconstruction host differ only at the end of the second.** Both run the same spine and the same `TickProcessor` bodies; `LiveRoomHost` advances weather behind `HasMeaningfulChange` and issues METARs, while `ReconstructionHost` re-applies recorded weather ungated and never issues METARs.
