...
- **`RoomEngine.AdvanceLiveSecond` vs `AdvanceOneSecond`.** The live loop and the headless soak host advance through `AdvanceLiveSecond` (physics plus position-history sampling, weather-timeline advance, and METAR issuance); reconstruction (`ReconstructViaServerTick`) still steps `AdvanceOneSecond` and re-applies recorded weather, so it never samples history or issues METARs — a pre-existing difference, now explicit.
