...
- **Room-owned state outside `Yaat.Sim`** — flight strips, TDLS, the ASDE-X alert set and safety-logic config, tower lists (tick-path step 4 moves them). A same-room rewind keeps whatever the room holds; a from-scratch reconstruction (bundle export, the oracle's reconstruct leg) starts empty and rebuilds only what the action log reaches — strip requests and the safety-logic push are recorded for exactly that reason (see the record list above), auto-printed strips are re-derived by the tick, and TDLS is refused while replaying rather than duplicated.
