...
- **Flight-plan commands are never dispatched on replay.** Live, `DA`/`FP`/`RMK` (typed, or the CRC `AS {tcp} DA …` echo the STARS path records) go through `RoomEngine`'s flight-plan arm, which records the resulting `RecordedAmendFlightPlan` beside the command; the aircraft dispatcher never sees them. `RecordedCommandClassifier` classifies them as `RecordedCommandKind.FlightPlan` and both replay paths skip them (the amendment carries the state); `CommandDispatcher` refuses them at its public entries (`CompoundPolicy.IsFlightPlanCommand`) so no path can push a flight-plan edit through the phase gate — a `VfrHoldPhase` cancels on any non-additive command, which is how a reconstructed hover hold was lost while the live one carried on (S2-OAK-5 bundle, t=1771).
