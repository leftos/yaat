...
- **TAXI has two independent readback outputs — don't conflate them.** The **RSP** (controller-facing
  `CommandResult.Message`) is built in `GroundCommandHandler.TryTaxi` — `"Taxi via " + route.ToSummary(turnHints, clearedRunways)` (or `"Already at runway 1L — holding short (runway 1L assigned)"` for the segment-less bare-`TAXI <rwy>` route, issue #393)
