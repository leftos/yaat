...
![An ERAM display](img/eram/eram.png)
...
![The Message Composition Area](img/eram/mca.png)
...
> ℹ️ The arrow keys move the cursor around the Preview Area.

> ℹ️ `Ctrl`+`Page Down` and `Ctrl`+`Page Up` jumps the cursor to the end or the start of the Preview Area, respectively.

> ℹ️ `Ctrl`+`Enter` moves the cursor to the beginning of the next line in the Preview Area.

> ℹ️ `Ctrl`+`R` recalls the last command into the Preview Area.

> ℹ️ `Esc` clears the Preview and Feedback areas.

By default, the Preview Area is in overstrike mode (represented by an underscore cursor), meaning new characters overwrite the character at the cursor. The Preview Area can also be placed into insertion mode (represented by an underscore cursor with two additional vertical lines), meaning new characters are entered to the left of the cursor. Pressing `Ins` toggles between the two modes.
...
![The Message Composition Area menu](img/eram/mca-menu.png)

*The Message Composition Area menu*
...
> ℹ️ For a full list of ERAM commands, please see the [Command Reference](#command-reference) section of the documentation.

### Response Area

![The Response Area](img/eram/ra.png)

*The Response Area*

The Response Area (RA) displays text outputted by commands. For example, using the [`QF`](#qf-command) command outputs flight plan details in the RA ([Figure 4](#fig-eram-ra)).
...
![The Response Area menu](img/eram/ra-menu.png)

*The Response Area menu*
...
> ℹ️ `Ctrl`+`Del` clears the Response Area.

### Time View

![The Time View](img/eram/time-view.png)
...
![The Time View menu](img/eram/time-view-menu.png)
...
![The Beacon Code View](img/eram/beacon.png)

*The Beacon Code View*
...
![The Beacon Code View menu](img/eram/beacon-menu.png)

*The Beacon Code View menu*
...
![The Check List View](img/eram/checklist.png)
...
![The Check List View menu](img/eram/checklist-menu.png)

*The Check List View menu*
...
![The Altimeter Settings View](img/eram/altimeter-list.png)

*The Altimeter Settings View*
...
![The Altimeter Settings View menu](img/eram/altimeter-list-menu.png)

*The Altimeter Settings View menu*
...
![The Continuous Range Readout View](img/eram/crr-view.png)

*The Continuous Range Readout View*
...
![The Continuous Range Readout View menu](img/eram/crr-menu.png)

*The Continuous Range Readout View menu*
...
- **LIST**: toggles the Continuous Range Readout View between the list and panel modes ([Figure 16](#fig-eram-crr-panel))
...
![The Continuous Range Readout View in panel mode](img/eram/crr-panel.png)

*The Continuous Range Readout View in panel mode*
...
> ℹ️ `LF <label> <aircraft>`

Aircraft are entered as above.

### Weather Station Report View

![The Weather Station Report View](img/eram/weather-list.png)

*The Weather Station Report View*
...
![The Weather Station Report View menu](img/eram/weather-list-menu.png)

*The Weather Station Report View menu*
...
A GeoMap Filter is toggled by left or middle-clicking the appropriate filter button in the [GeoMap](#geomap-toolbar-menu) Toolbar menu.

> ℹ️ Some GeoMap elements may not be assigned to a Filter and will always be visible.

> ℹ️ Some GeoMap elements may only be visible in [Top-Down mode](#top-down-mode).
...
> ℹ️ When disconnected from the VATSIM network, the message **NO RADAR DATA** is displayed in the center of the display.
...
| --- | --- | --- |
| Uncorrelated Primary | ![Uncorrelated Primary target](img/eram/targets/uncorrelated-primary.png) | An aircraft whose transponder is in the off/standby mode |
| Correlated Primary | ![Correlated Primary target](img/eram/targets/correlated-primary.png) | An aircraft whose transponder is in the off/standby mode and has been manually correlated to a flight plan |
| Code 1200 Beacon | ![Code 1200 Beacon target](img/eram/targets/vfr.png) | An aircraft below the conflict alert floor squawking VFR (1200) |
| Identing Beacon | ![Identing Beacon target](img/eram/targets/ident.png) | An aircraft squawking ident |
| Uncorrelated Beacon | ![Uncorrelated Beacon target](img/eram/targets/uncorrelated-beacon.png) | An aircraft below the conflict alert floor squawking a beacon code that is not correlated to a flight plan |
| Mode C Intruder | ![Mode C Intruder target](img/eram/targets/mci.png) | An aircraft above the conflict alert floor squawking a beacon code that is not correlated to a flight plan |
| Correlated Beacon | ![Correlated Beacon target](img/eram/targets/correlated-beacon.png) | An aircraft squawking a beacon code that is correlated to a flight plan |
| Reduced Separation | ![Reduced Separation target](img/eram/targets/reduced-separation.png) | An aircraft in a reduced separation area squawking a beacon code that is correlated to a flight plan |

### Target History

![A Target with a history trail](img/eram/targets/history.png)

*A Target with a history trail*
...
Table 2 - Tracks and Position Symbols

| Name | Position Symbol | Description |
| --- | --- | --- |
| Flight Plan Aided (Flat) Track | ![Flight Plan Aided (Flat) Track](img/eram/tracks/flat.png) | A Track associated with a Target following a flight plan route |
| Free Track | ![Free Track](img/eram/tracks/free.png) | A Track associated with a Target that is not following a flight plan route |
| Coast Track | ![Coast Track](img/eram/tracks/coast.png) | A Track that is not associated with a Target |
| Frozen Track | ![Frozen Track](img/eram/tracks/frozen.png) | A Track that has been frozen in a location |
...
> ℹ️ Sector IDs can be found in the [Controller List](overview.md#controller-list).

Pending handoffs are accepted or recalled by an Implied command with the aircraft's FLID:

> ℹ️ `<FLID>`

> ℹ️ Unlike legacy clients, handoffs cannot be rejected.
...
> ℹ️ Unlike legacy clients, point outs can only be initiated to other vNAS ERAM sectors.

![An FDB with a pending point out](img/eram/pointout.png)

*An FDB with a pending point out*
...
![A point out menu with a pending point out](img/eram/pointout-menu.png)

*A point out menu with a pending point out*
...
![A point out menu with an unacknowledged point out](img/eram/pointout-receive-menu.png)

*A point out menu with an unacknowledged point out*
...
![A frozen Track](img/eram/frozen.png)
...
> ℹ️ `LA [<location 1> | <track>] <location 2> T`
...
![An unpaired LDB](img/eram/data-blocks/unpaired-ldb.png)
...
![A paired LDB](img/eram/data-blocks/paired-ldb.png)
...
![An enhanced LDB](img/eram/data-blocks/eldb.png)
...
![An FDB](img/eram/data-blocks/fdb.png)

*An FDB*

Full Data Blocks (FDBs) have a [track position symbol](#tracks), [leader line](#leader-lines), and [velocity vector lines](#velocity-vector-lines). An FDB may be [positioned](#positioning-data-blocks) at any of the eight positions adjacent to its Target as listed in [Table 6](#table-eram-db-positions).

![FDB fields](img/eram/data-blocks/fdb-fields.png)
...
![VCI](img/eram/data-blocks/vci.png)
...
Table 3 - Field B character 4 values
...
![An FDB with a non-RVSM indicator](img/eram/data-blocks/non-rvsm.png)

*An FDB with a non-RVSM indicator*
...
> ℹ️ `Page Up` and `Page Down` increases and decreases the length of velocity vector lines, respectively.
...
> ℹ️ Unlike legacy clients, velocity vector line length is synced between all controllers on a sector.

#### Distance Reference Indicators

![A standard and reduced separation DRI](img/eram/data-blocks/dris.png)

*A standard and reduced separation DRI*
...
![An Altitude Menu](img/eram/data-blocks/altitude-menu.png)
...
> ℹ️ Assigned Altitudes in non-RVSM airspace are displayed in gray if the aircraft is not RVSM equipped.
...
![A Heading Menu](img/eram/data-blocks/heading-menu.png)
...
![A Speed Menu](img/eram/data-blocks/speed-menu.png)
...
![A Free Form Text Box Menu](img/eram/data-blocks/free-form-text-box-menu.png)

*A Free Form Text Box Menu*
...
> ℹ️ The Free Form Text Box Menu cannot be opened if the aircraft does not have any assigned fourth line free form text.

### Range Data Blocks

![An RDB](img/eram/data-blocks/rdb.png)
...
> ℹ️ `Ctrl`+`Shift`+`O` cycles the offset between the RDB and the FDB's leader line.

### Conflict Data Blocks

![A CDB](img/eram/data-blocks/cdb.png)
...
> ℹ️ To cycle between an LDB and an E-LDB, left or middle-click the altitude field.

> ⚠️ After a [point out](#point-outs) is initiated, an aircraft's FDB cannot be minimized with the standard `FLID` Implied command, even if the point out has been accepted and cleared. To minimize the FDB, the `QP` command must be used:

> ℹ️ `QP <FLID>`

### Positioning Data Blocks

A Data Block can be positioned relative to its Target or Track by an Implied command with one of the positions listed in [Table 6](#table-eram-db-positions):

> ℹ️ `<1-9> <FLID>`

Table 6 - Data Block positions
...
![A Track with Dwell Emphasis](img/eram/data-blocks/dwell.png)

*A Track with Dwell Emphasis*
...
![A QF command response](img/eram/ra.png)

*A QF command response*
...
The `SR` command opens the flight plan in the [Flight Plan Editor](overview.md#flight-plan-editor):
...
> ℹ️ `VP <aircraft type> <route> <FLID>`
...
> ℹ️ `QB <equipment> <FLID>`

The `QB` command also assigns a voice type to an aircraft:

> ℹ️ `QB [/v | /r | /t] <FLID>`

> ℹ️ Unlike legacy clients, `QB V`, `QB R`, and `QB T` no longer assign a voice type to an aircraft. Instead, the voice type parameter must be prefixed with a `/`.
...
> ℹ️ `QZ VFR <FLID>` or `QZ VFR/<altitude> <FLID>` or `QZ OTP <FLID>`

The `QZ` command is also used to set block altitudes:

> ℹ️ `QZ <floor>B<ceiling>`
...
> ⚠️ Instead of `/OK`, `/TT` or `///` is used in the `QQ` command to override logic checks: `QQ /TT P150 JBU123`

> ℹ️ Multiple FLIDs may be specified in one `QQ` command: `QQ 110 JBU123/429/AAL924`
...
> ℹ️ For more information on how altitudes are displayed in Data Blocks, please see the [Data Blocks](#data-blocks) section of the documentation.

### Routing

##### QU Command

![A route display](img/eram/route.png)
...
> ℹ️ Multiple FLIDs may be specified in one `QU` command: `QU 30 JBU123/429/AAL924`

> ℹ️ Unlike legacy clients, route displays are displayed to all controllers logged in to the same sector.

The QU command is also used to amend an aircraft's route direct to one or more specified fix or location:

> ℹ️ `QU [<fix> | <location>] <FLID>`
...
> ℹ️ Logic checks may be overridden with `/OK`: `QU /OK PROVI JBU123`

> ℹ️ Locations can be entered as left-clicked display locations. This makes it easy to draw a path around weather when rerouting an aircraft.

##### AM Command

The `AM` command is used to amend portions of a flight plan. It is used as follows:

Table 7 - The `AM` command
...
> ℹ️ The Field Number may be used in place of the field name, such as `AM <FLID> 3 B737/L`.
...
Table 9 - A Menu Button

| Closed Menu Button | Open Menu Button |
| --- | --- |
| ![Closed menu button](img/eram/toolbars/buttons/menu-closed.png) | ![Opened menu button](img/eram/toolbars/buttons/menu-opened.png) |

A Menu Button opens or closes its Toolbar menu when it is left or middle-clicked.

#### Toggle Buttons

Table 10 - A Toggle Button

| Deactivated Toggle Button | Activated Toggle Button |
| --- | --- |
| ![Toggle button, off](img/eram/toolbars/buttons/toggle-off.png) | ![Toggle button, on](img/eram/toolbars/buttons/toggle-on.png) |

A Toggle Button toggles a view or feature when it is left or middle-clicked.

#### Increment/Decrement Buttons

![An Increment/Decrement Button](img/eram/toolbars/buttons/inc-dec.png)
...
Table 11 - A Command Button

| Deactivated Command Button | Activated Command Button |
| --- | --- |
| ![Deactivated command button](img/eram/toolbars/buttons/command-deactivated.png) | ![Activated command button](img/eram/toolbars/buttons/command-activated.png) |
...
![The Toolbar Control menu](img/eram/toolbars/control.png)

*The Toolbar Control menu*
...
![The Master Toolbar menu](img/eram/toolbars/main.png)

*The Master Toolbar menu*
...
- **RADAR FILTER**: opens the [Radar Filter](#radar-filter-toolbar-menu) Toolbar menu
- **PREFSET**: not simulated
- **DELETE TEAROFF**: initiates the [delete tear-off](#tear-offs) function

### Views Toolbar Menu

![The Views Toolbar menu](img/eram/toolbars/views.png)

*The Views Toolbar menu*
...
![The ATC Tools Toolbar menu](img/eram/toolbars/atc-tools.png)

*The ATC Tools Toolbar menu*

The ATC tools Toolbar menu contains the following buttons:

- **CRR FIX**: displays CRR fixes
- **SPEED ADVSRY**: not simulated
- **WX**: opens the [Weather](#weather-toolbar-menu) Toolbar menu

### Weather Toolbar Menu

![The Weather Toolbar Menu](img/eram/toolbars/weather.png)

*The Weather Toolbar Menu*
...
![The Check Lists Toolbar menu](img/eram/toolbars/check-lists.png)

*The Check Lists Toolbar menu*
...
![The Cursor Toolbar menu](img/eram/toolbars/cursor.png)

*The Cursor Toolbar menu*
...
![An example GeoMap Toolbar menu](img/eram/toolbars/geomap.png)

*An example GeoMap Toolbar menu*
...
![An example Map Brightness Toolbar menu](img/eram/toolbars/map-brightness.png)

*An example Map Brightness Toolbar menu*
...
![The Font Toolbar menu](img/eram/toolbars/font.png)
...
![The Radar Filter Toolbar menu](img/eram/toolbars/radar-filter.png)

*The Radar Filter Toolbar menu*
...
![The Data Block Fields Toolbar menu](img/eram/toolbars/db-fields.png)

*The Data Block Fields Toolbar menu*
...
> ℹ️ `QD <lower limit>B<upper limit>`

## NEXRAD

![Hurricane Idalia depicted in NEXRAD](img/eram/nexrad/nexrad.png)

*Hurricane Idalia depicted in NEXRAD*
...
Table 12 - NEXRAD precipitation levels

| Moderate Precipitation | Heavy Precipitation | Extreme Precipitation |
| --- | --- | --- |
| ![Moderate NEXRAD precipitation](img/eram/nexrad/nexrad-moderate.png) | ![Heavy NEXRAD precipitation](img/eram/nexrad/nexrad-heavy.png) | ![Extreme NEXRAD precipitation](img/eram/nexrad/nexrad-extreme.png) |
...
Table 13 - NEXRAD level settings
...
While unrealistic, CRC's ERAM supports the following [Dot commands](overview.md#dot-commands) to show or hide scope reference markers:

Table 14 - Scope markers commands
...
![A Ground Target and Data Block with a correlated flight plan](img/eram/ground-data-block.png)

*A Ground Target and Data Block with a correlated flight plan*

> ⚠️ Note that the Ground Target symbol depicted and the aircraft type displayed in an aircraft's Data Block are derived from the aircraft type specified by the pilot on connection, not the aircraft type in the corresponding flight plan (though the two should normally correspond).

> ⚠️ Unlike ASDE-X, Boeing 757s are not depicted by the heavy aircraft icon.
...
The following commands are typed into the [MCA](#message-composition-area) and executed by pressing `Enter` or middle-clicking to select a track or location (if supported by the command).

Table 17 - ERAM commands
...
| `QB <ACID>` | [Requests](#qb-command_1) a beacon code |  |  |
| `QB <code> <ACID>` | [Assigns](#qb-command_1) a beacon code |  |  |
| `QB <code1> <code2> ...` | [Adds/removes](#qb-command) beacon codes from the [Beacon Code View](#beacon-code-view) |  |  |
| `QB <equipment> <FLID>` | [Assigns](#qb-command_1) an equipment suffix |  |  |
| `QB [/v\|/r\|/t] <FLID>` | [Assigns](#qb-command_1) a voice type |  |  |
| `QD <station ID>` | [Toggles](#qd-command) display of an [altimeter setting](#altimeter-settings-view) |  |  |
| `QD <lower limit>B<upper limit>` | [Sets](#qd-command_1) altitude limits |  |  |
...
| `QP J <FLID>` | [Toggles](#qp-command_1) display of a standard [DRI](#distance-reference-indicators) |  |  |
| `QP T <FLID>` | [Toggles](#qp-command_1) display of a reduced separation DRI for an eligible Track |  |  |
...
![ERAM display settings](img/eram/settings.png)

*ERAM display settings*
...
Table 19 - ERAM acronyms and abbreviations
