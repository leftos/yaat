...
![An ASDE-X display](img/asdex/asdex.png)
...
![The night color scheme](img/asdex/night.png)
...
> ℹ️ Boeing 757s are depicted by heavy aircraft Targets.
...
![A Target history trail](img/asdex/history.png)

*A Target history trail*
...
![A Target with a velocity vector line](img/asdex/vector.png)

*A Target with a velocity vector line*
...
![A Data Block displaying all standard fields](img/asdex/data-block.png)

*A Data Block displaying all standard fields*
...
> ℹ️ An aircraft's Data Block is hidden if the aircraft enters a [Data Block Off Area](#data-block-off-areas).
...
> ℹ️ Leader lengths cannot be set on a per-Data Block basis, though an aircraft's Data Block's leader length may be altered if the aircraft enters a [Data Block Trait Area](#data-block-trait-areas).

##### To set the position of all Data Blocks:

1. Type one of the valid inputs from [Table 2](#table-asdex-db-positions)
2. Press `Enter`
...
> ℹ️ An aircraft's Data Block's position may be altered if the aircraft enters a Data Block Trait Area.

Data Blocks contain the following fields:

Table 3 - Data Block fields
...
> ℹ️ If an aircraft's Track is coasted, field E is always displayed, even if disabled on the [Data Block Edit](#data-block-edit-submenu) DCB submenu or by a Data Block Trait Area.
...
An aircraft's category is assigned as listed in [Table 4](#table-asdex-cat-no-cwt) for facilities without CWT, or [Table 5](#table-asdex-cat-cwt) for facilities with CWT. The aircraft category also dictates which Target icon is displayed.

Table 4 - Aircraft categories - no CWT
...
> ℹ️ For the purposes of VATSIM, all non-piston aircraft are assumed to have TCAS.

Table 5 - Aircraft categories - CWT
...
> ℹ️ For the purposes of VATSIM, it is not possible to tag an unknown Target with the incorrect aircraft ID.

> ⚠️ Manually tagging an unknown Target is not the same as [initiating control](#initiating-control-of-a-coasted-or-suspended-track) of a [coasted](#coasted-and-dropped-tracks) or [suspended](#suspending-a-track) Track.
...
![Modifying a Data Block's fields in the Preview Area](img/asdex/modify-data-block.png)

*Modifying a Data Block's fields in the Preview Area*
...
> ⚠️ Editing an aircraft's Data Block fields (such as assigning an aircraft type to **TYP**) does not edit an aircraft's flight plan.

> ℹ️ For the purposes of VATSIM, an aircraft's ID (**A/C**) cannot be edited. However, if an aircraft is not squawking its assigned beacon code, the aircraft's ID can be manually entered. Only the correct aircraft ID can be entered.

## Display Control Bar

![The Main DCB menu](img/asdex/dcb/main.png)
...
> ⚠️ If the window containing the ASDE-X display is not large enough to fit the DCB, the DCB is collapsed to a minimized state.

### Main Menu

The Main DCB menu ([Figure 7](#fig-asdex-main-dcb)) contains the following buttons:

- **RANGE**: controls the Display Map's [range](#to-change-the-display-maps-range). The display's current horizontal range is displayed in hundreds of feet.
- **MAP RPOS**: [repositions](#to-reposition-the-display-map) the Display Map
- **ROTATE**: controls the Display Map's [orientation](#to-change-the-display-maps-orientation)
...
- **DAY/NITE**: toggles the Display Map's [color scheme](#to-change-the-display-maps-color-scheme)
...
> ℹ️ `F9` executes the undo function.

### Pref Sets Submenu

![The Pref Sets DCB submenu](img/asdex/dcb/pref-sets.png)

*The Pref Sets DCB submenu*
...
> ℹ️ For more information on Pref Sets, please see the [Pref Sets](#pref-sets) section of the documentation.

### Brightness Submenu

![The Brightness DCB submenu](img/asdex/dcb/brightness.png)

*The Brightness DCB submenu*
...
![The Character Size DCB submenu](img/asdex/dcb/char-size.png)

*The Character Size DCB submenu*
...
![The Safety Logic DCB submenu](img/asdex/dcb/safety-logic.png)

*The Safety Logic DCB submenu*
...
![The Closed Runway DCB submenu](img/asdex/dcb/closed-runway.png)

*The Closed Runway DCB submenu*
...
> ℹ️ For more information on closed runways, please see the [Safety Logic Closed Runways](#closed-runways) section of the documentation.

> ℹ️ The Closed Runway submenu is only populated while connected to the VATSIM network.

### Runway Configuration Submenu

![The Runway Configuration DCB submenu](img/asdex/dcb/runway-configuration.png)

*The Runway Configuration DCB submenu*
...
> ℹ️ For more information on safety logic runway configurations, please see the [Safety Logic Runway Configurations](#runway-configurations) section of the documentation.

> ℹ️ The Runway Configuration submenu is only populated while connected to the VATSIM network.

### Tower Configuration Submenu

![The Tower Configuration DCB submenu](img/asdex/dcb/tower-configuration.png)

*The Tower Configuration DCB submenu*
...
> ℹ️ For more information on safety logic tower configuration positions, please see the [Safety Logic Tower Configuration Positions](#tower-configuration-positions) section of the documentation.

> ℹ️ The Tower Configuration submenu is only populated while connected to the VATSIM network.

### Arrival Alerts Submenu

![The Arrival Alerts DCB submenu](img/asdex/dcb/tower-configuration.png)

*The Arrival Alerts DCB submenu*
...
> ℹ️ The Arrival Alerts submenu is only populated while connected to the VATSIM network.

### Tools Submenu

![The Tools DCB submenu](img/asdex/dcb/tools.png)

*The Tools DCB submenu*

The Tools DCB submenu contains the following buttons:

- **RANGE**: controls the Display Map's [range](#to-change-the-display-maps-range). The current horizontal range is displayed in hundreds of feet.
- **MAP RPOS**: [repositions](#to-reposition-the-display-map) the Display Map
- **ROTATE**: controls the Display Map's [orientation](#to-change-the-display-maps-orientation)
...
![The Temporary Data DCB submenu](img/asdex/dcb/temp-data.png)

*The Temporary Data DCB submenu*
...
![The Stored Global Temporary Data DCB submenu](img/asdex/dcb/temp-data-presets.png)
...
- **SAVE**: initiates the [save Temporary Data Preset](#to-save-a-temporary-data-object-to-a-preset) function
...
![The Data Block Area DCB submenu](img/asdex/dcb/db-area.png)

*The Data Block Area DCB submenu*
...
![The Data Block Edit DCB submenu](img/asdex/dcb/db-edit.png)

*The Data Block Edit DCB submenu*
...
> ℹ️ Settings in this submenu are only applied to Data Blocks for aircraft outside of [Data Block Trait Areas](#data-block-trait-areas).

> ℹ️ The Sensor coverage field is automatically enabled for the Data Blocks of [coasted](#coasted-and-dropped-tracks) Tracks.

## Preview Area

![The Preview Area](img/asdex/preview.png)

*The Preview Area*

The Preview Area displays system information and serves as the input and feedback area when executing functions. [Dot commands](overview.md#dot-commands) can also be entered into the Preview Area. The Preview Area displays lines of text in the following order:

Table 6 - Preview Area lines
...
Safety Logic status line 1 displays a list of [tower configuration positions](#tower-configuration-positions) with [disabled arrival alerts](#to-temporarily-inhibit-or-enable-arrival-alerts-for-a-positions-assigned-runways). Safety Logic status line 2 displays the message **TRK ALERT INHIB** when alerts are [inhibited](#to-inhibit-or-enable-alerts-for-an-individual-target) for one or more Tracks.

> ℹ️ When disconnected from the VATSIM network, the message **CRITICAL FAULT START** is displayed in the Preview Area. When reconnected, this message is replaced with the **CRITICAL FAULT END** message.
...
> ℹ️ `Ctrl`+`Shift`+`C` displays the cursor's x, y position in feet from the air traffic control tower in the Preview Area.
...
![The Coast/Suspend List](img/asdex/coast.png)
...
> ⚠️ Initiating control of a coasted or suspended Track is different than [manually tagging](#manually-tagging-an-unknown-target) an unknown Target.
...
> ℹ️ The default PIN for every operator is `1234`.
...
![An active Safety Logic Alert](img/asdex/safety.png)

*An active Safety Logic Alert*
...
> ℹ️ `F12` silences the active audible alert

### Alert Message Box

![The Alert Message Box](img/asdex/alert.png)
...
Table 7 - Alert description lines
...
![A closed runway's threshold](img/asdex/closed.png)

*A closed runway's threshold*

Runways can be closed to generate alerts for aircraft attempting to land or depart on a closed surface. Closed runways are depicted with white X's on both ends of the runway ([Figure 25](#fig-asdex-closed-runway)).

##### To close a runway:

1. Left-click the appropriate button on the [Closed Runway](#closed-runway-submenu) DCB submenu

> ⚠️ Opening or closing a runway is a global function that affects all other ASDE-X controllers in the facility.

### Hold Bars

![An occupied runway's hold bars](img/asdex/hold-bars.png)

*An occupied runway's hold bars*
...
> ℹ️ All ASDE-X facilities contain a **LIMITED** runway configuration which only generates alerts for arrivals or departures on closed runways.
...
> ⚠️ Setting a runway configuration is a global function that affects all other ASDE-X controllers in the facility.
...
> ℹ️ All ASDE-X facilities contain a default position that is always enabled, therefore alerts for runways assigned to the default position are always displayed (unless the **LIMITED** [runway configuration](#runway-configurations) is active).
...
> ℹ️ Enabling or disabling a tower configuration position is a local function that does not affect other ASDE-X controllers in the facility.
...
> ⚠️ Enabling or disabling arrival alerts for a tower configuration position is a global function that affects all other ASDE-X controllers in the facility.

All alerts involving an individual Target can also be inhibited.

![A Target with alerts inhibited](img/asdex/inhibit.png)

*A Target with alerts inhibited*
...
When alerts are inhibited for a Target, a square is drawn around the Target ([Figure 27](#fig-asdex-inhibit)).
...
> ℹ️ Alerts can be disabled between two or more aircraft by inserting `MIL` in the applicable aircraft's [scratchpads](#to-edit-a-data-blocks-scratchpad). This is useful for military or other aircraft formations.
...
> ⚠️ Editing Temporary Data is a global function that affects all other ASDE-X controllers in the facility.

### Temporary Map Areas

![Closed and Restricted Temporary Map Areas](img/asdex/temp-areas.png)

*Closed and Restricted Temporary Map Areas*

There are two types of Temporary Map Areas: closed areas and restricted areas (both depicted in [Figure 28](#fig-asdex-temp-areas)). Closed areas are depicted as red polygons filled with red parallel lines. Restricted areas are depicted as yellow polygons filled with yellow parallel lines. Temporary Area polygons can have up to 20 sides and cannot be edited after creation.
...
![Temporary Map Text Annotation](img/asdex/temp-text.png)

*Temporary Map Text Annotation*

Temporary Map Text Annotations are depicted as a star icon anchor and attached Data Block with one or two lines of text. Each line can contain up to 16 alphanumeric characters. Temporary Map Text Annotations cannot be edited or moved after creation. Temporary Map Text Data Blocks can be interacted with in many of the same ways as aircraft [Data Blocks](#data-blocks), such as [toggling display](#to-toggle-an-aircrafts-data-block) by left-clicking the Target, changing the [leader line length](#to-set-the-length-of-all-data-block-leader-lines), and changing the Data Block [position](#to-set-the-position-of-a-single-data-block). Additionally, Temporary Text is affected by [Data Block Trait Areas](#data-block-trait-areas).
...
> ⚠️ Deleting Temporary Data is a global function that affects all other ASDE-X controllers in the facility. Deleting Temporary Data is irreversible if the data is not saved in a [Preset](#to-save-a-temporary-data-object-to-a-preset).

> ℹ️ If the deleted Temporary Data is stored in a Preset, the Preset is [deactivated](#to-activate-or-deactivate-a-temporary-data-preset) but not deleted.
...
3. If a Preset is active and the associated Temporary Data is [visible](#to-hide-temporary-data), the Preset is deactivated and the associated Temporary Data object is hidden for all controllers in the facility.
4. If a Preset is active and the associated Temporary Data is [hidden](#to-hide-temporary-data), the associated Temporary Data object is displayed.
...
> ⚠️ Deleting a Temporary Data Preset is a global function that affects all other ASDE-X controllers in the facility. Deleting a Temporary Data Preset is irreversible.

## Data Block Areas

![Data Block Trait and Off Areas](img/asdex/db-areas.png)

*Data Block Trait and Off Areas*

Data Block Areas are defined geographic areas that alter the Data Blocks of Targets within the Area. The Area's outlines are only displayed while inside the [Data Block Area](#data-block-area-submenu) DCB submenu ([Figure 30](#fig-asdex-db-areas)), but the Area are always active. There are two types of Data Block areas: [Data Block Off Areas](#data-block-off-areas) and [Data Block Trait Areas](#data-block-trait-areas). Each Data Block Area is a polygon with up to 20 sides. Data Block Area polygons cannot be edited after creation. Data Block Areas are defined independently per [window](#secondary-windows), with each window supporting up to 25 Data Block Areas. Data Block Areas may not overlap.

> ℹ️ Data Block Areas are local and do not affect other ASDE-X controllers in the facility.

### Data Block Off Areas

When an aircraft enters a Data Block off Area, its Data Block is hidden until the aircraft exits the Area. The Data Block can still be [toggled](#to-toggle-an-aircrafts-data-block) by left-clicking on the aircraft Target.
...
> ⚠️ Deleting Data Block Trait Areas is irreversible

## Secondary Windows

![An active Secondary Window](img/asdex/secondary.png)

*An active Secondary Window*
...
> ⚠️ If the ASDE-X display is resized such that a secondary window is no longer contained within the display, or the secondary window would overlap another secondary window, the secondary window is hidden.
...
> ⚠️ Deleting a secondary window is irreversible and there is no confirmation prompt after left-clicking the window.

## Command Reference

### Implied Commands

Table 8 - Implied Commands
...
| Left-click a Target | Toggles the [Data Block](#to-toggle-an-aircrafts-data-block) for the Target |
...
| `F10` | Toggles the Display Map's [color scheme](#to-change-the-display-maps-color-scheme) |
...
![A Multifunction command in the Preview Area](img/asdex/mult.png)

*A Multifunction command in the Preview Area*

Multifunction commands are initiated by pressing the m=Multifunction (`F7`) key.

Table 10 - Multifunction Commands
...
| `MULTIFUNC H`, left-click a Target, then `<scratchpad> Enter` | Sets the aircraft's [scratchpad area](#to-edit-a-data-blocks-scratchpad) 2 |
...
| `MULTIFUNC Y`, left-click a Target, then `<scratchpad> Enter` | Sets the aircraft's [scratchpad area](#to-edit-a-data-blocks-scratchpad) 1 |

## Settings

![ASDE-X display settings](img/asdex/settings.png)

*ASDE-X display settings*
...
> ℹ️ For more information on the Status Text (an unrealistic addition to ASDE-X), please see the [Status Text](tower-cab.md#status-text) section of the Tower Cab documentation.
