...
| `IsIdleAwaitingCommands` | `bool` (virtual, default `false`) | `true` on terminal command-waiting phases whose `OnTick` never returns `true` on its own; while such a phase is current, `FlightPhysics` advances **untriggered** queued blocks (see the CommandQueue pitfall below). Never set it on a motion phase — queued blocks would fire mid-maneuver. Pinned by `GroundPhaseConvention.IdleAwaitingCommands_SetMatchesStationaryWhitelist`. |
...
**Ground** — `TaxiingPhase`, `HoldingShortPhase`, `CrossingRunwayPhase`, `RunwayExitPhase`, `PushbackPhase`, `AirTaxiPhase`, `AtParkingPhase`, `FollowingPhase`, `HoldingInPositionPhase`, `HoldingAfterPushbackPhase`, `HoldingAfterExitPhase`. `RunwayExitPhase.CompleteExit` sets `AircraftGroundOps.AwaitingTaxiInCall`; `HoldingAfterExitPhase` and `HoldingInPositionPhase` make the arrival's taxi-in call from it (`Pilot/TaxiInRequest.cs`). `HoldingShortPhase` accepts `FOLLOWG` at every bar. At a bar that protects a runway (`ProtectsARunway`) the follow is **armed**: `GroundCommandHandler.ArmFollowBehindRunwayHold` keeps the hold and leaves [hold, `FollowingPhase`], because following is not a crossing clearance (7110.65 §3-7-2d requires the crossing clearance in addition). `CROSS` then puts a `CrossingRunwayPhase` in front of the follow — built from the route when it resumes from this bar, else over its own synthetic path (`CrossingRunwayPhase.OverOwnPath`, snapshotted as `OwnPath`), never replacing `AssignedTaxiRoute`, so the follower's departure bar still files as `DestinationRunway`; the runways that `CROSS` cleared ride on the `FollowingPhase` (`CrossingClearedRunways`), which does not stop at their bars. `RES` at an armed runway bar is refused in `CommandDispatcher`, and `FOLLOWG` at a departure bar whose route is incomplete is refused (`LUAW`/`CTO` only). `FollowingPhase.CheckRunwayHoldShort` skips a bar of a runway the follower is already on (leaving is not crossing) and files the bar of the follower's own route destination as `DestinationRunway` (so `RES` there does not release it onto the runway, and the departure queue ranks it tier 0); every other bar stays `RunwayCrossing`. **A stand is `AtParkingPhase`; anything else a ground movement stops on is `HoldingInPositionPhase`** — the same rule in four writers: `GroundCommandHandler.AddAirTaxiTerminus` (`ATXI`/`LAND` to a helipad or parking vs a taxi spot), `TaxiingPhase.CompleteRoute` (`route.DestinationParking` vs a spot), `GroundCommandHandler.TryTaxi`'s zero-segment shortcut (the same discriminator, for a `TAXI` issued to an aircraft already standing on its destination), and `FlightCommandHandler.ApplyWarpGround` (`GroundNodeType.Parking or Helipad` vs everything else). `GroundCommandHandler.InstallTugMove` applies it too for every `PUSH` form and `PUSHM`, with `HoldingAfterPushbackPhase` in place of `HoldingInPositionPhase` for a spot or node terminus — `TryPushback` and `TryPushbackMulti` take either of those two (or a running `PushbackPhase`, for a redirect) as a precondition, so an aircraft pushed onto a spot can be pushed again. The stand writers also own `Ground.ParkingSpot`; a writer that lands the aircraft somewhere else clears it, or the Aircraft List keeps reporting a stand the aircraft left.
...
`TakeoffPhase` writes IAS directly with `TargetSpeed = null` (the spool ramp of `GroundRollProfile`, clocked by the phase's own
`RollElapsedSeconds` so a hand-ticked phase rolls exactly like an engine-ticked one) and deliberately never reads `Ground.SpeedLimit` — the taxi-calibrated
...
- **`FlightPhysics.NotifyPhaseAdvanced` runs once per completed phase, after the list is settled.** `PhaseRunner` calls it after the LAHSO rebuild, the full-stop exit append or the pattern auto-cycle, never straight after `AdvanceToNext`: a phase those branches skip must not be offered (and consume) a queued block. So a `;` block queued behind a LAHSO landing fires on the tick the aircraft stops, against `RunwayHoldingPhase`.
- **`CurrentPhase != null` bypasses ordinary CommandQueue advancement — except in idle phases.** Triggered blocks still fire, and while the current phase overrides `IsIdleAwaitingCommands` (AtParking, HoldingAfterPushback, HoldingAfterExit, HoldingInPosition, HoldingShort, LinedUpAndWaiting) `FlightPhysics.AdvanceQueueWhileIdle` also applies untriggered blocks in strict `;` order — these phases never complete on their own, so the queue is the aircraft's only source of progress (`PUSH; SQ; SQNORM; TAXI …`, issue #407). In every other phase an untriggered block waits; the phase either accepts a fresh command (`Allowed`), rejects it (`Rejected`), or yields (`ClearsPhase`).
...
- **A runway crossing can swallow the *next* runway's hold-short bar.** When a taxi route's crossing slice ends the route, `TaxiingPhase.BuildTerminalPhasesAtCrossingExit` — shared by `BuildResumePhases` and `BuildPreClearedCrossingPhases` — decides the terminal phase. If the crossing's exit node still carries an **uncleared** `HoldShortPoint`, the terminal is `HoldingShortPhase`, not `HoldingInPositionPhase`: on parallel-runway geometry the far side of the crossed runway *is* the next runway's hold line (SFO taxiway C — cross 10R/28L, hold short 28R, ~190 ft apart with one painted bar between), so `FindRunwayCrossingExitNode`'s `requireSameRunwayExit: false` fallback legitimately returns the *destination-runway* bar. Getting this wrong is silent and nasty (issue #315): the aircraft parks on the right line, skips its "holding short runway 28R" report, and LUAW routes to `DepartureClearanceHandler.LineUpFromPosition` instead of `LineUpFromHoldShort`. The same helper is where a departure clearance stored during taxi gets consumed on the crossing paths — `CompleteRoute` is not reached when the crossing slice consumes the remaining segments — so it mirrors `CompleteRoute`'s order exactly: clearance first, terminal phase second, or a route that also names a parking destination silently drops the clearance.
- **The crossing's tail-clearance overshoot must yield to a hold-short at its exit.** `CrossingRunwayPhase` appends a virtual node ½ a fuselage *past* the exit so the tail clears the runway behind it. Two conditions suppress that: a binding taxiway hold-short within a fuselage downstream (`BindingTaxiwayHoldShortWithin`, issue #172) and an uncleared hold-short on the exit node itself (`ExitIsBindingHoldShort`, issue #315). The second is the parallel-runway case above — without it the aircraft ends up ~54 ft *inside* the next runway's holding position markings while announcing that it is holding short of them (AIM 4-3-18.a.6). `ApplyExitHoldShortOffset` then snaps the navigator to the hold-short's nose-at-line coordinates, mirroring `TaxiingPhase.SetupCurrentSegment` — the node itself sits on the painted bar, not a fuselage short of it. Hold-short compliance outranks tail clearance in both cases; tail-over-runway exposure is what `HoldShortPoint.TailOverRunwayNodeId` reports.
- **`LineUpFromPosition` starts the phase list itself, so it must be handed a ground layout.** Every other LUAW/CTO path inserts phases and lets the tick loop start them with a full `PhaseContext`; `LineUpFromPosition` calls `Phases.Start(...)` directly. `LineUpPhase.OnStart` **faults outright** on a null `ctx.GroundLayout`, and the command still returns success — an "OK — line up and wait runway 28R" with an aircraft that never moves. `TryDepartureClearance` therefore takes a required (never-defaulted) `groundLayout` parameter. `CommandDispatcher.BuildMinimalContext`'s `groundLayout` still defaults to `null`; any new caller that starts a *ground* phase has to pass one.
- **A ground phase publishes `TargetSpeed` every `OnTick`, never only in `OnStart`.** Physics is the only integrator of ground speed and it nulls `TargetSpeed` on the snap, and `GroundConflictDetector.IsAtRest` reads a positive `TargetSpeed` as "this aircraft is a mover" (a pinned lining-up aircraft at 0 kt must stay a mover or it creeps through the LUAW occupant ahead, #409). A phase that publishes conditionally is invisible as a mover the first tick after its speed settles, and a hold that pins only in `OnStart` lets a stale positive target push the aircraft forward (`RunwayHoldingPhase` re-publishes 0 per tick for exactly this reason). Roll phases (`TakeoffPhase`, `StopAndGoPhase`, `TouchAndGoPhase`, `RejectedTakeoffPhase`) are the exception: they write IAS directly with `TargetSpeed = null`.
- **Re-inserting a phase re-runs its `OnStart`.** `PhaseList.AdvanceToNext`/`Start`/`SkipTo` always call `OnStart` when a phase becomes current — the only no-`OnStart` path is snapshot restore (`FromSnapshot` sets `CurrentIndex` directly). **A phase whose maneuver lives in live objects the DTO does not carry must therefore rebuild them on its first tick after a restore**: `CrossingRunwayPhase` (`_initialized = false`) and, since 2026-09-08, `LineUpPhase` (`_needsRestoreRebuild`, one predicate that never reaches `Fault`: within a main-gear turn radius of the centreline and within `LineUpGeometry.AlignedMaxTurnDeg` of runway heading → complete; within the radius but still turning → the already-aligned rollout, so the nose swings on while rolling; otherwise re-plan from the restored pose with the same `PlanFromCurrentPose` `OnStart` runs; the restored `RollingMode` decides roll vs hold, and `TryUpgradeToRolling` accepts a `CTO` that lands in the one-tick window before the rebuild). Before that, a restore landing on an active `LineUpPhase` parked the aircraft at 0 kt forever — `OnTick`'s `PathPlan is null` guard went to `TickFaulted` every second (found on the S2-OAK-4 bundle, N152SP); `LineUpPhaseRestoreTests` restores at every second of a real line-up and asserts each reaches `TakeoffPhase` / `LinedUpAndWaitingPhase`. `LandingPhase` closed the same hole 2026-09-09 the other way round: its DTO now carries the whole `LandingPlan` (geometry and category constants), so a restore rebuilds nothing and a snapshot taken before the first tick is lossless; a snapshot written before that change still rebuilds the constants from the category table on the first tick. Any other phase with live-object state and a dead-end `OnTick` has the same exposure; audit before adding one. So `InsertAfterCurrent([maneuver, resume])` re-fires the resume phase's `OnStart` side effects; guard one-shot effects (e.g. `FinalApproachPhase.CloneForResume` sets `_goAroundRolled` so an in-final 360 / S-turn doesn't re-roll the solo go-around RNG — see [approach-and-pattern-geometry.md](approach-and-pattern-geometry.md) footguns).
