...
- **The sync** — the last pre-physics step, so a sample lands at second *t*, is recorded at *t*, and replays pre-tick at *t*.
  It is split at a feed port (tick-path 04e): `SimulationEngine.TickLiveTrafficSync` (Sim) runs the per-track gates, spawn vs
  refresh and the removal tiers below; the room's port, `ShadowTrafficSync.FeedFor(room)` (server), owns everything that needs
  the wall clock or the store — scope, filter matching, the time anchor, DVR store selection and the inert cases — and the
  server's `OnShadowSpawned` / `OnShadowRemoved` / `OnCallsignInUse` / `OnFilteredOut` / `OnReacquired` consumers do the
  broadcasts and terminal lines. Inert while `IsBroadcastSuppressed` (rewind reconstruction) or in tape
...
- **Removal** — tiered, **explicit lifecycle first, silence last, never observation age**: (1) a store row present but
  *viewless* was ended by the feed itself (TAIS `terminated`/`drop`, SFDPS `DROPPED`/`COMPLETED`) → removed promptly
  ...
  (2) a track the store still delivers but outside the room's scope → removed at `SimulationEngine.LiveTrafficOutOfScopeRemovalSeconds`
