...
| `Unassume` | `UNASSUME` | Callsign | Sim: `ActionArms.Unassume` — `SimulationEngine.DeleteAircraft` without the feed suppression a `DEL` records, so the next sync re-spawns the shadow | Text |
...
`DispatchCompound(aircraft, compound, ctx)` is the entry point for non-track, non-coordination, non-strip commands. `Dispatch(command, aircraft, ctx)` is a wrapper that puts one verb in an unconditional block and calls it. A live-traffic shadow (`aircraft.IsShadow`) is **assumed right here before anything else** — `LiveTrafficAssumer.Assume`, then the same compound falls through to the rest of this list, with `{callsign} assumed — ` prefixed onto the one result message — so even a phase-transparent command takes the aircraft rather than bouncing off it. Three things are refused ahead of that gate instead: a flight-plan edit (never dispatched to an aircraft at all), a chain carrying `ASSUME`/`UNASSUME` (`CompoundPolicy.IsNonCompoundable`, the router's wording), and — the one refusal the shadow itself still gives — a compound whose every command is a `SAY*` query, because a read-only question must not take control as a side effect (see [live-traffic.md](live-traffic.md)). It then checks for a leading `WAIT`/`WAITD`/`BEHIND` and short-circuits to a **deferred dispatch** (see [Deferred dispatch](#deferred-dispatch--wait-behind-and-the-command-run-delay)); otherwise the big moves:
