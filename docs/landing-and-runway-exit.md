...
AIM 2-3-3.h.2: the portion of runway behind a displaced threshold "is available for **takeoffs in either direction** and landings from the opposite direction" — but not for landing in that direction. So the two datums split by *who is being measured*:
...
| The 7110.65 §5-7-1.b.4 final window and the "on final" test: `ApproachCommandHandler.IsOnFinal`, `FlightPhysics.AutoCancelSpeedAtFinal`, `SPD`'s 5 NM-final rejection (the last two pass `aircraft.Ground.Layout`), and the `ApplyArrivalSpacing` / `ApplySameRunwayArrivalProtection` passes (the runway's own field's layout, below) | — |
...
**The 5-mile final window is a judgement call.** §5-7-1.b.4 says "a point 5 miles from the runway" and names no datum; the references are otherwise silent. The landing threshold is chosen because the P/CG puts the APPROACH GATE "no closer than 5 miles from the landing threshold", defines THRESHOLD as "the beginning of that portion of the runway usable for landing", and AIM 2-3-3.h.2 makes the pavement behind a displaced threshold unavailable to the arrival. An aircraft crossing the pavement end of KSJC 30L still has 2,537 ft (0.42 NM) to fly, and one over the displaced stretch is on a short final, not past the runway (`FinalApproachLandingDatumTests`). `IsOnFinal` takes the layout as a required parameter; `null` (no ground map for the field) answers from the pavement threshold.

**A measurement about a runway resolves through the runway's own field.** An airborne arrival's `Ground.Layout` is null for a scripted arrival with no destination and for every live-traffic shadow, so the engine passes that compare several arrivals to one runway never read the displacement off the aircraft. `SimulationEngine.ResolveAirportLayout(runway.AirportId)` returns `World.GroundLayout` for the primary airport and the ground data's layout for any other field, and the on-final test and the threshold distance are both taken from that one layout (see [scenario-loading-and-generation.md](scenario-loading-and-generation.md)). A displaced runway at a secondary airport is therefore measured from its landing threshold too; a field with no ground map falls back to the pavement threshold.
...
`InferPreferredExitSide`'s priority order: (1) a per-end `exitDirections` override from the airport sidecar (`Data/ARTCCs/{ARTCC}/Airports/*.json`, via `AirportSidecarCatalog.GetExitDirection`) — needed where the GeoJSON `turnoff` flip for the reciprocal end is wrong (issue #405, KMIA 26R); (2) the GeoJSON-authored `turnoff` for the end (`GroundRunway.TurnoffForEnd`); (3) high-speed-exit count validated by parking proximity; (4) parking proximity; (5) parallel-runway HS inheritance. See the `exitDirections` section in `src/Yaat.Sim/Data/ARTCCs/README.md` for the authoring schema.
...
The pilot's obligation is AIM 4-3-11.b.6 — exit at the first convenient taxiway before the hold-short point, *otherwise* stop and hold at it — and the stopping datum is AIM 2-3-5.a.2: all of the aircraft short of the runway holding position marking.

- **Distance** is along-track from the landing threshold on the runway heading, the same form the exit planner uses for branch points.
- **Stop target** = the point − (`LahsoStopMarginNm` 50 ft + half the airframe length). `Position` is the centroid, so the half-length term is what puts the *nose* short of the marking; the 50 ft covers discrete-tick overshoot only (the hold-short distance already carries the safety-area setback).
- **Speed ceiling** (`ApplyLahsoCeiling`, applied after the exit planner and the freeze-at-current block, before the publish): `targetSpeed = min(targetSpeed, RolloutBraking.MaxEntrySpeedKts(stopDist, planningRate))`, with `planningRate = min(plan.DefaultDecel, ExpediteExitDecelRate)` — the normal rollout rate, so braking is front-loaded and there is no 40 kt coast up to the line. The published decel is `min(max(exit planner's rate, RequiredDecelKtsPerSec(gs, 0, stopDist)), ExpediteExitDecelRate)`: LAHSO never lowers the exit planner's rate and never exceeds the category's expedite rate. A B738 on SFO 28R `LAHSO 1L` (point 5,601 ft from the landing threshold) slows continuously from ~1,100 ft out and stops with its nose 50 ft short.
- **Stop branch** (`TickLahsoStop`, once the stop target is reached): target 0 at `min(firm, expedite)`; past the point itself the rate is the category's expedite rate and the stop logs `LAHSO overrun — stopped N ft past the hold-short point`. At ≤ 0.5 kt `StoppedForLahso` is set and the phase completes; `PhaseRunner` then appends `RunwayHoldingPhase` → `RunwayExitPhase` → `HoldingAfterExitPhase` and clears the target; any phases still queued behind the landing are ended first (`PhaseList.Clear`, Skipped), so the runway hold always runs next. There is no self-initiated go-around after touchdown.
- **Exit before the point wins.** While LAHSO is active `ResolveNextCandidate` (both the planner path and the straight-line fallback) skips any exit whose branch point lies past the stop target (`BranchFitsInsideLahso`) — an instructor-preferred exit included. The handoff to `RunwayExitPhase` needs a committed candidate that phase will honour: a real route (`Path.Count >= 2`) whose hold-short node is not occupied (the same test `RunwayExitPhase.OnStart` applies). `TickHandoff` nulls `Phases.LahsoHoldShort` in the same branch that publishes `ResolvedExit`, so the landing stops being a hold exactly when the exit is committed. An occupied or route-less candidate never hands off: the normal missed-exit path runs, and with nothing left before the point the aircraft stops and holds. A restore into `State.Handoff` without a usable candidate is routed back through `TickRollout` for the same reason.
- **An amended clearance ends it** (AIM 4-3-11.b.5): every go-around (`GoAroundHelper`), a `TG`/`SG`/`LA`/`COPT` that replaces the landing terminator, and `CANCLAND` null the target. `CLAND` restated after `LAHSO` keeps it (same clearance), and so does `CLANDF`.
