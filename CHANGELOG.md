...
### Added
- **Command run delay** — a teaching option that makes aircraft take time to comply with your instructions, simulating the seconds a real pilot needs to read back a clearance and set up the FMC / autopilot. Set a min–max range (seconds) in **Settings > General > Command run delay** or live in the command-bar session flyout; each command then takes effect after a random delay in that range (set both equal for a fixed delay; max 0 disables, the default). You get an immediate "Pilot complying in Ns" acknowledgement so you know the command landed, and the aircraft responds after the delay. Track/coordination commands and frequency changes are never delayed; `WAIT`/`BEHIND` keep their own timing.
...
- Approach clearances and their autocomplete accept the runway number **without the FAA-style leading zero** — `CAPP I8R` resolves like `I08R` across `CAPP`/`JAPP`/`PTAC`.
...
- **`CLRWY`** (alias `CLEARRWY`) — resolve the situation above: pull an aircraft that is holding short of a taxiway with its tail over a runway forward until it is just clear of the runway (half a length past the bars), then hold. It enters the taxiway minimally rather than driving on to the next junction, and clears the "runway not clear" warning. Only valid in that tail-over-runway hold.
- **Per-taxiway turn hints in `TAXI`** — prefix a taxiway with `>` (right) or `<` (left) to tell an aircraft which way to turn onto it, e.g. `TAXI >A B <C D` (right onto A, left onto C). On the first taxiway the hint sets the start direction from where the aircraft is pointed; on later taxiways it prefers the junction whose turn matches. It is a best-effort preference — when only the other direction is reachable the aircraft still taxis there and the TAXI echo notes the unhonored turn, and an unprefixed taxiway keeps the router's own choice. (The same glyphs still mean face/tail in `PUSH`.)
- Aircraft now read back taxi clearances aloud — previously a `TAXI` clearance produced no pilot readback. The pilot voices the route ("taxi via bravo, charlie"), any hold-short ("hold short of runway two eight right"), runway crossings, the destination runway, and the controller's turn hints ("right on bravo" / "left on charlie").
- Generated arrivals pick their airline in proportion to each airport's **actual carrier traffic** — a dominant carrier (e.g. Southwest at Oakland) now appears about as often as it really arrives, and carriers with only a handful of yearly flights stay rare. The previous square-root weighting over-represented that long tail.

### Fixed
- Arrival generators now sustain a realistic in-trail stream paced by their configured time interval, instead of emitting a long line of aircraft. Each arrival enters behind the last one on final, no closer than the configured in-trail spacing or the 7110.65 Table 5-5-2 wake-turbulence minimum (whichever is greater) and never beyond the generator's maximum distance — so a slow interval trickles arrivals in near the initial distance, a fast one fills the final back toward the cap and then waits for room rather than stacking aircraft up.
- Arrival generators set to the **SmallPlus** weight category now spawn the regional/commuter feed — regional jets (CRJ700, E175, E145) and commuter turboprops (ATR 72, Dash 8, Saab 340) — instead of being treated as Large and emitting mainline narrow-bodies. Large generators now produce mainline narrow-bodies only (737/A320 family). This matches the vNAS scenario weight categories and stops a SmallPlus generator on a short runway (e.g. Oakland's 5,448 ft runway 28R) from ever spawning a 737/A321 that couldn't land there. The `ADD` command gains an `S+` token for the same tier (e.g. `ADD I S+ J 28R 8`).
- A generator's **Randomize weight** option now produces a realistic arrival mix matched to the generator's engine type, instead of a fixed 15%-small / 85%-large roll that never included regionals and could roll a "heavy turboprop." A jet generator rolls mostly mainline narrow-bodies with a strong regional-jet share, a modest widebody share, and the occasional bizjet; a turboprop generator rolls only commuter and light turboprops; a piston generator stays light. In the generator editor the weight dropdown is now disabled while Randomize weight is checked, and the checkbox label says it overrides the selection — so it's clear the picked category is ignored (previously the label misreported the split and the dropdown stayed editable).
- The arrival-generator editor's **Runway** field is now an editable box with suggestions instead of a fixed dropdown, and it always lists the runways already configured on the loaded generators — not just the ones from the airport ground layout. Previously, if the editor opened before the ground layout finished loading, the runway dropdown was empty so you couldn't see or pick a generator's existing runway; you can now also type a runway directly.
- Aircraft from an arrival generator that has an **AutoTrack** configuration now spawn already owned by the configured position with the scratchpad set, and hand off to the student after the configured delay — exactly like scenario-loaded aircraft. Previously the generator's AutoTrack settings were ignored, so generated arrivals appeared untracked (no owner, scratchpad, or handoff). The owner is in the aircraft's first datablock (no untracked flash), the handoff waits until the student's position is staffed, and replaying a recording reproduces the owner and scratchpad.
- Arrival generators in published vNAS scenarios that don't set a **max time** now keep producing traffic for the whole session instead of silently stopping after an hour, and generators that don't set an **in-trail distance** now space arrivals at the radar/wake minimum instead of a fabricated 5 NM. Most scenarios leave one or both of these fields blank, so their arrival streams previously dried up early or were spaced wider than the scenario authored.
- A scenario aircraft handed off with an inherited datablock now carries its **interim and cleared altitudes on the ERAM datablock** (interim → interim altitude, cleared → assigned altitude). Previously the interim altitude was dropped entirely and only the cleared altitude was shown, on STARS. The STARS datablock no longer shows these scenario-inherited altitudes for now, pending confirmation of how terminal (non-ERAM) scenarios use them — the altitudes never affect what the aircraft flies, only the datablock display.
- Generated arrivals no longer spawn under **defunct or foreign airlines mislabeled onto US airports** — e.g. SalamAir at Oakland, or Corendon/Shanxi and Electra elsewhere. The airport-airline data is built from BTS flight statistics through a carrier-code crosswalk that reused two-letter codes across defunct and current airlines and silently took the last match, relabeling real US operators as unrelated foreign carriers. The crosswalk now trusts a defunct airline's code only when its name matches the carrier that would actually be displayed, and prefers a known carrier over one the trainer doesn't model — so code `8C` resolves to Air Transport International (not Shanxi Airlines), and real international carriers (Asiana, Singapore, Nippon Cargo) now appear at the gateways they actually serve.
- Wake-turbulence separation on final now uses the FAA's per-type CWT (Consolidated Wake Turbulence) mile-based minima (7110.65 Table 5-5-2) rather than a coarse four-class approximation, so ATPA in-trail spacing is correct for each specific pair — tighter behind a lower-category heavy, and properly applied behind regional jets and the heavy widebodies that were previously mis-bucketed. Reduced 2.5 NM final spacing now still applies the wake minimum where one is required instead of dropping it.
- Rewinding or replaying the timeline now preserves active **TIMER** countdowns and **hold-for-release** state, and reproduces departure-release (`REL`) timing exactly. Previously a rewind silently dropped running timers and the set of airports armed for hold-for-release, and replaying a release re-rolled its spawn delay — which also shifted the timing of later auto-generated traffic from that point on.
- Aircraft flying a charted SID with heading legs — like the **LINDZ ONE** out of Aspen (KASE) — now fly the published procedure: climb on the charted heading to the level-off altitude, make the charted (e.g. climbing left) turn, then intercept and track the published course to the fix, instead of turning direct to the first fix at 400 ft AGL. SIDs coded with ARINC-424 heading-to-altitude, heading-to-intercept, and course-to-fix legs are flown as charted.
- Arrivals on a STAR that ends with a "fly the published course, expect vectors" leg — the way most US STARs terminate — now fly that published outbound course at the final fix and hold it awaiting vectors, instead of maintaining whatever heading they happened to arrive on.
- Playing a recording past a mid-session weather change or flight-plan amendment no longer hangs the session or ends playback early; rewinding no longer duplicates those actions in the recording.
...
- With no weather profile loaded, the vStrips browser METAR bar and the radar/ground per-airport winds-and-altimeters overlay now show the standard default report (calm wind, 10SM, clear, 29.92) that the sim is actually applying — the same default the desktop METAR panel already showed — instead of staying blank. The browser bar still follows the facility switcher and re-scopes to each facility's airports.
...
- A helicopter told to air-taxi or land at a spot (`ATXI`/`LAND @spot`) now actually turns toward the spot, flies to it, and sets down on it. Previously, if the helicopter had been given a heading or altitude first (e.g. `FH`/`CM`), the air-taxi flew straight past the spot on the old heading and held that altitude instead of descending — it never reached the pad.
- An airborne helicopter mid-air-taxi (or on final to a spot via `LAND`) can now be redirected with a normal airborne command — `FH`, a turn, `CM`/`DM`, `SPD`, or `DCT` — which drops the relocation and flies the new clearance. `HPP` hovers it in place (re-issue `ATXI`/`LAND` to continue). Previously every airborne command was rejected while the helicopter was air-taxiing.
- Two aircraft whose taxi routes cross at a junction no longer both freeze into a mutual stop (a deadlock that could strand both for the rest of the session); one holds and the other proceeds, as in real ground sequencing. A genuine nose-to-nose head-on on the same path still stops both.
- A taxiing aircraft approaching a junction that faster traffic will clear well before it arrives is no longer braked toward a stop to "yield" for that already-clearing crossing; the automatic ground slowdown now only applies when the two aircraft would genuinely reach the shared point together.
- Naming the taxiway an aircraft is already on (e.g. just after a pushback) as the first taxiway in a clearance no longer rejects it as "unreachable" — `TAXI M4 M2 …` from M4 is now accepted instead of failing when the start position resolved onto an adjacent parallel taxiway.
- Re-routing an aircraft holding at the dead-end of a short connector or spot no longer rejects the clearance as "transition infeasible" when starting on that connector can't reach the next taxiway — e.g. `TAXI B B1 …` to an aircraft parked at the end of the B5 spot; the route now bridges onto the first cleared taxiway the productive way instead.
- A taxi clearance ending on a taxiway with no destination (e.g. `TAXI G B`) now stops at that taxiway's intersection instead of guessing a direction and sometimes turning the wrong way; turn the aircraft either way on it with a follow-up taxi. A clearance that holds short of a runway on its final taxiway (e.g. `TAXI B K HS 10R`) now heads toward that hold-short instead of away from it.
- A taxi clearance that holds short of a taxiway it also taxis along (e.g. `TAXI G B HS B`) no longer echoes the hold-short repeated dozens of times (`G B HS B HS B HS B …`); the route lists each taxiway hold-short once.
- An aircraft told to hold short of a taxiway right after crossing a runway, where the taxiway sits closer than the aircraft's own length past the runway (e.g. `TAXI G B HS B` across 01L/19R at SFO), no longer spins ~180° and drives back toward the runway to reach the hold-short. It now stops at the taxiway hold line as it arrives — nose at the line, tail still over the runway — without backtracking.
- `TAXI <twy> CROSS <rwy>` with no taxiway named past the crossing now uses the crossed runway to set the taxi direction: the aircraft heads toward and across that runway and holds just past it. Previously, when the taxiway crosses more than one runway (e.g. `TAXI G CROSS 28R` from a plane that just exited the parallel 28L), the route could resolve the wrong way — back across the runway behind it.
...
- The aircraft-list **Hdg** column now reads in magnetic, matching the **AHdg** (assigned heading) column and what you'd read off a heading bug. It previously showed true heading, so it could differ from AHdg by the local magnetic variation (~13° in the Bay Area) even when the aircraft was flying its assigned heading. All headings shown in the UI — the Hdg/AHdg columns, the aircraft-list status text, and the radar right-click heading menu — are now displayed as 3-digit numbers (`090`, `005`, `360`) instead of `90`/`5`. The pilot's spoken heading readback (`SHDG` / "say heading") now reports magnetic too, instead of the aircraft's true heading.
- Loading a scenario from the server catalog (the **Load Scenario → ARTCC** tab, or a recent server scenario) now prompts you to pick a difficulty when the scenario offers more than one — and, in solo-training mode, shows the same parking/arrival/go-around pacing controls — exactly like loading a local file. Previously the catalog path skipped the setup prompt entirely and silently loaded every aircraft at the hardest difficulty with default pacing.
- Arrivals cleared for a **single-digit runway** (e.g. runway **2** at KMSY) no longer get set up on the opposite runway end, then overrun and stick on the runway. The designator wasn't zero-padded to match the airport's runway data (`2` vs `02`), so an aircraft cleared for runway 2 was placed on runway 20's approach and could never find a taxiway exit to turn off. Two-digit runways (e.g. 11) were unaffected.
- Departures with on-handoff or at-altitude presets (`ONHO`, `AT`) now auto-taxi instead of staying parked; queued conditional commands no longer cancel each other — only a new immediate command clears them.
- `JFAC`/`JLOC` (join the localizer / final approach course) is now a lateral-only vector: the aircraft turns to intercept and tracks the localizer while **holding its assigned altitude and speed**, and only descends on the glideslope once you issue `CAPP`. Previously it behaved like an approach clearance — it would descend to the runway and land without ever being cleared, and could roll out slightly off the localizer. It now tracks the localizer accurately and stays level until cleared.
- `JFAC`/`JLOC` now **joins the localizer from any vector instead of blowing through it**. Armed on a heading (e.g. `FH 220, JLOC`), the aircraft keeps flying its vector and turns onto the localizer when it reaches it, at whatever cut the vector gives — a steep or late intercept is flown as a join (correcting back onto centerline if it overshoots) rather than sailing straight through and reporting "unable, passing through localizer." Joining is a relaxed instruction the pilot completes on their own; only a forced `PTAC` expects a controller-set intercept angle. It still holds the assigned altitude until `CAPP` and only descends once established on the localizer.
- Issuing `CAPP` to an aircraft already established on a `JFAC`/`JLOC` join now **authorizes the descent in place** — it clears the aircraft to descend on the same approach instead of cancelling the join and re-clearing it. The spurious "… cancelled by CAPP" terminal warning (reported as "pattern to RWY 18L cancelled by CAPP") no longer appears for the normal join-then-clear sequence.
...
- An aircraft vectored to an ILS no longer **starts down the glideslope before it is established on the localizer**. On a steep or close intercept it now holds the assigned altitude until it is tracking within ~5° of the final approach course and on centerline, then captures the glideslope and descends — matching "maintain until established on the localizer, cleared ILS." Previously a steep intercept could begin the descent while still 10–30° off the localizer and slewing onto course. Pattern traffic, visual approaches, and forced (PTAC) intercepts are unaffected.
