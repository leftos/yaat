...
- vTDLS pre-departure clearance: a **vTDLS** tab next to **Strips** in YAAT Client, plus a `/vtdls/` browser app on yaat-server. DCL Pending PDCs auto-queue the moment a flight plan is filed at a TDLS facility — the list populates immediately on scenario load without waiting for the sim to tick. The tab auto-binds to the scenario's primary airport (e.g. OAK for an OAK scenario), with a Facility Menu to switch and a Dark Mode toggle that persists per user. Nine-field flight-plan editor with mandatory-field gating; Send (F12) emits an RPO terminal entry on a dedicated amber **TDLS** channel that mirrors the ACARS PDC text the pilot's client actually receives.
- Multi-facility vTDLS: **View → vTDLS → New vTDLS Tab…** opens additional TDLS facilities in their own tabs (useful for top-down TRACON ops like NCT with OAK/SFO/SJC/SMF/RNO). Tabs pop out with per-facility geometry persistence.
