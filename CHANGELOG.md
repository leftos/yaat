...
- Speech recognition now understands "turn left/right (taxiway)" and "if able turn left/right (taxiway)" runway-exit instructions (FAA 7110.65 §3-10-9). The captured taxiway name is sanity-checked (length/charset) and validated against the scenario's taxiway list when populated, so Whisper transcribing "heading" as "hitting" still falls through to the partial-fallback path.
