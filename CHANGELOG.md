...
### Changed
- The PDC lifecycle (auto-queue on spawn, send, the three-second auto-WILCO, the two-hour expiry, removal on track, `TDLSOPS`) is simulation logic now, so replaying a recording in the client rebuilds the DCL and PDC lists as the live session had them instead of leaving them empty.
