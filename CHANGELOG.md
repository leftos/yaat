...
### Added
- Solo-training mode now hears airborne aircraft check in on first contact. An aircraft that spawns (or is handed off) airborne announces itself once on the student's frequency: IFR aircraft give callsign and altitude (or flight level above FL180); VFR aircraft give position-from-field plus intent ("inbound for landing", "request transition", or AIM 4-3-1's "VFR southbound at one thousand five hundred, four miles north of the field"). Templates branch on student position (TWR / APP / CTR; airborne aircraft skip a GND student). No effect outside solo-training mode.

### Fixed
- A student joining a room and activating their position now appears in the instructor's CRC controller list. Previously the controller list snapshot the instructor first received stayed stale — connecting, activating, or deactivating a position didn't push a delta, so the student stayed invisible until the instructor disconnected and reconnected. Same fix covers peers seeing each other's mid-session activate/deactivate state changes.
- CRC clients waiting in the lobby (connected but not yet pulled into a training room) no longer receive any controller, position, or consolidation data from the server. Previously, an unbound CRC student subscribing to `OpenPositions` saw every position across every active room on the server, and a `StarsConsolidation` subscription returned the facility hierarchy for any ARTCC. Both initial-data-on-subscribe and the periodic broadcasts now skip unbound clients; the moment the instructor pulls a student into a room, the held-back data is delivered.
- `CTOC` (cancel takeoff clearance) now works when issued after a `CTO` given mid-taxi. Previously, clearing an aircraft for takeoff while it was still taxiing to the runway stored the clearance for later (so the aircraft would line up and roll on arrival), but a subsequent `CTOC` returned "No takeoff clearance to cancel" even though the aircraft had read back the clearance. `CTOC` now revokes the stored clearance and reinstates the destination runway hold-short — the aircraft will reach the threshold and stop. Runway crossings the controller had already cleared via `LV`/`RC`/`CROSS` before the `CTO` are left untouched.
- `CTOC` also works while the aircraft is taxiing into position on the runway after a `CTO` (the line-up phase). Previously the same "No takeoff clearance to cancel" rejection fired during this brief window even though takeoff hadn't started. Now `CTOC` reverts the line-up: a rolling-takeoff aircraft brakes to a stop on the runway and holds; a non-rolling aircraft (heavy/super) completes the line-up and then holds. Either way, a fresh `CTO` is needed to actually depart.
- ASDE-X tracks now show CONUS destination airports in FAA form (e.g. `SFO`) instead of ICAO (`KSFO`), matching real CRC. ARTCC `asdexConfiguration` fix-rule patterns also accept either form — a `SFO` config pattern now matches a `KSFO` flight plan and vice versa.
...
- `SCAN` — copy a flight strip into an external facility's bay while keeping your own copy. Right-click a full strip in YAAT Flight Strips → "Scan to" lists every linked external bay; wire form is `SCAN <bay>[/<rack>[/<index>]]` (slash-compound 1-based, same as `STRIP`). Useful for handoff coordination: a tower can scan a strip to the parent TRACON's bay so the receiving controller sees it, but the tower still owns the working copy. Each scan creates a fresh copy — repeated scans stack rather than dedupe.
...
- The standalone command cheatsheet at `docs/command-cheatsheet.html` got a major UX overhaul: collapsible categories whose open/closed state is remembered between visits, role profiles (Ground+Clearance, Local, Approach, All) that preset which categories to show, a smarter prefix-aware filter, keyboard navigation (`/` focuses the search, arrows walk matches, `Enter` copies the verb to the clipboard), and new rows for previously-missing commands (PORJ, PORT, CAACK, CAINH, GHOST, LDR, JRING, CONE, HSC, HSA, HSD, PRA, CIFR, TRATE) plus filled-in CTO modes and aliases.
...
- Dragging a flight strip to a new bay/rack no longer flashes the strip back to its original slot before snapping to the destination. The strip now lands instantly when you release the drag, and the server's broadcast reconciles silently in the background.
- Right-click commands on **empty** half-strips (annotate, move, swap, delete) now work. Previously the strip view emitted the strip's internal id as the lookup key (since an empty strip has no first-line text), the parser mistook it for a bay name, and the command failed silently.
- YAAT Flight Strips now locks down its UI while disconnected from the server. Bays clear their strip contents (the bay layout itself stays put so you don't lose your arrangement), a red banner overlays the workspace, and right-click menus, drag-drop, keyboard shortcuts, and the printer modal all noop until you reconnect — previously the UI looked active but actions silently failed.
