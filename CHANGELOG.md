...
- Speech recognition now understands cross-fix altitude restrictions: "cross CEPIN at five thousand", "at or above five thousand", "at or below five thousand", "at and maintain five thousand", and the §5-7 alt+speed form "at and maintain five thousand at two five zero knots". Pilot AI also reads these back with the matching modifier. Flight-level forms work via the existing altitude normalizer. Compound "cross fix … cleared (type) approach" instructions chain automatically through the greedy multi-clause matcher.
...
- Speech recognition now understands named SID/STAR phraseology: "descend via the (STAR) arrival [, (transition) transition]", "(STAR) arrival" / "cleared (STAR) arrival", and "climb via the (SID) departure [except maintain (alt)]" (FAA 7110.65 §4-5, §4-7, §5-2, §5-5, AIM §5-4). Multi-token spoken procedure names ("eagul five") collapse to the canonical CIFP ID ("EAGUL5") via a new fuzzy-matching normalizer that uses the existing PhoneticFixMatcher — so Whisper transcription variants like "eagle five" or "egal five" still resolve. Procedure scope is the active aircraft's filed departure + destination airports, plus the loaded ground layout's primary airport.
- Speech recognition now understands "maintain (speed) until (fix)" (FAA 7110.65 §5-7) and "cruise (altitude)" (FAA 7110.65 §4-5, §6-6, AIM §4-4). The speed-until-fix form chains via the existing `SPD X UNTIL Y` shorthand to `SPD X; AT Y RNS`. Typed `CRUISE 5000` and spoken "cruise five thousand" now both round-trip to the same canonical altitude (the parser accepts either shorthand-hundreds or literal-feet input). Same change applies to `TAL` (temporary altitude) and pilot-reported altitude.
- Speech recognition now understands "turn left/right (taxiway)" and "if able turn left/right (taxiway)" runway-exit instructions (FAA 7110.65 §3-10-9), plus a wider family of informal exit phrasings — "exit left when able", "exit right on delta if able", "exit left to alpha when able", and the equivalent forms with "if able" or "when able" appearing either as a leading preamble or between the verb and the taxiway. The captured taxiway name is sanity-checked and validated against the scenario's taxiway list when populated, so Whisper transcribing "heading" as "hitting" still falls through to the partial-fallback path.
