...
- Taxiing from a standstill accelerates at a realistic breakaway rate (1.0 kt/s for jets and turboprops, 0.6 for pistons and helicopters) instead of 5 kt/s: the navigator and physics were each integrating speed toward the same target, and the navigator's nominal 3 kt/s was never what the aircraft did. Physics is now the only integrator of ground speed (pushback and runway-exit re-acceleration use the same taxi rates), braking curves execute at the rate they were planned with, and speed no longer jumps by up to 2 kt when it nears its target on the ground. An aircraft told to follow another closes up to its stop gap at walking pace behind a stopped lead instead of freezing at the follow distance.
- A lining-up aircraft pinned behind a lined-up-and-waiting one no longer creeps toward it one sub-tick in four: the ground conflict detector treats an aircraft commanding forward speed as a mover even while a stop has it at 0 kt.
...
- Two STARS facilities that share a tower P-list id (ZOA's FAT and NCT both define `P1`) keep separate lists: the coordination lists no longer re-broadcast every second on every ZOA room, dwell order is stable, and snapshots no longer differ each second.
...
### Changed
- Coordination (`RD`/`RDH`/`RDR`/`RDACK`/`RDDEL`/`RDPOS`/`RDTXT`/`RDAUTO`, the release-expiry timers and the removal on `TRACK`) is simulation logic now: a replay, a rewind or a bug bundle rebuilds the STARS coordination lists as the live session had them, under the same item ids, and a rewind re-syncs CRC's lists when it lands.
- The STARS tower lists keep each aircraft's entry time through a rewind, a bug bundle and a session restore, so the list order matches the live session.
- The PDC lifecycle (auto-queue on spawn, send, the three-second auto-WILCO, the two-hour expiry, removal on track, `TDLSOPS`) and the flight-strip logic (every strip verb, the auto-printed departure and arrival strips, the preset and deferred strip commands, the amendment reprint) are simulation logic now, so replaying a recording in the client rebuilds the strips, DCL and PDC lists as the live session had them instead of leaving them empty.
...
- A live session pre-selects the airport the chosen position names (`OAK_APP` → OAK, `SFO_B_APP` → SFO) instead of the facility's first STARS-area airport, which put NCT positions on SJC.
- The Favorites panel switches to the **Ground** tab when a ground aircraft is selected and to **Air** for an airborne one (Vehicle and Airport stay manual), and its buttons sit edge to edge in the grid, filling from the top-left, instead of floating in oversized cells.
- With a pilot-reaction delay set, a new immediate instruction cancels a pending `WAIT`/`BEHIND` at once; the superseded `WAIT` no longer fires later and overrides the newer heading, altitude or speed.
- A coordination message with a `,` or `;` in it (`RDTXT /1 HOLD, GO`, `RDH 1 EXPECT DELAY; CALL`) is sent whole: the text used to be cut at the separator and the tail dispatched to the aircraft as a command whenever it happened to parse as one (`GO`, `HDG 280`). The message runs to the end of the line, also when it follows another chained command (`HO 3G; RDTXT /1 HOLD, GO`). Scratchpad, `AN` and half-strip text still chain on `,`/`;`.
- `R270`, `L270`, `R360` and `L360` are refused for an aircraft on the ground instead of wedging it: `CTO, R270` used to arm an airborne 270 ahead of the takeoff roll, the aircraft never moved, and `CTO`/`CTOC` were refused until a `WARPG`. The paired form is now refused before anything applies, with a pointer to `CTO MR270` / `CTO ML270` (a 360 has no departure form; clear for takeoff and issue it once airborne); `CTO; R270` still queues the turn until airborne.
- A chain led by a pattern modifier (`EXT DOWNWIND; CLAND 28R`) issued after a queued pattern entry (`AT FIX ERD 28R`) is no longer refused with "No upcoming downwind leg to extend" when the queue is only partly superseded (a pilot reaction delay, or an incoming command on a single axis): the pre-dispatch validation dropped the queued entry from its working copy before checking for it.
- A bare `FOLLOWF` typed while a traffic call is still pending is no longer refused with "Unable, say traffic callsign": the pre-dispatch validation ran on a copy of the aircraft that had no record of the pending call.
- A chained surface clearance (`TAXI D; CROSS 28L`, `PUSH; TAXI …`, an exit instruction in a chain) no longer cancels the aircraft's queued altitude, speed and heading instructions — a pre-armed departure turn or altitude survives the taxi phase (7110.65 §3-7-2 taxi clearances never amend the departure clearance). `GO`, `CTOPP`, `APT`, `FOLLOW` and helicopter `LAND` no longer clear the whole command queue when issued.
- `RWY` to an airborne aircraft no longer clears its queued altitude and speed instructions (it re-assigns the runway and cancels only queued lateral plans), applies to an aircraft that is not yet in a phase instead of answering "Unable to Assign runway", and a `TAXI` or `TAXIAUTO` rejected because the aircraft is airborne no longer empties the command queue on its way out.
...
- Replaying a recording in the client runs delayed handoffs, auto-accept and the point-out timeout, so track ownership follows the live session between snapshots instead of snapping to it at each restore.
- Rewinding or exporting a bundle reproduces which positions a CRC client was working at each second, so auto-accept, point-out withdrawal, delayed handoffs and consolidations decide as they did live instead of as if nobody (a bundle) or today's clients (a rewind) were signed on.
...
### Highlights
- Simulated arrivals go around on short final when another aircraft is on their runway — new session setting, on by default.
- `HOLD` stops a taxiing aircraft, and commands sequenced after `PUSH` or a hold-short taxi now execute. (#407)
- Free-form datablock deconfliction (DCNF F) settles dense clusters of parked aircraft into a stable, readable layout. (#406)
- In development: live real-world traffic (FAA SWIM) and live sessions — internal testing only, not yet enabled on the public server.
...
- **Work in progress — internal testing only; not yet enabled on the public server.** Session flyout gains a **Live Traffic (SWIM)** toggle (shown only on servers that enable it) with a server-side altitude ceiling; `WARP` above 1× is refused while it is on. With it on, real aircraft inside the room's airspace (the tower's or TRACON's airports and Class B/C, or the center's boundary) appear as read-only shadow targets that follow the feed, coast when it drops out, and disappear when they leave; `DEL` hides one. Shadows draw with a dashed position symbol (an outline silhouette on the Ground View) and fade while coasting; their right-click menus offer **Assume control** / **Assume and track** (`ASSUME`) to take one over as a simulated aircraft, plus the usual track, coordination and display items — everything else is refused until assumed. The status bar shows the feed's health (`LIVE · 42 tracks · 3 s`) while the toggle is on; right-click it to show, hide, or show only live traffic in the Aircraft List. The sim-rate picker locks at 1× while live traffic is on. Recordings and bug bundles replay live traffic exactly as it was seen, and rewinding keeps the shadows in step. Servers that enable it feed the shadows from the FAA SWIM data service — terminal, en-route and surface surveillance nationwide, with flight-plan details filled in — and keep a rolling window of the raw feed for replaying reported problems; the self-hosting guide covers the subscription and settings. Aircraft on the FAA's LADD privacy list are filtered out before anything sees them, and a server refuses to run the feed at all without a current list. Bug bundles record the feed's health over the session and, for every shadow sample, which facility produced it and when, so a report can be traced back to the raw feed. Center rooms scope live traffic by the FAA's published ARTCC boundaries, and exported bundles pseudonymise any aircraft that is on the LADD list at export time.
- **Work in progress — internal testing only; not yet enabled on the public server.** **Scenario > Start Live Session...** opens a room on real traffic with no authored scenario: pick the facility and position to work, the airport the Ground View shows (a tower offers itself, a TRACON its towers, a center every tower — your last choice is remembered) and an optional ceiling; the room opens at that position with live traffic on, the clock running and the current live weather loaded. A live session shows a `LIVE` / `PAUSED` / `PLAYBACK` badge by the timeline (`LIVE · feed lost` when the server loses the feed) and a **Go Live** button that drops back into real time from a pause or a rewind; reaching the end of a rewound tape runs on into live traffic instead of pausing. Pausing a live session works like a DVR on servers that keep a feed log: resume continues from the moment you paused, behind real time (`LIVE −02:00`); click the badge to scrub anywhere in the server's feed log (up to about a day back) or **Go Live** to rejoin now, and **Start Live Session...** can open a session at a past UTC time.
...
### Highlights
- Taxiing onto a parallel ramp taxilane the ground map doesn't connect (SFO Terminal 1 M3/M4/M5) now cuts across the apron and continues as cleared instead of dropping the taxiway with an "unable via" warning. (#396)
...
### Highlights
- Clicking any YAAT window brings all YAAT windows to the front together, like CRC (toggleable in Settings, on by default).
...
### Highlights
- Quitting no longer hangs the app when the push-to-talk keyboard hook gets stuck shutting down.
- Closing a pop-out window by its title-bar X no longer risks a freeze.
- Server operators can force-close a stuck training room without restarting the server.
...
## v0.12.1-beta [2026/08/10]

### Highlights
- `FOLLOWG` now works from parking, mid-taxi, and any holding state — a parked aircraft starts up and falls in behind the leader with no `TAXI` route needed, and the ground right-click menu offers **Follow...** on those aircraft too.
...
- A visual-approach follower is judged against one weather picture: the traffic-in-sight checks that run every second now use the nearest reporting station to the aircraft (like the follow logic already did) instead of the destination airport's METAR, ending the spurious "lost sight of the traffic" / "traffic in sight" flapping when the two disagreed in multi-airport scenarios.
- Losing sight on a visual approach now has real consequences (AIM §5-5-11.a.3 — the pilot must always have either the airport or the preceding traffic in sight). Losing the lead while still holding the field cancels the follow — without freezing the pattern leg — with "lost sight of the traffic, field in sight" and a radar-separation-required warning for the instructor; the visual itself continues. Losing the only reference ends the approach: on short final the pilot goes around, speaking the reason; farther out the pilot levels off, holds present heading, and asks for vectors. Either way the clearance is voided, so a passing weather flicker can no longer silently resume an approach the controller believes is dead — a fresh `CVA` is required.
- A follower that loses the field while still watching its traffic stays quietly on the visual (the field report was never required on a `FOLLOW` clearance) instead of transmitting "lost sight of the field".
- Issuing `CVA` no longer erases the field/traffic report it was predicated on, so the pilot stops re-reporting "field in sight" seconds after every visual approach clearance.
- A go-around on a visual approach — automatic or via `GA` — now voids the visual clearance, so the climb-out no longer silently re-reports the field against a dead approach.
- The legally-clearable 1000 ft / 3 SM visual approach is now flyable: surface visibility caps field acquisition as a slant-range envelope (more of the field is visible from above a haze layer, per 7110.65 §7-5-7's flight-vs-ground-visibility note) instead of a hard 2.6 nm ceiling that made the field unreportable before the aircraft was practically at the threshold. Point-traffic spotting keeps the strict surface figure.
- `CVA` enforces the 7110.65 §7-4-3.b weather minimums: rejected below a 1000 ft ceiling or 3 SM visibility (using the destination's METAR, or area weather from nearby stations when the field has none), and a wide-angle join is rejected when the ceiling would put its 2000 ft AGL downwind inside the deck ("vector for a closer join"). A scenario with no weather still allows the clearance, and `CVAF` bypasses the gates.
...
- A typed `<condition> WAIT n <command>` compound (e.g. `AT TTE WAIT 170 DM 110`, `LV 050 WAIT 5 DM 110`, `ONHO WAIT 30 CM 360`) fires its delayed command n seconds after the condition again. Previously the delayed command lost its condition on the way to the server and only fired once the aircraft's current instruction finished — on a STAR arrival, never.
...
### Highlights
- `.rbl A B` (or `*T A B`) draws a measurement line between two typed points — fixes, FRDs, or callsigns in any combination, with a callsign endpoint latching to the aircraft.
- Measurement lines belong to the view they were drawn in — a radar measurement no longer shows on the ground view; numbering stays shared.
- A measurement with an off-screen endpoint keeps its reading visible where the line exits the screen.
...
- A general-aviation callsign whose spoken suffix letter the speech recognizer garbles beyond recognition (for example "november 9225 lima" heard as "november 9225 lien") now resolves to the right aircraft when exactly one active callsign matches the heard digits with 1-2 more trailing letters. Previously the command went to an unknown "N9225" and had to be re-addressed by hand.
...
- The vTDLS footer status updates as you change a dropdown instead of lagging up to a second behind, and names only the fields still blank.
- The vTDLS footer status is legible in light mode instead of rendering white on a near-white bar.
...
- A CRC Tower Cab now only shows traffic within range of its own field instead of every cab in the ARTCC receiving the same aircraft, and altitudes read against its own field elevation.
- **Load Live Weather** now fetches real-world METARs for your ARTCC's airports instead of reporting "No airports found for ARTCC".
...
- Clearing a light aircraft to a different runway during a low approach (e.g. `LA` then `CLAND 33`) flies the low pass, then turns onto that runway to land.
