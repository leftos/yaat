...
- vTDLS pre-departure clearance: a **vTDLS** tab next to **Strips** in YAAT Client, plus a `/vtdls/` browser app on yaat-server. Shows DCL Pending PDCs and PDC Sent/Wilco'd lists, with a nine-field flight-plan editor (mandatory-field gating) and Send/Dump (F12 / F4). Each Send emits an RPO terminal entry.
