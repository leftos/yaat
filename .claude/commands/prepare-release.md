...
8. **Ask how to push.** Pushing causes no downtime — the room-occupancy check and the deploy
   ...
   `release.yml` **always** creates the GitHub Release as a *draft* — CI cannot judge deployment
   scope (a release can be server-affecting purely via yaat-server, which the workflow can't see),
   so publishing is never decided there. Something in this flow must publish the draft, or the
