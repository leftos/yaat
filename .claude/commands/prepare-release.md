...
8. **Check the target server for active rooms, then ask how to push and deploy.**
   ...
   **If Step 7 was SERVER_AFFECTING** — lead with deploying (this release changes what the server runs):
   ...
   - **Push only** — push the release, no droplet deploy (leaves the live server on the previous build — only pick this if you'll deploy separately). **Warn explicitly:** on a SERVER_AFFECTING release, `release.yml` creates the GitHub Release as a *draft* (draft-until-deployed), so the release stays invisible to users and to auto-update until a deploy publishes it — or until it's published manually with `gh release edit v{version} --repo leftos/yaat --draft=false`.
   - **Abort** — stop here; the user resumes manually.
