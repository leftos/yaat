...
**Departure-queue numbers.** When two or more departures are stacked at the same runway hold short, each shows its place in line as a **#1 / #2 / …** suffix on its ground datablock callsign (`UAL123 #2`), so you can tell which aircraft is which when a clump is bunched at the hold short and you're zoomed out. The Aircraft List Info column says the same thing in words — *"taxi to RWY 28R (#2)"*, *"holding short 28R (#1)"* — instead of a bare *"taxi to runway"*. The aircraft holding short at the front is #1; those still taxiing up behind it (within about half a mile) fill in #2, #3, and so on in order of distance. Each hold-short intersection is its own line, so a runway fed from two intersections can show a #1 at each. Once the front aircraft is told to line up and wait it leaves the line and the next aircraft becomes #1, and a lone departure with no one behind it shows no number.
