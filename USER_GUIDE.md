...
- **Live, paused, playback** — a live session's badge at the right of the timeline reads `LIVE` while the room follows real time, `PAUSED` while paused (the shadows freeze while the real aircraft fly on without you), and `PLAYBACK` after you rewind (the room's own recording plays back; real traffic replays exactly as it was seen). A red `LIVE · feed lost` means the server has lost the feed. **Go Live** (shown while paused or in playback) drops back into real time at once, and when a playback reaches the end of the tape a live session simply runs on into live traffic instead of pausing. Rejoining real time after more than a few seconds works like a radar outage: every shadow is dropped and re-acquired from the feed where the aircraft really are, and the terminal says how long the room was away (`live traffic rejoined — real traffic moved on 02:00; 14 shadows re-acquired from the feed`). Aircraft you had assumed keep flying from where they were, so expect them to be out of step with the real traffic around them by that much.
