...
public static class UsageText
{
    public static void Print()
    {
        ...
        Console.WriteLine("  --node-depth N           Also include every node within N graph hops of each --node seed");
        Console.WriteLine("  --exits <designator>[,...]      Show all exits for one or more runways (BFS)");
        Console.WriteLine("  --bfs <node-id> <twy>    BFS trace from node through taxiway to hold-short");
        Console.WriteLine("  --walk-trace <node-id> <twy>    Single-taxiway WalkTaxiway trace (steps + stop reason)");
        ...
        Console.WriteLine("  --html-taxiway <name>[,<name>...]    Highlight one or more taxiways (also repeatable)");
        Console.WriteLine("  --html-runway <desig>[,<desig>...]   Highlight one or more runways (also repeatable)");
        ...
        Console.WriteLine("  --ticks [LABEL=]<json>   Overlay tick data (TickRecorder JSON); repeat to compare runs, each LABEL names its aircraft");
        Console.WriteLine();
