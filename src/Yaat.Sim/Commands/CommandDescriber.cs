...
public static class CommandDescriber
{
    public static CanonicalCommandType ToCanonicalType(ParsedCommand command)
    {
        return command switch
        {
            ...
            TurnLeftDirectToCommand => CanonicalCommandType.TurnLeftDirectTo,
            TurnRightDirectToCommand => CanonicalCommandType.TurnRightDirectTo,
            ...
            SayExpectedApproachCommand => CanonicalCommandType.SayExpectedApproach,
            SayAltitudeCommand => CanonicalCommandType.SayAltitude,
            SayHeadingCommand => CanonicalCommandType.SayHeading,
            SayPositionCommand => CanonicalCommandType.SayPosition,
            ...
            NoteCommand => CanonicalCommandType.Note,
            ...
            AssumeCommand => CanonicalCommandType.Assume,
            UnassumeCommand => CanonicalCommandType.Unassume,
            ...
            StripScanCommand => CanonicalCommandType.StripScan,
            ...
            // Track commands — routed by RoomEngine before reaching dispatcher, but map correctly
            SetActivePositionCommand => CanonicalCommandType.SetActivePosition,
            TrackAircraftCommand => CanonicalCommandType.TrackAircraft,
            DropTrackCommand => CanonicalCommandType.DropTrack,
            InitiateHandoffCommand => CanonicalCommandType.InitiateHandoff,
            ...
            ConvertPointoutCommand => CanonicalCommandType.ConvertPointout,
            ForceQuicklookCommand => CanonicalCommandType.ForceQuicklook,
    ...
    internal static TrackedCommandType ClassifyCommand(ParsedCommand command)
    {
        return command switch
        {
            ...
            FlyPresentHeadingCommand => TrackedCommandType.Heading,
            ...
            TurnLeftDirectToCommand => TrackedCommandType.Navigation,
            TurnRightDirectToCommand => TrackedCommandType.Navigation,
    ...
    public static string DescribeCommand(ParsedCommand command)
    {
        return command switch
        {
            ...
            TurnLeftDirectToCommand cmd => $"TLDCT {string.Join(" ", cmd.Fixes.Select(f => f.Name))}",
            TurnRightDirectToCommand cmd => $"TRDCT {string.Join(" ", cmd.Fixes.Select(f => f.Name))}",
            ...
            ExitRightCommand er => (er.Taxiway is not null ? $"ER {er.Taxiway}" : "ER") + (er.Expedite ? " EXP" : ""),
            ExitTaxiwayCommand et => $"EXIT {et.Taxiway}" + (et.Expedite ? " EXP" : ""),
            ...
            SayExpectedApproachCommand => "SEAPP",
            ...
            ConvertPointoutCommand => "POHO",
            ForceQuicklookCommand cmd => $"FQL {string.Join(' ', cmd.TcpCodes)}",
    ...
    internal static string DescribeNatural(ParsedCommand command)
    {
        return command switch
        {
            ...
            TurnLeftDirectToCommand cmd => $"Turn left, direct {string.Join(" ", cmd.Fixes.Select(f => f.Name))}",
            TurnRightDirectToCommand cmd => $"Turn right, direct {string.Join(" ", cmd.Fixes.Select(f => f.Name))}",
            ...
            ExitLeftCommand el => (el.Taxiway is not null ? $"Exit left at {el.Taxiway}" : "Exit left") + (el.Expedite ? ", without delay" : ""),
            ...
            SayExpectedApproachCommand => "Say expected approach",
            ...
            AssumeCommand => "Assume live traffic",
            UnassumeCommand => "Return to live traffic",
            ...
            ConvertPointoutCommand => "Convert pointout to handoff",
            ForceQuicklookCommand cmd => $"Force quicklook at {string.Join(", ", cmd.TcpCodes)}",
    ...
    private static string FormatCtoCanonical(ClearedForTakeoffCommand cto)
    {
        var suffix = cto.Departure switch
        {
            ...
            PatternExitDeparture { ExitLeg: PatternEntryLeg.Crosswind, Direction: PatternDirection.Right } => " MRC",
            PatternExitDeparture { ExitLeg: PatternEntryLeg.Downwind, Direction: PatternDirection.Right } => " MRD",
            PatternExitDeparture { ExitLeg: PatternEntryLeg.Crosswind, Direction: PatternDirection.Left } => " MLC",
            PatternExitDeparture { ExitLeg: PatternEntryLeg.Downwind, Direction: PatternDirection.Left } => " MLD",
