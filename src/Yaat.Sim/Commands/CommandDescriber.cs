...
public static class CommandDescriber
{
    ...
    public static string DescribeCommand(ParsedCommand command)
    {
        return command switch
        {
            ...
            WarpGroundCommand cmd => cmd.NodeId is int nid ? $"WARPG !{nid}" : $"WARPG {cmd.Taxiway1} {cmd.Taxiway2}",
    ...
    internal static string DescribeNatural(ParsedCommand command)
    {
        return command switch
        {
            ...
            WarpGroundCommand cmd => cmd.NodeId is int nid2 ? $"Warp to node !{nid2}" : $"Warp to {cmd.Taxiway1}/{cmd.Taxiway2} intersection",
            ...
            DescendViaCommand cmd => cmd.Altitude is not null ? $"Descend via STAR, except maintain {cmd.Altitude:N0}" : "Descend via STAR",
