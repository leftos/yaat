...
public record WarpGroundCommand(string Taxiway1, string Taxiway2, int? NodeId = null, string? ParkingName = null, string? SpotName = null)
    : ParsedCommand;
...
/// the one the aircraft is currently holding short of is satisfied immediately
/// (it starts crossing), and any others are pre-cleared on the upcoming taxi
/// route so the aircraft flows through without stopping. <see cref="HoldShorts"/>
/// optionally arms further hold-shorts on the route (same semantics as
/// <see cref="ResumeCommand.HoldShorts"/>). Both lists are always non-null and
/// empty for bare <c>CROSS</c>, which clears the very next uncleared hold-short
...
public record AtFixCondition(string FixName, double Lat, double Lon, int? Radial = null, int? Distance = null) : BlockCondition
{
    ...
    public static AtFixCondition FromName(string fixName, int? radial = null, int? distance = null)
    {
        (double Lat, double Lon)? pos =
            Data.NavigationDatabase.Instance.GetFixPosition(fixName)
...
/// <summary>
/// A Call-For-Release command for the selected departure (<c>CFR</c>).
/// <paramref name="Action"/> selects set / clear / check; <paramref name="Hhmm"/> is the optional Zulu
/// release time when setting (null → immediate release). Alert-only — never gates takeoff.
/// </summary>
public record CfrDepartureCommand(int? Hhmm, CfrAction Action) : ParsedCommand;
