...
public static class CommandParser
{
    ...
    private static PR ParseByType(CanonicalCommandType type, string? arg, string? aircraftRoute, string rawInput)
    {
        return type switch
        {
            ...
            ClearedIntoMilitaryRoute => MilitaryRouteCommandParser.ParseClearedInto(arg),
            ...
            PositionTurnAltitudeClearance => ApproachCommandParser.ParsePtac(arg, forced: false),
            PositionTurnAltitudeClearanceForce => ApproachCommandParser.ParsePtac(arg, forced: true),
            ...
            // ASDE-X display overrides — empty arg clears the override.
            AsdexScratchpad1 => PR.Ok(new AsdexEditCommand(AsdexEditField.Scratchpad1, arg?.Trim().ToUpperInvariant() ?? "")),
            AsdexScratchpad2 => PR.Ok(new AsdexEditCommand(AsdexEditField.Scratchpad2, arg?.Trim().ToUpperInvariant() ?? "")),
            AsdexCallsign => PR.Ok(new AsdexEditCommand(AsdexEditField.Callsign, arg?.Trim().ToUpperInvariant() ?? "")),
            AsdexBeaconCode => PR.Ok(new AsdexEditCommand(AsdexEditField.BeaconCode, arg?.Trim() ?? "")),
            AsdexCategory => PR.Ok(new AsdexEditCommand(AsdexEditField.Category, arg?.Trim().ToUpperInvariant() ?? "")),
            AsdexAircraftType => PR.Ok(new AsdexEditCommand(AsdexEditField.AircraftType, arg?.Trim().ToUpperInvariant() ?? "")),
            AsdexFix => PR.Ok(new AsdexEditCommand(AsdexEditField.Fix, arg?.Trim().ToUpperInvariant() ?? "")),
            // ASDE-X verb commands — no argument.
            AsdexTagTarget when arg is null => PR.Ok(new AsdexVerbCommand(AsdexVerb.Tag)),
            AsdexTerminate when arg is null => PR.Ok(new AsdexVerbCommand(AsdexVerb.Terminate)),
            AsdexSuspend when arg is null => PR.Ok(new AsdexVerbCommand(AsdexVerb.Suspend)),
            AsdexUnsuspend when arg is null => PR.Ok(new AsdexVerbCommand(AsdexVerb.Unsuspend)),
    ...
    /// <summary>
    /// Resolves a space-separated list of fix names to positions.
    /// Every name must resolve (via navdb or FRD); returns a failure reason on the first unknown.
    /// </summary>
    private static (List<ResolvedFix>? Fixes, string? FailReason) ResolveAllFixes(string[] fixNames)
    {
        ...
        foreach (var name in fixNames)
        {
            ...
            return (null, $"fix not found: {name.ToUpperInvariant()}");
        }

        return resolved.Count > 0
            ? (resolved, null)
            : (null, $"no fixes found (tried: {string.Join(", ", fixNames.Select(n => n.ToUpperInvariant()))})");
    }
    ...
    private static PR ParseDirectTo(string? arg, string? aircraftRoute)
    {
        ...
        return PR.Ok(new DirectToCommand(resolved, []));
    }
    ...
    private static PR ParseAppendDirectTo(string? arg, string? aircraftRoute)
    {
        ...
        return PR.Ok(new AppendDirectToCommand(resolved, []));
    }
    ...
    private static PR ParseAppendForceDirectTo(string? arg, string? aircraftRoute)
    {
        ...
        return PR.Ok(new AppendForceDirectToCommand(resolved, []));
    }
    ...
    private static PR ParseForceDirectTo(string? arg, string? aircraftRoute)
    {
        ...
        foreach (var token in fixNames)
        {
            ...
            return PR.Fail($"fix not found: {name.ToUpperInvariant()}");
        }
        ...
        if (hasConstraints)
        {
            return PR.Ok(new ConstrainedForceDirectToCommand(resolved, altConstraints, null, []));
        }
        ...
        return PR.Ok(new ForceDirectToCommand(resolved, []));
    }
    ...
    private static PR ParseTurnDirectTo(string? arg, string? aircraftRoute, TurnDirection direction)
    {
        ...
        return direction == TurnDirection.Left ? PR.Ok(new TurnLeftDirectToCommand(resolved, [])) : PR.Ok(new TurnRightDirectToCommand(resolved, []));
    }
