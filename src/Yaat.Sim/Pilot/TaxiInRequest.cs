...
public static class TaxiInRequest
{
    ...
    public static bool TryAnnounce(PhaseContext ctx, double phaseElapsedSeconds, string? runwayId, string? taxiway)
    {
        ...
        string? atAirportId = PilotContactRoster.SurfaceAirportOf(aircraft);
        if (ctx.PilotContacts.ResolveFor(aircraft, "GND", atAirportId, ctx.ToEligibilityContext(), false) is not { } answering)
        {
            return false;
        }

        PilotAnsweringPosition? tower = ctx.PilotContacts.ResolveFor(aircraft, "TWR", atAirportId, ctx.ToEligibilityContext(), false);
        ...
        string? parking = ArrivalParkingPicker.Pick(aircraft, ctx.GroundLayout, ctx.ListAircraft?.Invoke() ?? [], 0);
        if (parking is null)
        {
        ...
        PilotSpeechText line = PilotResponder.BuildTaxiInRequest(aircraft, facilityCallName, runwayId, taxiway, parking);
