...
/// <b>slash-compound destination specs</b> (<c>facility/bay/rack/index</c>) across every
...
///
/// <para>The facility segment is required; omitting trailing parts is valid:
/// <c>facility/bay</c> alone appends to the first rack, <c>facility/bay/rack</c>
/// appends to that rack (first free slot bottom-up), and
/// <c>facility/bay/rack/index</c> targets a specific slot. On
...
public static class VStripsCanonicalBuilder
{
    ...
    /// </summary>
    public static string BuildStripMove(string facilityId, string bayName, int rack, int? index) =>
        index is int i
            ? $"STRIP {BaySpec(facilityId, bayName)}/{OneBased(rack)}/{OneBased(i)}"
            : $"STRIP {BaySpec(facilityId, bayName)}/{OneBased(rack)}";
    ...
    /// </summary>
    public static string BuildStripMoveById(string stripId, string facilityId, string bayName, int rack, int? index) =>
        index is int i
            ? $"STRIP {stripId} {BaySpec(facilityId, bayName)}/{OneBased(rack)}/{OneBased(i)}"
            : $"STRIP {stripId} {BaySpec(facilityId, bayName)}/{OneBased(rack)}";
    ...
    /// </summary>
    public static string BuildStripScan(string facilityId, string bayName, int rack, int? index) =>
        index is int i
            ? $"SCAN {BaySpec(facilityId, bayName)}/{OneBased(rack)}/{OneBased(i)}"
            : $"SCAN {BaySpec(facilityId, bayName)}/{OneBased(rack)}";
    ...
    public static string BuildHalfStripMove(string stripId, string destFacilityId, string destBayName, int rack, int index) =>
        $"HSM {stripId} {BaySpec(destFacilityId, destBayName)}/{OneBased(rack)}/{OneBased(index)}";
    ...
    /// </summary>
    public static string BuildSeparatorCreate(SeparatorStyle style, string facilityId, string bayName, int rack, int? index, string? label)
    {
    ...
    /// </summary>
    public static string BuildSeparatorMove(string stripId, string facilityId, string bayName, int rack, int index) =>
        $"SEPM {stripId} {BaySpec(facilityId, bayName)}/{OneBased(rack)}/{OneBased(index)}";
    ...
    /// </summary>
    public static string BuildBlankCreate(string? facilityId, string? bayName, int? rack, int? index)
    {
    ...
    public static string BuildBlankDelete(string facilityId, string bayName, int? rack)
    {
        if (rack is null)
        {
            return $"BLANKD {BaySpec(facilityId, bayName)}";
        }
        return $"BLANKD {BaySpec(facilityId, bayName)}/{OneBased(rack.Value)}";
    }

    /// <summary>
    /// The facility-qualified bay token every verb takes: <c>FACILITY/BAY</c>.
    /// Bay names are only unique within a facility, so a strips window opened for
    /// a non-student facility could not otherwise address its own bays.
    /// </summary>
    private static string BaySpec(string facilityId, string bayName) => $"{facilityId}/{bayName}";
